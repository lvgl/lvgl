/**
 * @file lv_st7796.h
 *
 * This driver is just a wrapper around the generic MIPI compatible LCD controller driver
 *
 */

#ifndef LV_ST7796_H
#define LV_ST7796_H

#ifdef __cplusplus
extern "C" {
#endif

/*********************
 *      INCLUDES
 *********************/

#include "lv_lcd_generic_mipi.h"

#if LV_USE_ST7796

/*********************
 *      DEFINES
 *********************/

/**********************
 *      TYPEDEFS
 **********************/

typedef lv_lcd_send_cmd_cb_t lv_st7796_send_cmd_cb_t;
typedef lv_lcd_send_color_cb_t lv_st7796_send_color_cb_t;

/**********************
 *  GLOBAL PROTOTYPES
 **********************/

/**
 * Create an LCD display with ST7796 driver. The display is unusable until `lv_st7796_init()`
 * runs, so it almost always follows on the next line. Only what the callbacks need goes in
 * between, such as `lv_display_set_user_data()`.
 * @param hor_res       horizontal resolution
 * @param ver_res       vertical resolution
 * @return              pointer to the created display, or NULL on allocation failure
 */
lv_display_t * lv_st7796_create(uint32_t hor_res, uint32_t ver_res);

/**
 * Initialize the ST7796 controller of a display created with `lv_st7796_create()`. Set the
 * callbacks with `lv_lcd_generic_mipi_set_send_cmd_cb()` and
 * `lv_lcd_generic_mipi_set_send_color_cb()` first; this function fails without them.
 * The display is not flushed before this function returns.
 * @param disp          display object
 * @param flags         default configuration settings (mirror, RGB ordering, etc.)
 * @return              LV_RESULT_OK on success
 */
lv_result_t lv_st7796_init(lv_display_t * disp, lv_lcd_flag_t flags);

/**
 * Set gap, i.e., the offset of the (0,0) pixel in the VRAM
 * @param disp          display object
 * @param x             x offset
 * @param y             y offset
 */
void lv_st7796_set_gap(lv_display_t * disp, uint16_t x, uint16_t y);

/**
 * Set color inversion
 * @param disp          display object
 * @param invert        false: normal, true: invert
 */
void lv_st7796_set_invert(lv_display_t * disp, bool invert);

/**
 * Set gamma curve
 * @param disp          display object
 * @param gamma         gamma curve
 */
void lv_st7796_set_gamma_curve(lv_display_t * disp, uint8_t gamma);

/**
 * Send list of commands.
 * @param disp          display object
 * @param cmd_list      controller and panel-specific commands
 */
void lv_st7796_send_cmd_list(lv_display_t * disp, const uint8_t * cmd_list);

/**********************
 *      OTHERS
 **********************/

/**********************
 *      MACROS
 **********************/

#endif /*LV_USE_ST7796*/

#ifdef __cplusplus
} /*extern "C"*/
#endif

#endif /*LV_ST7796_H*/
