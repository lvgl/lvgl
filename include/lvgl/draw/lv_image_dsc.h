...
typedef enum _lvimage_flags_t {
    ...
    /**
     * The buffer's content can be regenerated by its producer if the VRAM
     * backing is lost (e.g., re-decoded from the source file). VRAM-capable
     * draw units may discard the backing under memory pressure; consumers
     * detect the loss through the residency check and reload on demand.
     * Set by the image decoder cache (whose lookup validates the backing
     * and re-decodes dropped entries). Never set on unique content such as
     * canvas buffers.
     */
    LV_IMAGE_FLAGS_RELOADABLE      = 0x0004,
