...
{
    if(!indev) return;
    indev->data.pointer.scroll_throw_vect.x = 0;
    indev->data.pointer.scroll_throw_vect.y = 0;
...
void indev_read_core(lv_indev_t * indev, lv_indev_data_t * data)
{
    ...
    if(indev->type == LV_INDEV_TYPE_POINTER) {
        data->point.x = indev->data.pointer.last_raw_point.x;
        data->point.y = indev->data.pointer.last_raw_point.y;
    }
    /*Similarly set at least the last key in case of the user doesn't set it on release*/
        ...
        data->key = indev->data.keypad.last_key;
    }
...
bool lv_indev_get_press_moved(const lv_indev_t * indev)
{
    ...
    return indev->data.pointer.press_moved;
}
...
void lv_indev_set_cursor(lv_indev_t * indev, lv_obj_t * cur_obj)
{
    ...
    lv_obj_set_pos(indev->cursor, indev->data.pointer.act_point.x, indev->data.pointer.act_point.y);
...
void lv_indev_get_point(const lv_indev_t * indev, lv_point_t * point)
{
    if(indev == NULL) {
        ...
        point->x = indev->data.pointer.act_point.x;
        point->y = indev->data.pointer.act_point.y;
    }
}
...
lv_dir_t lv_indev_get_gesture_dir(const lv_indev_t * indev)
{
    ...
    return indev->data.pointer.gesture_dir;
}
...
uint32_t lv_indev_get_key(const lv_indev_t * indev)
{
    ...
    return indev->data.keypad.last_key;
}
...
uint8_t lv_indev_get_short_click_streak(const lv_indev_t * indev)
{
    ...
    return indev->data.pointer.short_click_streak;
}
...
lv_dir_t lv_indev_get_scroll_dir(const lv_indev_t * indev)
{
    ...
    return indev->data.pointer.scroll_dir;
}
...
lv_obj_t * lv_indev_get_scroll_obj(const lv_indev_t * indev)
{
    ...
    return indev->data.pointer.scroll_obj;
}
...
void lv_indev_get_vect(const lv_indev_t * indev, lv_point_t * point)
{
    ...
    point->x = indev->data.pointer.vect.x;
    point->y = indev->data.pointer.vect.y;
}
...
static void indev_pointer_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    i->data.pointer.last_raw_point.x = data->point.x;
    i->data.pointer.last_raw_point.y = data->point.y;
    ...
    if(i->cursor != NULL &&
       (i->data.pointer.last_point.x != data->point.x || i->data.pointer.last_point.y != data->point.y)) {
        lv_obj_set_pos(i->cursor, data->point.x, data->point.y);
    }

    i->data.pointer.act_point.x = data->point.x;
    i->data.pointer.act_point.y = data->point.y;
    i->data.pointer.diff = data->enc_diff;
    ...
    i->data.pointer.last_point.x = i->data.pointer.act_point.x;
    i->data.pointer.last_point.y = i->data.pointer.act_point.y;
}
...
static void indev_keypad_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    if(i->wait_until_release) {
        ...
        i->pr_timestamp            = 0;
        i->long_pr_sent            = 0;
        i->data.keypad.last_state = LV_INDEV_STATE_RELEASED; /*To skip the processing of release*/
    }
    ...
    uint32_t prev_key = i->data.keypad.last_key;
    i->data.keypad.last_key = data->key;
     ...
     *for the next time*/
    uint32_t prev_state             = i->data.keypad.last_state;
    i->data.keypad.last_state = data->state;
...
static void indev_encoder_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    lv_indev_state_t last_state     = i->data.keypad.last_state;
    i->data.keypad.last_state = data->state;
    i->data.keypad.last_key   = data->key;
...
static void indev_button_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    if(data->state == LV_INDEV_STATE_PRESSED) {
        if(i->data.pointer.last_point.x != x ||
           i->data.pointer.last_point.y != y) {
            indev_proc_release(i);
        }
    }
    ...
    /*Save the new points*/
    i->data.pointer.act_point.x = x;
    i->data.pointer.act_point.y = y;
...
static void indev_proc_press(lv_indev_t * indev)
{
    ...
    indev_obj_act = indev->data.pointer.act_obj;
    ...
    if(indev_obj_act == NULL) {
        indev_obj_act = pointer_search_obj(disp, &indev->data.pointer.act_point);
        new_obj_searched = true;
    }
    /*If there is an active object it's not scrolled and not press locked also search*/
    else if(indev->data.pointer.scroll_obj == NULL &&
    ...
    if(new_obj_searched && indev->data.pointer.scroll_obj) {
    ...
    if(indev_obj_act != indev->data.pointer.act_obj) {
        /*If no previous object was lost, overwrite the last point.*/
        if(indev->data.pointer.act_obj == NULL) {
            indev->data.pointer.last_point.x = indev->data.pointer.act_point.x;
            indev->data.pointer.last_point.y = indev->data.pointer.act_point.y;
        }
        indev->data.pointer.pressed = indev->prev_state == LV_INDEV_STATE_RELEASED;

        /*Without `LV_OBJ_FLAG_PRESS_LOCK` new widget can be found while pressing.*/
        if(indev->data.pointer.last_hovered && indev->data.pointer.last_hovered != indev_obj_act) {
            lv_obj_send_event(indev->data.pointer.last_hovered, LV_EVENT_HOVER_LEAVE, indev);
            if(indev_reset_check(indev)) return;

            lv_indev_send_event(indev, LV_EVENT_HOVER_LEAVE, indev->data.pointer.last_hovered);
            if(indev_reset_check(indev)) return;

            indev->data.pointer.last_hovered = indev_obj_act;
        }
        ...
        if(indev->data.pointer.act_obj != NULL) {
            ...
            lv_obj_t * prev_act_obj = indev->data.pointer.act_obj;
            lv_obj_send_event(prev_act_obj, LV_EVENT_PRESS_LOST, indev_act);
            if(indev_reset_check(indev)) return;
        }

        indev->data.pointer.act_obj  = indev_obj_act; /*Save the pressed object*/
        ...
        if(indev_obj_act != NULL) {
            ...
            indev->long_pr_sent                 = 0;
            indev->data.pointer.scroll_sum.x     = 0;
            indev->data.pointer.scroll_sum.y     = 0;
            indev->data.pointer.scroll_dir = LV_DIR_NONE;
            indev->data.pointer.scroll_obj = NULL;
            indev->data.pointer.gesture_dir = LV_DIR_NONE;
            indev->data.pointer.gesture_sent   = 0;
            indev->data.pointer.gesture_sum.x  = 0;
            indev->data.pointer.gesture_sum.y  = 0;
            indev->data.pointer.press_moved    = 0;
            indev->data.pointer.vect.x         = 0;
            indev->data.pointer.vect.y         = 0;
            ...
            if(indev->prev_state != LV_INDEV_STATE_PRESSED) {
                const bool is_enabled = !lv_obj_has_state(indev_obj_act, LV_STATE_DISABLED);
                if(is_enabled) {
                    if(indev->data.pointer.last_hovered != indev_obj_act) {
    ...
    indev->data.pointer.vect.x = indev->data.pointer.act_point.x - indev->data.pointer.last_point.x;
    indev->data.pointer.vect.y = indev->data.pointer.act_point.y - indev->data.pointer.last_point.y;

    indev->data.pointer.vect_hist[indev->data.pointer.vect_hist_index] = indev->data.pointer.vect;
    indev->data.pointer.vect_hist_timestamp[indev->data.pointer.vect_hist_index] = indev->timestamp;
    indev->data.pointer.vect_hist_index = (indev->data.pointer.vect_hist_index + 1) % LV_INDEV_VECT_HIST_SIZE;

    indev->data.pointer.scroll_throw_vect.x = 0;
    indev->data.pointer.scroll_throw_vect.y = 0;
    for(int i = 0; i < LV_INDEV_VECT_HIST_SIZE; i++) {
        int32_t t = lv_tick_diff(indev->timestamp, indev->data.pointer.vect_hist_timestamp[i]);
        indev->data.pointer.scroll_throw_vect.x += indev_scroll_throw_decay(indev->data.pointer.vect_hist[i].x, t);
        indev->data.pointer.scroll_throw_vect.y += indev_scroll_throw_decay(indev->data.pointer.vect_hist[i].y, t);
    }

    indev->data.pointer.scroll_throw_vect_ori = indev->data.pointer.scroll_throw_vect;

    if(LV_ABS(indev->data.pointer.vect.x) > indev->scroll_limit ||
       LV_ABS(indev->data.pointer.vect.y) > indev->scroll_limit) {
        indev->data.pointer.press_moved = 1;
    }
    ...
    if(indev_obj_act) {
        ...
        if(indev->data.pointer.scroll_obj) {
            lv_obj_stop_scroll_anim(indev->data.pointer.scroll_obj);
        }
        ...
        if(indev->data.pointer.scroll_obj == NULL && indev->long_pr_sent == 0) {
        ...
        if(indev->data.pointer.scroll_obj == NULL && indev->long_pr_sent == 1) {
...
static void indev_proc_release(lv_indev_t * indev)
{
    if(indev->wait_until_release || /*Hover the new widget even if the coordinates didn't changed*/
       (indev->data.pointer.last_point.x != indev->data.pointer.act_point.x ||
        indev->data.pointer.last_point.y != indev->data.pointer.act_point.y)) {
        lv_obj_t ** last = &indev->data.pointer.last_hovered;
        lv_obj_t * hovered = pointer_search_obj(lv_display_get_default(), &indev->data.pointer.act_point);
        if(*last != hovered) {
    ...
    if(indev->wait_until_release) {
        lv_obj_send_event(indev->data.pointer.act_obj, LV_EVENT_PRESS_LOST, indev_act);
        if(indev_reset_check(indev)) {
            indev->wait_until_release = 0;
            return;
        }

        indev->data.pointer.act_obj  = NULL;
        indev->pr_timestamp           = 0;
        indev->longpr_rep_timestamp   = 0;
        indev->wait_until_release     = 0;
    }
    indev_obj_act = indev->data.pointer.act_obj;
    lv_obj_t * scroll_obj = indev->data.pointer.scroll_obj;
    ...
    if(indev_obj_act) {
        ...
        if(scroll_obj) {
            ...
            if(angle != 0 || scale_y != LV_SCALE_NONE || scale_x != LV_SCALE_NONE) {
                ...
                lv_point_transform(&indev->data.pointer.scroll_throw_vect, angle, scale_x, scale_y, &pivot, false);
                lv_point_transform(&indev->data.pointer.scroll_throw_vect_ori, angle, scale_x, scale_y, &pivot, false);
            }
        }
    }
...
static lv_result_t indev_proc_short_click(lv_indev_t * indev)
{
    ...
    indev->data.pointer.short_click_streak++;
    if(lv_tick_diff(indev->timestamp, indev->data.pointer.last_short_click_timestamp) > indev->long_press_time) {
        indev->data.pointer.short_click_streak = 1;
    }
    else if(indev->type == LV_INDEV_TYPE_POINTER || indev->type == LV_INDEV_TYPE_BUTTON) {
        int32_t dx = indev->data.pointer.last_short_click_point.x - indev->data.pointer.act_point.x;
        int32_t dy = indev->data.pointer.last_short_click_point.y - indev->data.pointer.act_point.y;
        if(dx * dx + dy * dy > indev->scroll_limit * indev->scroll_limit) indev->data.pointer.short_click_streak = 1;
    }

    indev->data.pointer.last_short_click_timestamp = indev->timestamp;
    lv_indev_get_point(indev, &indev->data.pointer.last_short_click_point);
    ...
    switch((indev->data.pointer.short_click_streak - 1) % 3) {
        case 0:
...
static void indev_proc_pointer_diff(lv_indev_t * indev)
{
    lv_obj_t * obj = indev->data.pointer.last_pressed;
    if(obj == NULL) return;
    if(indev->data.pointer.diff == 0) return;
    ...
    if(editable) {
        ...
        int32_t diff = (int32_t)((int32_t)indev->data.pointer.diff * indev_sensitivity * obj_sensitivity + 32768) >> 16;
        send_event(LV_EVENT_ROTARY, &diff);
    }
    else {

        int32_t vect = indev->data.pointer.diff > 0 ? indev->scroll_limit : -indev->scroll_limit;
        indev->data.pointer.vect.y = vect;
        indev->data.pointer.act_obj = obj;
        ...
        indev->data.pointer.scroll_throw_vect.y = diff;
        indev->data.pointer.scroll_throw_vect_ori.y = diff;
        lv_indev_scroll_handler(indev);
    }
...
static void indev_proc_reset_query_handler(lv_indev_t * indev)
{
    if(indev->reset_query) {
        indev->data.pointer.act_obj           = NULL;
        indev->data.pointer.scroll_obj        = NULL;
        indev->data.pointer.last_hovered      = NULL;
        ...
        if(indev->type == LV_INDEV_TYPE_ENCODER) {
             ...
             * From v9.5 onward, we no longer reset the key every frame to simplify the user's
             ...
             */
            indev->data.keypad.last_key = LV_KEY_ENTER;
        }
...
static void indev_click_focus(lv_indev_t * indev)
{
    ...
    lv_group_t * g_prev = indev->data.pointer.last_pressed ? lv_obj_get_group(indev->data.pointer.last_pressed) : NULL;
    ...
    if(g_act == g_prev) {
        ...
        /*The object are not in group*/
            ...
            if(indev->data.pointer.last_pressed != indev_obj_act) {
                lv_obj_send_event(indev->data.pointer.last_pressed, LV_EVENT_DEFOCUSED, indev_act);
                if(indev_reset_check(indev)) return;
    ...
    /*The object are not in the same group (in different groups or one has no group)*/
        ...
        if(g_prev == NULL && indev->data.pointer.last_pressed) {
            lv_obj_send_event(indev->data.pointer.last_pressed, LV_EVENT_DEFOCUSED, indev_act);
            if(indev_reset_check(indev)) return;
        }
        /*Focus on a non-group object*/
        else {
            if(indev->data.pointer.last_pressed) {
                ...
                /*If the prev. object also was in a group at least "LEAVE" it instead of defocus*/
                    ...
                    lv_obj_send_event(indev->data.pointer.last_pressed, LV_EVENT_LEAVE, indev_act);
                    if(indev_reset_check(indev)) return;
                }
            }
        }
    ...
    indev->data.pointer.last_pressed = indev_obj_act;
}
...
{
    if(indev->data.pointer.scroll_obj) return;
    if(indev->data.pointer.gesture_sent) return;

    lv_obj_t * gesture_obj = indev->data.pointer.act_obj;
    ...
    if((LV_ABS(indev->data.pointer.vect.x) < indev_act->gesture_min_velocity) &&
       (LV_ABS(indev->data.pointer.vect.y) < indev_act->gesture_min_velocity)) {
        indev->data.pointer.gesture_sum.x = 0;
        indev->data.pointer.gesture_sum.y = 0;
    }

    /*Count the movement by gesture*/
    indev->data.pointer.gesture_sum.x += indev->data.pointer.vect.x;
    indev->data.pointer.gesture_sum.y += indev->data.pointer.vect.y;

    if((LV_ABS(indev->data.pointer.gesture_sum.x) > indev_act->gesture_min_distance) ||
       (LV_ABS(indev->data.pointer.gesture_sum.y) > indev_act->gesture_min_distance)) {

        indev->data.pointer.gesture_sent = 1;

        if(LV_ABS(indev->data.pointer.gesture_sum.x) > LV_ABS(indev->data.pointer.gesture_sum.y)) {
            if(indev->data.pointer.gesture_sum.x > 0)
                indev->data.pointer.gesture_dir = LV_DIR_RIGHT;
            else
                indev->data.pointer.gesture_dir = LV_DIR_LEFT;
        }
        else {
            if(indev->data.pointer.gesture_sum.y > 0)
                indev->data.pointer.gesture_dir = LV_DIR_BOTTOM;
            else
                indev->data.pointer.gesture_dir = LV_DIR_TOP;
        }
...
static void indev_reset_core(lv_indev_t * indev, lv_obj_t * obj)
{
    ...
    if(indev->type == LV_INDEV_TYPE_POINTER || indev->type == LV_INDEV_TYPE_KEYPAD) {
        if(obj == NULL || indev->data.pointer.last_pressed == obj) {
            indev->data.pointer.last_pressed = NULL;
        }

        if(indev->data.pointer.act_obj) {
            /* Avoid recursive calls */
            act_obj = indev->data.pointer.act_obj;
            indev->data.pointer.act_obj = NULL;
        ...
        if(indev->data.pointer.scroll_obj) {
            /* Avoid recursive calls */
            scroll_obj = indev->data.pointer.scroll_obj;
            indev->data.pointer.scroll_obj = NULL;
        ...
        if(obj == NULL || indev->data.pointer.last_hovered == obj) {
            indev->data.pointer.last_hovered = NULL;
        }
    }
}
...
static void indev_scroll_throw_anim_cb(void * var, int32_t v)
{
    ...
    if(indev->data.pointer.scroll_dir == LV_DIR_NONE || indev->data.pointer.scroll_obj == NULL) {
        if(indev->scroll_throw_anim) {
