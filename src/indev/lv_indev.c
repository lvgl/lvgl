...
#if LV_USE_EXT_DATA
void lv_indev_set_external_data(lv_indev_t * indev, void * data, void (* destructor)(void * data))
{
    if(!indev) {
        LV_LOG_WARN("Can't attach external user data and destructor callback to a NULL indev");
        return;
    }

    indev->ext_data.data = data;
    indev->ext_data.free_cb = destructor;
}
#endif
