...
void lv_obj_scroll_by(lv_obj_t * obj, lv_coord_t dx, lv_coord_t dy, lv_anim_enable_t anim_en)
{
    ...
    if(anim_en == LV_ANIM_ON) {
        ...
        if(dx) {
            uint32_t t = lv_anim_speed_to_time((lv_disp_get_horizonal_resolution(d) * 2) >> 2, 0, dx);
            ...
            lv_res_t res;
            res = lv_obj_send_event(obj, LV_OBJ_EVENT_SCROLL_BEGIN, &a);
            if(res != LV_RES_OK) return;
            lv_anim_start(&a);
        }

        if(dy) {
            uint32_t t = lv_anim_speed_to_time((lv_disp_get_vertical_resolution(d) * 2) >> 2, 0, dy);
            if(t < SCROLL_ANIM_TIME_MIN) t = SCROLL_ANIM_TIME_MIN;
            if(t > SCROLL_ANIM_TIME_MAX) t = SCROLL_ANIM_TIME_MAX;
            lv_anim_set_time(&a, t);
        ...
        lv_res_t res;
        res = lv_obj_send_event(obj, LV_OBJ_EVENT_SCROLL_BEGIN, NULL);
        if(res != LV_RES_OK) return;

        res = _lv_obj_scroll_by_raw(obj, dx, dy);
        if(res != LV_RES_OK) return;

        res = lv_obj_send_event(obj, LV_OBJ_EVENT_SCROLL_END, NULL);
        if(res != LV_RES_OK) return;
    }
}
...
lv_res_t _lv_obj_scroll_by_raw(lv_obj_t * obj, lv_coord_t x, lv_coord_t y)
{
    ...
    lv_res_t res = lv_obj_send_event(obj, LV_OBJ_EVENT_SCROLL, NULL);
...
static void scroll_anim_ready_cb(lv_anim_t * a)
{
    lv_obj_send_event(a->var, LV_OBJ_EVENT_SCROLL_END, NULL);
}
                                  ...
                                  lv_anim_enable_t anim_en)
{
    ...
    if(y_del || x_del) {
        lv_res_t res;
        res = lv_obj_send_event(parent, LV_OBJ_EVENT_SCROLL_END, NULL);
        if(res != LV_RES_OK) return;
    }
