...
static lv_obj_t * find_scroll_obj(lv_indev_t * indev)
{
    ...
    bool hor_en = false;
    bool ver_en = false;
    if(LV_ABS(indev->pointer.scroll_sum.x) > LV_ABS(indev->pointer.scroll_sum.y)) {
        hor_en = true;
    }
    else {
        ver_en = true;
    ...
    while(obj_act) {
        ...
        if((st > 0 || sb > 0)  &&
           ((up_en    && indev->pointer.scroll_sum.y >=   scroll_limit) ||
            (down_en  && indev->pointer.scroll_sum.y <= - scroll_limit))) {
            obj_candidate = obj_act;
            dir_candidate = LV_DIR_VER;
        }

        if((sl > 0 || sr > 0)  &&
           ((left_en   && indev->pointer.scroll_sum.x >=   scroll_limit) ||
            (right_en  && indev->pointer.scroll_sum.x <= - scroll_limit))) {
            obj_candidate = obj_act;
        ...
        if((left_en  && indev->pointer.scroll_sum.x >=   scroll_limit) ||
           (right_en && indev->pointer.scroll_sum.x <= - scroll_limit) ||
           (up_en    && indev->pointer.scroll_sum.y >=   scroll_limit) ||
           (down_en  && indev->pointer.scroll_sum.y <= - scroll_limit)) {
