...
static void indev_pointer_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    /*Simple sanity check*/
    if(data->point.x < 0) {
        LV_LOG_WARN("X is %" LV_PRId32 " which is smaller than zero", data->point.x);
    }
    if(data->point.x >= lv_disp_get_hor_res(i->driver->disp)) {
        LV_LOG_WARN("X is %" LV_PRId32 " which is greater than hor. res", data->point.x);
    }
    if(data->point.y < 0) {
        LV_LOG_WARN("Y is %" LV_PRId32 " which is smaller than zero", data->point.y);
    }
    if(data->point.y >= lv_disp_get_ver_res(i->driver->disp)) {
        LV_LOG_WARN("Y is %" LV_PRId32 " which is greater than ver. res", data->point.y);
    }
...
static void indev_button_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    if(prev_state != data->state) {
        if(data->state == LV_INDEV_STATE_PRESSED) {
            LV_LOG_INFO("button %" LV_PRIu32 " is pressed (x:%" LV_PRId32 " y:%" LV_PRId32 ")", data->btn_id, x, y);
        }
        else {
            LV_LOG_INFO("button %" LV_PRIu32 " is released (x:%" LV_PRId32 " y:%" LV_PRId32 ")", data->btn_id, x, y);
        }
    }
...
static void indev_proc_press(_lv_indev_proc_t * proc)
{
    LV_LOG_INFO("pressed at x:%" LV_PRId32 " y:%" LV_PRId32, proc->types.pointer.act_point.x,
