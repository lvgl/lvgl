...
static inline const lv_img_src_t * lv_obj_get_style_bg_img_src(const struct _lv_obj_t * obj, uint32_t part)
{
    lv_style_value_t v = lv_obj_get_style_prop(obj, part, LV_STYLE_BG_IMG_SRC);
    return (const lv_img_src_t *)v.ptr;
}
...
void lv_obj_set_style_bg_img_src(struct _lv_obj_t * obj, const lv_img_src_t * value, lv_style_selector_t selector);
