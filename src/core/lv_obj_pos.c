...
bool lv_obj_refr_size(lv_obj_t * obj)
{
    ...
    if(obj->w_layout) {
        ...
        if(w_is_content) {
            ...
            if(parent->w_layout == 0 && lv_obj_get_style_width(parent, LV_PART_MAIN) == LV_SIZE_CONTENT) {
                w = lv_obj_get_style_space_left(obj, LV_PART_MAIN) + lv_obj_get_style_space_right(obj, LV_PART_MAIN);
            }
            else {
    ...
    if(obj->h_layout) {
        ...
        if(h_is_content) {
            ...
            /*If parent has content size and the child has pct size
             *a circular dependency will occur. To solve it keep child size at zero */
            if(parent->h_layout == 0 && lv_obj_get_style_height(parent, LV_PART_MAIN) == LV_SIZE_CONTENT) {
                h = lv_obj_get_style_space_top(obj, LV_PART_MAIN) + lv_obj_get_style_space_bottom(obj, LV_PART_MAIN);
            }
            else {
