...
void lv_group_del(lv_group_t * group)
{
    ...
    if(group->obj_focus != NULL) {
        lv_obj_send_event(*group->obj_focus, LV_OBJ_EVENT_DEFOCUSED, get_indev(group));
...
void lv_group_remove_obj(lv_obj_t * obj)
{
    ...
    if(g->obj_focus && *g->obj_focus == obj) {
        ...
        if(_lv_ll_get_head(&g->obj_ll) == g->obj_focus && _lv_ll_get_tail(&g->obj_ll) == g->obj_focus) {
            lv_obj_send_event(*g->obj_focus, LV_OBJ_EVENT_DEFOCUSED, get_indev(g));
        }
    ...
    _LV_LL_READ(&g->obj_ll, i) {
        if(*i == obj) {
            if(g->obj_focus != NULL && obj != *g->obj_focus) {  /*Do not defocus if the same object needs to be focused again*/
                lv_res_t res = lv_obj_send_event(*g->obj_focus, LV_OBJ_EVENT_DEFOCUSED, get_indev(g));
            ...
            if(g->obj_focus != NULL) {
                ...
                lv_res_t res = lv_obj_send_event(*g->obj_focus, LV_OBJ_EVENT_FOCUSED, get_indev(g));
                if(res != LV_RES_OK) return;
                lv_obj_invalidate(*g->obj_focus);
            }
            break;
...
lv_res_t lv_group_send_data(lv_group_t * group, uint32_t c)
{
    ...
    return lv_obj_send_event(act, LV_OBJ_EVENT_KEY, &c);
}
...
void lv_group_set_editing(lv_group_t * group, bool edit)
{
    ...
    if(focused) {
        lv_res_t res = lv_obj_send_event(*group->obj_focus, LV_OBJ_EVENT_FOCUSED, get_indev(group));
        if(res != LV_RES_OK) return;
                            ...
                            void * (*move)(const lv_ll_t *, const void *))
{
    ...
    if(group->obj_focus) {
        lv_res_t res = lv_obj_send_event(*group->obj_focus, LV_OBJ_EVENT_DEFOCUSED, get_indev(group));
