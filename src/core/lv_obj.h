...
typedef void (*lv_delete_cb_t)(void * item);
 ...
 *
 * The registered callback will be automatically invoked with `item` as its
 * argument when the object is deleted, allowing associated resources to be
 * released or any other cleanup logic to be executed by the callback.
 *
 * This is a utility function that simplifies attaching an `LV_EVENT_DELETE`
 * event callback to `obj` and passing `item` to that callback when the
 * object is deleted.
 *
 * @param obj   Pointer to the LVGL object to attach the delete callback to.
 * @param cb    The delete callback function to register.
 * @param item  User data pointer passed to `cb` when the object is deleted.
 *
