                    ...
                    lv_color_t color, lv_opa_t opa)
{
    ...
    lv_coord_t pos_x = pos_p->x + g.ofs_x;// (g.ofs_x + 3) / 3;
    ...
    lv_coord_t col_start = pos_x >= mask_p->x1 ? 0 : (mask_p->x1 - pos_x) * 3;
    lv_coord_t col_end   = pos_x + g.box_w/3 <= mask_p->x2 ? g.box_w : (mask_p->x2 - pos_x + 1) * 3;
    lv_coord_t row_start = pos_y >= mask_p->y1 ? 0 : mask_p->y1 - pos_y;
    lv_coord_t row_end   = pos_y + g.box_h <= mask_p->y2 ? g.box_h : mask_p->y2 - pos_y + 1;

    bool subpx = true;

    /*Be sure the letter starts from a full px*/
    if(subpx) {
        ...
        for(col = col_start; col < col_end; col++) {
            ...
            if(sub_px_cnt == 3)
            {
                if(disp->driver.set_px_cb) {
                    ...
                    vdb_buf_tmp->ch.red = (uint16_t)((uint16_t)txt_rgb[0] * font_rgb[0] + (bg_rgb[0] * (255 - font_rgb[0]))) >> 8;
                    vdb_buf_tmp->ch.green = (uint16_t)((uint16_t)txt_rgb[1] * font_rgb[1] + (bg_rgb[1] * (255 - font_rgb[1]))) >> 8;
                    vdb_buf_tmp->ch.blue = (uint16_t)((uint16_t)txt_rgb[2] * font_rgb[2] + (bg_rgb[2] * (255 - font_rgb[2]))) >> 8;
...
//                    }
        ...
        map_p += (col_bit >> 3);
        col_bit = col_bit & 0x7;
        vdb_buf_tmp += vdb_width - (col_end - col_start)/3; /*Next row in VDB*/
    }
}
