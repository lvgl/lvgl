...
err:
    if(buf->handle) {
        ...
        (void)drmCloseBufferHandle(drm_dev->fd, buf->handle);
        buf->handle = 0;
    }
...
static void drm_free(lv_event_t * e)
{
    ...
    /* Final moments of drm_dev, free the draw buffers as well */
    if(drm_dev->draw_buf) {
        lv_free(drm_dev->draw_buf);
        drm_dev->draw_buf = NULL;
    }
    if(drm_dev->draw_buf_2) {
        lv_free(drm_dev->draw_buf_2);
        drm_dev->draw_buf_2 = NULL;
    }
    lv_free(drm_dev);
}
