...
static EGLConfig create_egl_config(lv_opengles_egl_t * ctx)
{
    const EGLint config_attribs[] = {
        EGL_RENDERABLE_TYPE,
        EGL_OPENGL_ES2_BIT,
        /* Ask for all surface types and let `select_config` pick the kind it
         * needs, which every driver's callback already filters on. */
        EGL_SURFACE_TYPE,
        EGL_DONT_CARE,
        EGL_NONE
    };
    ...
    /* Compact both arrays in lockstep while dropping configs that cannot be queried,
     * so that the index `select_config` returns still addresses the matching EGLConfig. */
...
static EGLContext create_egl_context(lv_opengles_egl_t * ctx)
{
    /* Ask for ES3 first: the glTF renderer needs "#version 300 es" shaders. Everything
     * else in LVGL is ES2 code, which runs unchanged on an ES3 context, so there is
     * nothing to lose by preferring it. Drivers that cannot provide ES3 fall back to
     * ES2, where only glTF becomes unavailable. */
