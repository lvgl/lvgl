...
struct buffer * decorators_buf[8] = { NULL };
/**********************
 *      MACROS
 **********************/
...
static struct buffer * lv_wayland_dmabuf_create_draw_buffers_internal(struct window * window, int width, int height)
{
    ...
    for(int i = 0; i < LV_WAYLAND_BUF_COUNT; i++) {
        ...
        drmcf = lv_wayland_dmabuf_get_format(window);
                                ...
                                int32_t fd, uint32_t size)
{
    ...
    /* Look for preferred formats based on color depth */
    bool format_found = false;

    for(size_t i = 0; i < num_formats && !format_found; i++) {
    ...
    if(!format_found && ctx->format == DRM_FORMAT_INVALID) {
        LV_LOG_WARN("No suitable format found in table, using default");
        ctx->format = (LV_COLOR_DEPTH == 32) ? DRM_FORMAT_ARGB8888 : DRM_FORMAT_RGB565;
    }
...
static struct buffer * dmabuf_acquire_buffer(dmabuf_ctx_t * context, unsigned char * color_p)
{
    ...
    struct buffer * buffer = &context->buffers[buf_num];
    while(found) {
        wl_display_roundtrip(lv_wl_ctx.display);
...
static void create_decorators_buf(struct window * window, struct graphic_object * decoration)
{
    ...
    decorators_buf[id] = (struct buffer *)calloc(1, sizeof(struct buffer));
    LV_ASSERT_MALLOC(decorators_buf[id]);

    const int stride        = lv_draw_buf_width_to_stride(decoration->width, lv_display_get_color_format(window->lv_disp));
    decorators_buf[id]->window = window;
    decorators_buf[id]->lv_draw_buf =
        lv_draw_buf_create(decoration->width, decoration->height, lv_display_get_color_format(window->lv_disp), stride);

    decorators_buf[id]->strides[0]    = stride;
    decorators_buf[id]->width        = decoration->width;
    decorators_buf[id]->height       = decoration->height;
    decorators_buf[id]->dmabuf_fds[0] = g2d_get_buf_fd(decorators_buf[id]->lv_draw_buf);
    decorators_buf[id]->buf_base[0]   = decorators_buf[id]->lv_draw_buf->data;
    params                   = zwp_linux_dmabuf_v1_create_params(window->wl_ctx->dmabuf_ctx.handler);

    zwp_linux_buffer_params_v1_add(params, decorators_buf[id]->dmabuf_fds[0], 0, decorators_buf[id]->offsets[0],
                                   decorators_buf[id]->strides[0], 0,
                                   0);

    zwp_linux_buffer_params_v1_add_listener(params, &params_listener, decorators_buf[id]);
...
static void destroy_decorators_buf(uint8_t id)
{
    if(decorators_buf[id] != NULL) {
        buffer_free(decorators_buf[id]);
        free(decorators_buf[id]);
        decorators_buf[id] = NULL;
    }
}
...
{
    uint8_t id = decoration->type;
    if(decorators_buf[id] == NULL || (decorators_buf[id]->width == (uint32_t)decoration->width &&
                                      decorators_buf[id]->height == (uint32_t)decoration->height)) {
        create_decorators_buf(window, decoration);

        return decorators_buf[id];
    }
    else {
        destroy_decorators_buf(id);
        create_decorators_buf(window, decoration);
