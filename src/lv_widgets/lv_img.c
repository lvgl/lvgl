...
static lv_design_res_t lv_img_design(lv_obj_t * obj, const lv_area_t * clip_area, lv_design_mode_t mode);
...
static lv_design_res_t lv_img_design(lv_obj_t * obj, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        if(img->src_type == LV_IMG_SRC_UNKNOWN || img->src_type == LV_IMG_SRC_SYMBOL) return LV_DESIGN_RES_NOT_COVER;

        /*Non true color format might have "holes"*/
        if(img->cf != LV_IMG_CF_TRUE_COLOR && img->cf != LV_IMG_CF_RAW) return LV_DESIGN_RES_NOT_COVER;

        /*With not LV_OPA_COVER images can't cover an area */
        if(lv_obj_get_style_img_opa(obj, LV_PART_MAIN) != LV_OPA_COVER) return LV_DESIGN_RES_NOT_COVER;
        ...
        if(zoom_final == LV_IMG_ZOOM_NONE) {
            if(_lv_area_is_in(clip_area, &obj->coords, 0) == false) return LV_DESIGN_RES_NOT_COVER;
        }
        else {
            lv_area_t a;
        ...
        if(lv_obj_get_style_img_blend_mode(obj, LV_PART_MAIN) != LV_BLEND_MODE_NORMAL) return LV_DESIGN_RES_NOT_COVER;
#endif
