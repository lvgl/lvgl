...
void lv_canvas_set_px(lv_obj_t * obj, int32_t x, int32_t y, lv_color_t color, lv_opa_t opa)
{
    ...
    lv_color_format_t cf = lv_draw_buf_get_color_format(draw_buf);
    ...
    if(LV_COLOR_FORMAT_IS_INDEXED(cf)) {
        ...
        uint8_t * buf = lv_draw_buf_get_data(canvas->draw_buf);
        buf += 8;
        buf += y * stride;
        buf += x >> 3;
...
lv_color32_t lv_canvas_get_px(lv_obj_t * obj, int32_t x, int32_t y)
{
    ...
    lv_color_format_t cf = lv_draw_buf_get_color_format(canvas->draw_buf);
...
void lv_canvas_fill_bg(lv_obj_t * obj, lv_color_t color, lv_opa_t opa)
{
    ...
    uint32_t stride = lv_draw_buf_get_stride(draw_buf);
    uint8_t * data = lv_draw_buf_get_data(draw_buf);
    lv_color_format_t cf = lv_draw_buf_get_color_format(draw_buf);
    int32_t w = lv_draw_buf_get_width(draw_buf);
    int32_t h = lv_draw_buf_get_height(draw_buf);
...
void lv_canvas_init_layer(lv_obj_t * obj, lv_layer_t * layer)
{
    ...
    int32_t w = lv_draw_buf_get_width(canvas->draw_buf);
    int32_t h = lv_draw_buf_get_height(canvas->draw_buf);
    lv_color_format_t cf = lv_draw_buf_get_color_format(canvas->draw_buf);
    lv_area_t canvas_area = {0, 0, w - 1,  h - 1};
    lv_memzero(layer, sizeof(*layer));

    layer->buf = canvas->draw_buf;
