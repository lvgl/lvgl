...
static void draw_main(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_DRAW_MAIN) {
        ...
        lv_draw_rect(draw, &sel_dsc, &sel_area);
    }
    /*Post draw when the children are drawn*/
        ...
        lv_area_t mask_sel;
        bool area_ok;
        area_ok = _lv_area_intersect(&mask_sel, draw->clip_area, &sel_area);
        if(area_ok) {
            lv_obj_t * label = get_label(obj);
            ...
            lv_draw_label(draw, &label_dsc, &label_sel_area, lv_label_get_text(label), NULL);
...
static void draw_label(lv_event_t * e)
{
    ...
    if(_lv_area_intersect(&clip2, draw->clip_area, &clip2)) {
        const lv_area_t * clip_area_ori = draw->clip_area;
        draw->clip_area = &clip2;
        lv_draw_label(draw, &label_draw_dsc, &label_obj->coords, lv_label_get_text(label_obj), NULL);
        draw->clip_area = clip_area_ori;
    }
