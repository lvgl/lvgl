...
static lv_result_t add_char_internal(lv_obj_t * obj, uint32_t c);
...
void lv_textarea_add_char(lv_obj_t * obj, uint32_t c)
{
    ...
    lv_result_t res = add_char_internal(obj, c);
    if(res != LV_RESULT_OK) {
        return;
    }
...
static void add_text(lv_obj_t * obj, const char * txt)
{
    ...
    while(txt[i] != '\0') {
        ...
        lv_result_t res = add_char_internal(obj, lv_text_unicode_to_encoded(c));
        if(res != LV_RESULT_OK) {
            continue;
        }
...
static lv_result_t add_char_internal(lv_obj_t * obj, uint32_t c)
{
    ...
    /* Try to allocate memory for pwd mode first, if we fial we don't insert the character either*/
    if(ta->pwd_mode) {
        /*+2: the new char + \0*/
