...
void lv_keyboard_def_event_cb(lv_event_t * e)
{
    ...
    if(strcmp(txt, "abc") == 0) {
        ...
        lv_res_t res = lv_obj_send_event(obj, LV_OBJ_EVENT_CANCEL, NULL);
        if(res != LV_RES_OK) return;

        if(keyboard->ta) {
            res = lv_obj_send_event(keyboard->ta, LV_OBJ_EVENT_CANCEL, NULL);
            if(res != LV_RES_OK) return;
        }
        return;
    }
    else if(strcmp(txt, LV_SYMBOL_OK) == 0) {
        lv_res_t res = lv_obj_send_event(obj, LV_OBJ_EVENT_READY, NULL);
        if(res != LV_RES_OK) return;

        if(keyboard->ta) {
            res = lv_obj_send_event(keyboard->ta, LV_OBJ_EVENT_READY, NULL);
            if(res != LV_RES_OK) return;
        }
        return;
    }
    ...
    if(strcmp(txt, "Enter") == 0 || strcmp(txt, LV_SYMBOL_NEW_LINE) == 0) {
        ...
        if(lv_textarea_get_one_line(keyboard->ta)) {
            lv_res_t res = lv_obj_send_event(keyboard->ta, LV_OBJ_EVENT_READY, NULL);
            if(res != LV_RES_OK) return;
        }
    }
...
static void lv_keyboard_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    lv_obj_add_event_cb(obj, lv_keyboard_def_event_cb, LV_OBJ_EVENT_VALUE_CHANGED, NULL);
