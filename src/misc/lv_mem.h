 ...
 */
#define LV_ZALLOC(v, s) do { void * p = lv_malloc(s); LV_ASSERT_MALLOC(p); if (p != NULL) lv_memzero(p, s); lv_memcpy(&v, &p, sizeof(p)); } while(0)
/**
 * Allocate the object pointed by v, check and zero in one operation
 */
#define LV_ZERO_ALLOC(v) do { void * p = lv_malloc(sizeof(*v)); LV_ASSERT_MALLOC(p); if (p != NULL) lv_memzero(p, sizeof(*v)); lv_memcpy(&v, &p, sizeof(p)); } while(0)
