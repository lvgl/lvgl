 ...
 *
 * data will point to new memory on successful reallocation. On failure data will not be modified.
 *
 * Behavior depends of data and new_size:
 * - *data != NULL and new_size != 0: Simple reallocation. Equivalent to realloc(*data, new_size)
 * - *data == NULL and new_size != 0: Equivalent to lv_mem_alloc(new_size)
 * - *data != NULL and new_size == 0: free(data) but sets it to valid (special) pointer.
 * - Any other set of inputs are invalid and will return LV_RES_INV with no operations being
 * performed.
 *
 * @param data      pointer to pointer to pointer to reallocate.
 * @param new_size  the desired new size in bytes
 * @return          LV_RES_OK when the operation is successful, LV_RES_INV otherwise.
 */
