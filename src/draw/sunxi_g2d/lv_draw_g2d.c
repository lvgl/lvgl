...
static bool _g2d_draw_img_supported(const lv_draw_image_dsc_t * draw_dsc, lv_coord_t size)
{
    const lv_image_dsc_t * img_dsc = draw_dsc->src;

    bool has_recolor = (draw_dsc->recolor_opa != LV_OPA_TRANSP);
    bool has_transform = (draw_dsc->rotation != 0 || draw_dsc->zoom_x != LV_SCALE_NONE ||
    ...
    if(((draw_dsc->zoom_x != LV_SCALE_NONE) || (draw_dsc->zoom_y != LV_SCALE_NONE)) &&
...
static int32_t _g2d_evaluate(lv_draw_unit_t * u, lv_draw_task_t * t)
{
    ...
    lv_coord_t size = lv_area_get_width(&img_area) * lv_area_get_height(&img_area);
    switch(t->type) {
        ...
        case LV_DRAW_TASK_TYPE_BG_IMG: {
                ...
                if(!_g2d_cf_supported(draw_dsc->img_header.cf))
                    return 0;
...
static void _g2d_execute_drawing(lv_draw_g2d_unit_t * u)
{
    ...
    lv_draw_buf_invalidate_cache(&layer->buf, lv_draw_buf_width_to_stride(lv_area_get_width(&layer->buf_area),
                                                                          layer->color_format), layer->color_format, (const lv_area_t *)&t->area);
    ...
    switch(t->type) {
        case LV_DRAW_TASK_TYPE_FILL:
            ...
            break;
        case LV_DRAW_TASK_TYPE_BG_IMG:
            lv_draw_g2d_bg_img(draw_unit, t->draw_dsc, &t->area);
            break;
        case LV_DRAW_TASK_TYPE_IMAGE:
