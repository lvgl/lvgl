...
static void _g2d_blit_recolor(uint8_t * dest_buf, const lv_area_t * dest_area, lv_coord_t dest_stride,
...
static void _g2d_blit_transform(uint8_t * dest_buf, const lv_area_t * dest_area, lv_coord_t dest_stride,
                                lv_color_format_t dest_cf, const uint8_t * src_buf, const lv_area_t * src_area,
                                lv_coord_t src_stride, lv_color_format_t src_cf, const lv_draw_image_dsc_t * dsc, const lv_area_t * disp_area);

/* Blit simple w/ opa and alpha channel */
static void _g2d_blit(uint8_t * dest_buf, const lv_area_t * dest_area, lv_coord_t dest_stride,
                      lv_color_format_t dest_cf, const uint8_t * src_buf, const lv_area_t * src_area,
                     ...
                     const lv_area_t * coords)
{
    ...
    bool has_transform = dsc->rotation != 0 || dsc->zoom_x != LV_SCALE_NONE || dsc->zoom_y != LV_SCALE_NONE;
    /*if(has_transform)
    ...
    lv_result_t res = lv_image_decoder_open(&decoder_dsc, dsc->src, dsc->recolor, -1);
    if(res != LV_RESULT_OK) {
        LV_LOG_ERROR("Failed to open image");
        return;
    }
    lv_coord_t src_stride = decoder_dsc.header.stride;
    lv_color_format_t src_cf = img_dsc->header.cf;

    uint8_t * dest_buf = layer->buf;
    lv_coord_t dest_stride = lv_draw_buf_width_to_stride(lv_area_get_width(&layer->buf_area), layer->color_format);
    lv_color_format_t dest_cf = layer->color_format;
    bool has_recolor = (dsc->recolor_opa != LV_OPA_TRANSP);

    if(has_recolor && !has_transform)
        _g2d_blit_recolor(dest_buf, &blend_area, dest_stride, dest_cf,
                          src_buf, &src_area, src_stride, src_cf, dsc);
    else if(has_transform) {
        lv_area_t disp_area;
        lv_area_copy(&disp_area, &layer->buf_area);
        _g2d_blit_transform(dest_buf, &blend_area, dest_stride, dest_cf,
                            src_buf, &src_area, src_stride, src_cf, dsc, &disp_area);
    }
    else
                              ...
                              lv_coord_t src_stride, lv_color_format_t src_cf, const lv_draw_image_dsc_t * dsc)
{
    ...
    hal_dcache_clean_invalidate((unsigned long)(src_buf + src_stride * src_area->y1 + src_px_size * src_area->x1),
                                src_stride * src_h);
    hal_dcache_clean_invalidate((unsigned long)(dest_buf + dest_stride * dest_area->y1 + dest_px_size * dest_area->x1),
                                dest_stride * dest_h);
                                ...
                                lv_coord_t src_stride, lv_color_format_t src_cf, const lv_draw_image_dsc_t * dsc, const lv_area_t * disp_area)
{
    ...
    bool has_scale = (dsc->zoom_x != LV_SCALE_NONE) || (dsc->zoom_y != LV_SCALE_NONE);
    uint8_t src_px_size = lv_color_format_get_size(src_cf);
    uint8_t dest_px_size = lv_color_format_get_size(dest_cf);

    //LV_LOG_WARN("src_w=%d src_h=%d\n", src_w, src_h);
    //LV_LOG_WARN("pivot.x=%d pivot.y=%d\n", pivot.x, pivot.y);
    if(has_scale) {
        lv_area_t zoom_area;
        _lv_image_buf_get_transformed_area(&zoom_area, src_w, src_h, 0, dsc->zoom_x, dsc->zoom_y, &pivot);
        //LV_LOG_WARN("zoom_area->x1=%d zoom_area->y1=%d zoom_area->x2=%d zoom_area->y2=%d\n", zoom_area.x1, zoom_area.y1, zoom_area.x2, zoom_area.y2);
        lv_area_move(&zoom_area, pivot.x - (zoom_area.x1 + zoom_area.x2) / 2, pivot.y - (zoom_area.y1 + zoom_area.y2) / 2);
        //LV_LOG_WARN("zoom_area->x1=%d zoom_area->y1=%d zoom_area->x2=%d zoom_area->y2=%d\n", zoom_area.x1, zoom_area.y1, zoom_area.x2, zoom_area.y2);
        lv_coord_t zoom_w = lv_area_get_width(&zoom_area);
        lv_coord_t zoom_h = lv_area_get_height(&zoom_area);
        lv_color_t * scale_buf = lv_draw_buf_malloc(zoom_w * zoom_h * dest_px_size, dest_cf);
        ...
        hal_dcache_clean_invalidate((unsigned long)(src_buf + src_stride * src_area->y1 + src_px_size * src_area->x1),
                                    src_stride * src_h);
        hal_dcache_clean_invalidate((unsigned long)scale_buf_aligned, zoom_w * zoom_h * dest_px_size);
        if(sunxi_g2d_control(G2D_CMD_BITBLT_H, &info) < 0) {
            lv_draw_buf_free(scale_buf);
            return;
        }
        lv_area_t src_area_clip, dest_area_clip;
        lv_area_copy(&src_area_clip, src_area);
        //LV_LOG_WARN("src_area_clip->x1=%d src_area_clip->y1=%d src_area_clip->x2=%d src_area_clip->y2=%d\n", src_area_clip.x1, src_area_clip.y1, src_area_clip.x2, src_area_clip.y2);
        if(!_lv_area_intersect(&src_area_clip, &zoom_area, &src_area_clip))
            return;
        //LV_LOG_WARN("src_area_clip->x1=%d src_area_clip->y1=%d src_area_clip->x2=%d src_area_clip->y2=%d\n", src_area_clip.x1, src_area_clip.y1, src_area_clip.x2, src_area_clip.y2);
        lv_area_move(&src_area_clip, -zoom_area.x1, -zoom_area.y1);
        //LV_LOG_WARN("src_area_clip->x1=%d src_area_clip->y1=%d src_area_clip->x2=%d src_area_clip->y2=%d\n", src_area_clip.x1, src_area_clip.y1, src_area_clip.x2, src_area_clip.y2);

        lv_area_copy(&dest_area_clip, dest_area);
        lv_area_move(&zoom_area, (dest_area_clip.x1 + dest_area_clip.x2) / 2 - (zoom_area.x1 + zoom_area.x2) / 2,
                     (dest_area_clip.y1 + dest_area_clip.y2) / 2 - (zoom_area.y1 + zoom_area.y2) / 2);
        if(!_lv_area_intersect(&dest_area_clip, &zoom_area, &dest_area_clip))
            return;
        //LV_LOG_WARN("src_area_clip->x1=%d src_area_clip->y1=%d src_area_clip->x2=%d src_area_clip->y2=%d\n", src_area_clip.x1, src_area_clip.y1, src_area_clip.x2, src_area_clip.y2);
        //LV_LOG_WARN("dest_area_clip->x1=%d dest_area_clip->y1=%d dest_area_clip->x2=%d dest_area_clip->y2=%d\n", dest_area_clip.x1, dest_area_clip.y1, dest_area_clip.x2, dest_area_clip.y2);
        _g2d_blit(dest_buf, &dest_area_clip, dest_stride, dest_cf,
                  scale_buf_aligned, &src_area_clip, zoom_w * dest_px_size, dest_cf, dsc->opa);
                      ...
                      lv_coord_t src_stride, lv_color_format_t src_cf, lv_opa_t opa)
{
    ...
    if(opa >= (lv_opa_t)LV_OPA_MAX && !src_has_alpha) {
        ...
        info.src_image[1].laddr[0] = (uintptr_t)(src_buf + src_stride * src_area->y1 + src_px_size * src_area->x1);
        ...
        info.dst_image.laddr[0] = (uintptr_t)(dest_buf + dest_stride * dest_area->y1 + dest_px_size * dest_area->x1);
        ...
        /*static int cnt = 0;
        char file[20];
        sprintf(file, "/data/%d.rgb", cnt);
        int fd = open(file, O_RDWR | O_CREAT);
        write(fd, dest_buf, 800 * 480 * 4);
        close(fd);
        cnt++;*/
    }
}
