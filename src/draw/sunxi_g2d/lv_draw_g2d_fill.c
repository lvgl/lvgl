...
static void _g2d_fill(uint8_t * dest_buf, const lv_area_t * dest_area, lv_coord_t dest_stride,
                      ...
                      lv_area_t * disp_area, lv_color_format_t dest_cf, const lv_draw_fill_dsc_t * dsc)
{
    ...
    hal_dcache_clean_invalidate((unsigned long)(dest_buf + dest_stride * dest_area->y1 + px_size * dest_area->x1),
                                dest_stride * dest_h);
    g2d_fillrect_h info;
    ...
    info.dst_image_h.color = (dsc->opa << 24) | lv_color_to_u32(dsc->color);
