...
static void blend_texture_layer(lv_draw_task_t * t)
{
    ...
    lv_opengles_render_texture_dualflip(src_texture, &area, draw_dsc->opa, targ_tex_w, targ_tex_h, &t->clip_area, h_flip,
                                        v_flip);
...
static void lv_draw_opengles_3d(lv_draw_task_t * t, const lv_draw_3d_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    lv_opengles_render_texture_dualflip(dsc->tex_id, coords, dsc->opa, targ_tex_w, targ_tex_h, &clip_area, dsc->h_flip,
                                        dsc->v_flip);
