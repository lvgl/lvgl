...
void lv_draw_vg_lite_fill(lv_draw_unit_t * draw_unit, const lv_draw_fill_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    bool has_gradient = dsc->grad.dir != LV_GRAD_DIR_NONE;

    if(0 && dsc->opa >= LV_OPA_MAX && dsc->radius == 0 && !has_gradient) {
        vg_lite_color_t color = lv_vg_lite_color(dsc->color, dsc->opa, false);
        vg_lite_rectangle_t rect;
        lv_vg_lite_rect(&rect, &clip_area);

        LV_VG_LITE_ASSERT_BUFFER(&u->target_buffer);
        LV_VG_LITE_CHECK_ERROR(vg_lite_clear(&u->target_buffer, &rect, color));
    }
    else {
