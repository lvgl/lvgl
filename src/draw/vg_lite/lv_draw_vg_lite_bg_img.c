...
void lv_draw_vg_lite_bg_img(lv_draw_unit_t * draw_unit, const lv_draw_bg_image_dsc_t * dsc,
                            const lv_area_t * coords)
{
    ...
    if(src_type == LV_IMAGE_SRC_SYMBOL) {
        ...
        lv_draw_vg_lite_label(draw_unit, &label_draw_dsc, &a);
    }
    else {
        ...
        if(dsc->tiled == false) {
            ...
            lv_draw_vg_lite_img(draw_unit, &img_dsc, &area);
        }
        else {
            lv_area_t area;
            area.y1 = coords->y1;
