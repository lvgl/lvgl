...
void lv_draw_sw_arc(lv_draw_task_t * t, const lv_draw_arc_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    if(dsc->img_src == NULL) {
        ...
        else if(!lv_draw_buf_ensure_resident((lv_draw_buf_t *)decoder_dsc.decoded, t->draw_unit)) {
            /*The decoder is closed together with the other resources at the end*/
            LV_LOG_WARN("Can't ensure arc image residency, falling back to solid color");
            blend_dsc.color = dsc->color;
        }
        else {
            img_area.x1 = 0;
