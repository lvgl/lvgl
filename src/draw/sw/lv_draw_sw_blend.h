...
typedef struct {
    const lv_area_t * fill_area;    /**< the area with absolute coordinates to fill on `dest_buf`*/
    const lv_color_t * src_buf;     /**<  pointer to the destination buffer*/
    const lv_area_t * src_area;     /**< coordinates of the `src_buf` relative to the `dest_buf`*/
