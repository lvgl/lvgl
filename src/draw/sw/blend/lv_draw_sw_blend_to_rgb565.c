        ...
        if(mask_buf == NULL && opa >= LV_OPA_MAX) {
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_RGB565(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; dest_x < w; dest_x++, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        dest_buf_u16[dest_x] = l8_to_rgb565(pixel ? 0xFF : 0x00);
                    }
                    dest_buf_u16 = drawbuf_next_row(dest_buf_u16, dest_stride);
                    src_buf_i1 += src_stride;
                }
            }
        }
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_RGB565_WITH_OPA(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; dest_x < w; dest_x++, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        dest_buf_u16[dest_x] = lv_color_8_16_mix(pixel ? 0xFF : 0x00, dest_buf_u16[dest_x], opa);
                    }
                    dest_buf_u16 = drawbuf_next_row(dest_buf_u16, dest_stride);
                    src_buf_i1 += src_stride;
                }
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_RGB565_WITH_MASK(dsc)) {
                ...
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; dest_x < w; dest_x++, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        dest_buf_u16[dest_x] = lv_color_8_16_mix(pixel ? 0xFF : 0x00, dest_buf_u16[dest_x], mask_buf[dest_x]);
                    }
                    dest_buf_u16 = drawbuf_next_row(dest_buf_u16, dest_stride);
                    src_buf_i1 += src_stride;
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_RGB565_MIX_MASK_OPA(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; dest_x < w; dest_x++, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        dest_buf_u16[dest_x] = lv_color_8_16_mix(pixel ? 0xFF : 0x00, dest_buf_u16[dest_x], LV_OPA_MIX2(mask_buf[dest_x], opa));
                    }
                    dest_buf_u16 = drawbuf_next_row(dest_buf_u16, dest_stride);
                    src_buf_i1 += src_stride;
                    mask_buf += mask_stride;
            ...
            for(dest_x = 0, src_x = 0; dest_x < w; dest_x++, src_x += 4) {
                ...
                switch(dsc->blend_mode) {
                    case LV_BLEND_MODE_ADDITIVE:
                        ...
                        res = (LV_MIN(dest_buf_u16[dest_x] + l8_to_rgb565(pixel ? 0xFF : 0x00), 0xFFFF));
                        break;
                    case LV_BLEND_MODE_SUBTRACTIVE:
                        // Subtractive blending mode
                        res = (LV_MAX(dest_buf_u16[dest_x] - l8_to_rgb565(pixel ? 0xFF : 0x00), 0));
                        break;
                    case LV_BLEND_MODE_MULTIPLY:
                        // Multiply blending mode
                        res = ((((dest_buf_u16[dest_x] >> 11) * (l8_to_rgb565(pixel ? 0xFF : 0x00) >> 3)) & 0x1F) << 11) |
                              ((((dest_buf_u16[dest_x] >> 5) & 0x3F) * ((l8_to_rgb565(pixel ? 0xFF : 0x00) >> 2) & 0x3F) >> 6) << 5) |
                              (((dest_buf_u16[dest_x] & 0x1F) * (l8_to_rgb565(pixel ? 0xFF : 0x00) & 0x1F)) >> 5);
                        break;
                    default:
