...
static void LV_ATTRIBUTE_FAST_MEM i1_image_blend(_lv_draw_sw_blend_image_dsc_t * dsc)
{
    ...
    if(dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
        if(mask_buf == NULL && opa >= LV_OPA_MAX) {
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_ARGB8888(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x++, src_x++) {
                        ...
                        dest_buf_c32[dest_x].alpha = pixel ? 0xFF : 0x00;
                        dest_buf_c32[dest_x].red = pixel ? 0xFF : 0x00;
                        dest_buf_c32[dest_x].green = pixel ? 0xFF : 0x00;
                        dest_buf_c32[dest_x].blue = pixel ? 0xFF : 0x00;
                    }
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_ARGB8888_WITH_OPA(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x++, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        lv_color_8_32_mix(pixel ? 0xFF : 0x00, &dest_buf_c32[dest_x], opa);
                    }
                    dest_buf_c32 = drawbuf_next_row(dest_buf_c32, dest_stride);
                    src_buf_i1 = drawbuf_next_row(src_buf_i1, src_stride);
                }
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_ARGB8888_WITH_MASK(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x++, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        lv_color_8_32_mix(pixel ? 0xFF : 0x00, &dest_buf_c32[dest_x], mask_buf[src_x]);
                    }
                    dest_buf_c32 = drawbuf_next_row(dest_buf_c32, dest_stride);
                    src_buf_i1 = drawbuf_next_row(src_buf_i1, src_stride);
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_ARGB8888_MIX_MASK_OPA(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x++, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        lv_color_8_32_mix(pixel ? 0xFF : 0x00, &dest_buf_c32[dest_x], LV_OPA_MIX2(mask_buf[src_x], opa));
                    }
                    dest_buf_c32 = drawbuf_next_row(dest_buf_c32, dest_stride);
                    src_buf_i1 = drawbuf_next_row(src_buf_i1, src_stride);
                    mask_buf += mask_stride;
        ...
        for(y = 0; y < h; y++) {
            for(dest_x = 0, src_x = 0; src_x < w; dest_x++, src_x++) {
                uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                src_argb.red = pixel ? 0xFF : 0x00;
                src_argb.green = pixel ? 0xFF : 0x00;
                src_argb.blue = pixel ? 0xFF : 0x00;
