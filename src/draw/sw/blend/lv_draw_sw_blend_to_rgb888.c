        ...
        if(mask_buf == NULL && opa >= LV_OPA_MAX) {
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_888(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++) {
                        ...
                        dest_buf_u8[dest_x + 2] = pixel ? 0xFF : 0x00;
                        dest_buf_u8[dest_x + 1] = pixel ? 0xFF : 0x00;
                        dest_buf_u8[dest_x + 0] = pixel ? 0xFF : 0x00;
                    }
                    dest_buf_u8 += dest_stride;
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_888_WITH_OPA(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        lv_color_8_24_mix(pixel ? 0xFF : 0x00, &dest_buf_u8[dest_x], opa);
                    }
                    dest_buf_u8 += dest_stride;
                    src_buf_i1 += src_stride;
                }
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_888_WITH_MASK(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        lv_color_8_24_mix(pixel ? 0xFF : 0x00, &dest_buf_u8[dest_x], mask_buf[src_x]);
                    }
                    dest_buf_u8 += dest_stride;
                    src_buf_i1 += src_stride;
            ...
            if(LV_RESULT_INVALID == LV_DRAW_SW_I1_BLEND_NORMAL_TO_888_MIX_MASK_OPA(dsc)) {
                for(y = 0; y < h; y++) {
                    for(dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++) {
                        uint8_t pixel = src_buf_i1[src_x / 8] & (1 << (7 - (src_x % 8)));
                        lv_color_8_24_mix(pixel ? 0xFF : 0x00, &dest_buf_u8[dest_x], LV_OPA_MIX2(opa, mask_buf[src_x]));
                    }
                    dest_buf_u8 += dest_stride;
                    src_buf_i1 += src_stride;
                    mask_buf += mask_stride;
