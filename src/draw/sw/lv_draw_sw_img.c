...
static void colorkey_only(lv_draw_task_t * t, const lv_draw_image_dsc_t * draw_dsc,
                          const lv_image_decoder_dsc_t * decoder_dsc,
                          const lv_area_t * img_coords, const lv_area_t * clipped_img_area);
...
#endif /*LV_DRAW_SW_COMPLEX*/
    else if(!transformed && !radius && draw_dsc->recolor_opa <= LV_OPA_MIN && draw_dsc->colorkey) {
        colorkey_only(t, draw_dsc, decoder_dsc, img_coords, clipped_img_area);
    }
                         ...
                         const lv_area_t * img_coords, const lv_area_t * clipped_img_area)
{
    ...
    while(blend_area.y1 <= y_last) {
        ...
        if(has_colorkey) {
            colorkey_and_recolor(relative_area, decoded->data, tmp_buf, img_stride, blend_dsc.src_color_format, draw_dsc);
        }
        else {
                                 ...
                                 lv_color_format_t cf, const lv_draw_image_dsc_t * draw_dsc)
{
    ...
    if(cf == LV_COLOR_FORMAT_RGB565A8 || cf == LV_COLOR_FORMAT_RGB565) {
        ...
        for(y = 0; y < h; y++) {
            int32_t x;
            for(x = 0; x < w; x++) {
                uint16_t normal = lv_color_swap_16(buf16_src[x]);

                lv_color16_t tmp;
                lv_memcpy(&tmp, &normal, sizeof(lv_color16_t));
                lv_color_t src_color = lv_color16_to_color(tmp);
                ...
                /* Apply recolor */
                    ...
                    *buf16_dest = lv_color_swap_16(recolor16);
                }
                else if(mix > LV_OPA_MIN) {
                    uint16_t src16 = normal;
                    uint16_t result = (((c_mult[2] + ((src16 >> 11) & 0x1F) * mix_inv) << 3) & 0xF800) +
                                      (((c_mult[1] + ((src16 >> 5) & 0x3F) * mix_inv) >> 3) & 0x07E0) +
                                      ((c_mult[0] + (src16 & 0x1F) * mix_inv) >> 8);
                    *buf16_dest = lv_color_swap_16(result);
                }
                else {
                    *buf16_dest = buf16_src[x]; // No change
                }
