...
# add_library(lvgl_draw_renesas STATIC ${SOURCE_FILES})
#
# # Add the current directory as an include directory
# target_include_directories(lvgl_draw_renesas PUBLIC ${CMAKE_CURRENT_SOURCE_DIR})"""
#     )
    ...
    if os.path.isdir(dir):
        ...
        with open(f"{dir}/CMakeLists.txt", "w") as f:
            ...
            f.write(
                f"""set(SOURCE_FILES {"\n".join(c_files)})
# Create the main static library for this directory
add_library(lvgl_draw_renesas_{dir} STATIC ${{SOURCE_FILES}})

# Add the current directory as an include directory
target_include_directories(lvgl_draw_renesas_{dir} PUBLIC ${{CMAKE_CURRENT_SOURCE_DIR}})"""
            )
...
foreach(subdir ${SUBDIRS})
   ...
   target_link_libraries(lvgl_draw_renesas PRIVATE ${subdir})
endforeach()
"""
    )
