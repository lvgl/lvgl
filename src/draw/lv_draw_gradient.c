...
static lv_gradient_cache_t * allocate_item(lv_gradient_t * g, lv_coord_t w, lv_coord_t h);
...
static  uint32_t compute_key(lv_gradient_t * g, lv_coord_t w, lv_coord_t h);
...
uint32_t compute_key(lv_gradient_t * g, lv_coord_t size, lv_coord_t w)
{
    union void_cast v; v.ptr = g;
    return (v.value ^ size ^ (w >> 1)); /*Yes, this is correct, it's like a hash that changes if the width changes*/
}


size_t get_cache_item_size(lv_gradient_cache_t * c)
{
...
lv_gradient_cache_t * next_in_cache(lv_gradient_cache_t * first) {
    if(first == NULL)
...
lv_res_t iterate_cache(op_cache_t func, void * ctx, lv_gradient_cache_t ** out)
{
...
lv_res_t find_oldest_item_life(lv_gradient_cache_t * c, void * ctx)
{
...
void free_item(lv_gradient_cache_t * c) {
...
lv_res_t kill_oldest_item(lv_gradient_cache_t * c, void * ctx)
{
    uint32_t * min_life = (uint32_t*)ctx;
    if (c->life == *min_life) {
        /*Found, let's kill it*/
...
lv_res_t find_item(lv_gradient_cache_t * c, void * ctx)
{
...
lv_gradient_cache_t * allocate_item(lv_gradient_t * g, lv_coord_t w, lv_coord_t h)
{
    ...
    item->hmap_size = size;
#if LV_DITHER_ERROR_DIFFUSION
    item->error_acc = (lv_scolor24_t*)(grad_cache_end + sizeof(*item) + size * sizeof(lv_grad_color_t) + w * sizeof(lv_color_t));
#endif
#endif
...
lv_gradient_cache_t * lv_grad_get_from_cache(lv_gradient_t * g, lv_coord_t w, lv_coord_t h)
{
...
#if DITHER_GRADIENT
    for(size_t i = 0; i < item->hmap_size; i++) {
...
void lv_grad_evict_from_cache(lv_gradient_t * g, lv_coord_t w, lv_coord_t h)
{
    lv_coord_t size = g->dir == LV_GRAD_DIR_HOR ? w : h;
    uint32_t key = compute_key(g, size, w);
    lv_gradient_cache_t * item = NULL;
...
LV_ATTRIBUTE_FAST_MEM lv_grad_color_t lv_grad_get(const lv_gradient_t * dsc, lv_coord_t range, lv_coord_t frac)
{
    ...
    lv_grad_color_t r = GRAD_CM(lv_udiv_255(two.ch.red * mix   + one.ch.red * imix),
                                lv_udiv_255(two.ch.green * mix + one.ch.green * imix),
                                lv_udiv_255(two.ch.blue * mix  + one.ch.blue * imix));
    return r;
}
