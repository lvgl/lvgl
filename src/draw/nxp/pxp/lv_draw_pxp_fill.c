                      ...
                      const lv_area_t * coords)
{
    ...
    _pxp_fill(dest_buf, &blend_area, dest_stride, layer->color_format, dsc);
}
                      ...
                      lv_color_format_t dest_cf, const lv_draw_fill_dsc_t * dsc)
{
    ...
    pxp_output_buffer_config_t outputConfig = {
        ...
        .buffer0Addr = (uint32_t)(dest_buf + px_size * (dest_stride * dest_area->y1 + dest_area->x1)),
        .buffer1Addr = (uint32_t)NULL,
        .pitchBytes = dest_stride * px_size,
        .width = dest_w,
        .height = dest_h
    };
