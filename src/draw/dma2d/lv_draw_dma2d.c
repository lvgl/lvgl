...
typedef enum {
    LV_DMA2D_OUTPUT_CF_ARGB8888 = 0,
    LV_DMA2D_OUTPUT_CF_RGB888,
    LV_DMA2D_OUTPUT_CF_RGB565,
    LV_DMA2D_OUTPUT_CF_ARGB1555,
    LV_DMA2D_OUTPUT_CF_ARGB4444
} lv_dma2d_output_cf_t;

typedef enum {
    LV_DMA2D_FGBG_CF_ARGB8888 = 0,
    LV_DMA2D_FGBG_CF_RGB888,
    LV_DMA2D_FGBG_CF_RGB565,
    LV_DMA2D_FGBG_CF_ARGB1555,
    LV_DMA2D_FGBG_CF_ARGB4444,
    LV_DMA2D_FGBG_CF_L8,
    LV_DMA2D_FGBG_CF_AL44,
    LV_DMA2D_FGBG_CF_AL88,
    LV_DMA2D_FGBG_CF_L4,
    LV_DMA2D_FGBG_CF_A8,
    LV_DMA2D_FGBG_CF_A4,
    LV_DMA2D_FGBG_CF_YCBCR
} lv_dma2d_fgbg_cf_t;

typedef enum {
    LV_DMA2D_MODE_MEMORY_TO_MEMORY = 0,
    LV_DMA2D_MODE_MEMORY_TO_MEMORY_WITH_PFC,
    LV_DMA2D_MODE_MEMORY_TO_MEMORY_WITH_BLENDING,
    LV_DMA2D_MODE_REGISTER_TO_MEMORY,
    LV_DMA2D_MODE_MEMORY_TO_MEMORY_WITH_BLENDING_AND_FIXED_COLOR_FG,
    LV_DMA2D_MODE_MEMORY_TO_MEMORY_WITH_BLENDING_AND_FIXED_COLOR_BG
} lv_dma2d_mode_t;

typedef struct {
    lv_dma2d_mode_t mode;
    uint32_t w;
    uint32_t h;

    void * ouput_address;
    uint32_t output_offset;
    lv_dma2d_output_cf_t output_cf;
    ...
    void * bg_address;
    uint32_t bg_offset;
    lv_dma2d_fgbg_cf_t bg_cf;

} lv_dma2d_configuration_t;
...
static void opaque_fill(void * first_pixel, int32_t w, int32_t h, int32_t stride, lv_color_t color, lv_color_format_t cf, lv_draw_task_t * task);
static void fill(void * first_pixel, int32_t w, int32_t h, int32_t stride, lv_color_t color, lv_color_format_t cf, lv_opa_t opa, lv_draw_task_t * task);
static void prepare_post_transfer_data(lv_draw_task_t * task, const lv_dma2d_cache_area_t * affected_cache_area);
static void configure_and_start_transfer(const lv_dma2d_configuration_t * conf);
static void invalidate_cache(const lv_dma2d_cache_area_t * mem_area);
static void clean_cache(const lv_dma2d_cache_area_t * mem_area);
...
static struct {
    volatile int * task_state_p;
    lv_dma2d_cache_area_t affected_memory;
} g_transferring;
...
{
    invalidate_cache(&g_transferring.affected_memory);
    *g_transferring.task_state_p = LV_DRAW_TASK_STATE_READY;
...
static int32_t dispatch_cb(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
    ...
    if(t->type == LV_DRAW_TASK_TYPE_FILL) {
        ...
        if(dsc->radius == 0 && dsc->grad.dir == LV_GRAD_DIR_NONE && (dsc->base.layer->color_format == LV_COLOR_FORMAT_ARGB8888
...
static void opaque_fill(void * first_pixel, int32_t w, int32_t h, int32_t stride, lv_color_t color, lv_color_format_t cf, lv_draw_task_t * task)
{
    ...
    switch(cf) {
        ...
        case LV_COLOR_FORMAT_XRGB8888:
            output_cf = LV_DMA2D_OUTPUT_CF_ARGB8888;
            cf_size = 4;
            color_u32 = lv_color_to_u32(color);
            break;
        case LV_COLOR_FORMAT_RGB888:
            output_cf = LV_DMA2D_OUTPUT_CF_RGB888;
            cf_size = 3;
            color_u32 = lv_color_to_u32(color);
            break;
        case LV_COLOR_FORMAT_RGB565:
            output_cf = LV_DMA2D_OUTPUT_CF_RGB565;
            cf_size = 2;
            color_u32 = lv_color_to_u16(color);
            break;
        default:
            LV_LOG_WARN("unsupported output color format");
            return;
    }

    lv_dma2d_cache_area_t cache_area = {
        .first_byte = first_pixel,
        .width_bytes = w * cf_size,
        .height = h,
        .stride = stride
    };
    prepare_post_transfer_data(task, &cache_area);

    lv_dma2d_configuration_t conf = {
        .mode = LV_DMA2D_MODE_REGISTER_TO_MEMORY,
        .w = w,
        .h = h,
...
static void fill(void * first_pixel, int32_t w, int32_t h, int32_t stride, lv_color_t color, lv_color_format_t cf, lv_opa_t opa, lv_draw_task_t * task)
{
    ...
    lv_dma2d_configuration_t conf = {
        .mode = LV_DMA2D_MODE_MEMORY_TO_MEMORY_WITH_BLENDING_AND_FIXED_COLOR_FG,
        .w = w,
        .h = h,
        ...
        .bg_cf = (lv_dma2d_fgbg_cf_t) output_cf
    };
    configure_and_start_transfer(&conf);
}
...
static void prepare_post_transfer_data(lv_draw_task_t * task, const lv_dma2d_cache_area_t * affected_cache_area)
{
    g_transferring.task_state_p = &task->state;
    lv_memcpy(&g_transferring.affected_memory, affected_cache_area, sizeof(lv_dma2d_cache_area_t));
}
...
static void configure_and_start_transfer(const lv_dma2d_configuration_t * conf)
{
    ...
    /* Fill color. Only for mode LV_DMA2D_MODE_REGISTER_TO_MEMORY */
    ...
    DMA2D->CR = DMA2D_CR_START | (((uint32_t) conf->mode) << DMA2D_CR_MODE_Pos) | DMA2D_CR_TCIE;
}
