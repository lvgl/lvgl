...
void lv_draw_dma2d_invalidate_cache(const lv_draw_dma2d_cache_area_t * mem_area)
{
    if(SCB->CCR & SCB_CCR_DC_Msk) {
        SCB_InvalidateDCache_by_Addr((void *)mem_area->first_byte, mem_area->height * mem_area->stride);
    }
}
...
{
    if(SCB->CCR & SCB_CCR_DC_Msk) {
        SCB_CleanDCache_by_Addr((void *)mem_area->first_byte, mem_area->height * mem_area->stride);
    }
}
#endif
