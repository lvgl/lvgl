...
{
    char * json = (char *)lv_malloc(len + 1);
...
static lv_res_t decoder_open(lv_img_dec_dsc_t * dsc, const lv_img_dec_flags_t flags, void * user_data)
{
    ...
    if(dsc->dec_ctx->user_data != NULL) {
        ...
        if(dec_ctx == NULL || dsc->input.size_hint.x != dsc->header.w || dsc->input.size_hint.y != dsc->header.h) {
            lv_free(context->allocated_buf);
            lv_free(context);
        }
    ...
    if(context->lines_in_buf != 0) {  /*Too big picture to fit the maximum buffer size (default to 1024px) ?*/
        context->allocated_buf = lv_malloc(context->lines_in_buf * context->scanline_width);
