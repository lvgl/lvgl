...
static int stbtt_InitFont_internal(stbtt_fontinfo * info, lv_fs_file_t * stream, int fontstart)
{
    stbtt_uint32 cmap, t;
...
STBTT_DEF void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo * info, stbtt_render_callback callback,
...
STBTT_DEF int stbtt_InitFont(stbtt_fontinfo * info, lv_fs_file_t * stream, int offset)
{
...
#endif
typedef struct lv_ttf_font_info {
    int x1, y1, x2, y2;
    int ascent, descent;
    float scale;
} lv_ttf_font_info_t;
...
{
    if(out_ttf == NULL || file == NULL) {
        return LV_TTF_RES_ARG;
    }
    if(1 != stbtt_InitFont(out_ttf, file, 0)) {
        return LV_TTF_RES_FMT;
    }
    return LV_TTF_RES_OK;
}
                                 ...
                                 uint32_t unicode_letter_next)
{
    ...
    // float ofs_y =y1+dsc_out->box_h-(fi->ascent*fi->scale);
...
static const uint8_t * ttf_get_glyph_bitmap_cb(const lv_font_t * font, uint32_t unicode_letter)
{
    ...
    lv_ttf_render_state_t rs;
    rs.bmp = buffer;
    rs.width = w;
    stbtt_MakeGlyphBitmapSubpixel(info, ttf_render_cb, &rs, w, h, 0, fi->scale, fi->scale, 0, 0, g1);
                                ...
                                const lv_font_t * fallback)
{
    ...
    stbtt_GetFontBoundingBox(ttf, &fi->x1, &fi->y1, &fi->x2, &fi->y2);
    stbtt_GetFontVMetrics(ttf, &fi->ascent, &fi->descent, &line_gap);
    // out_font->base_line = (lv_coord_t)(fi->ascent*scale);
    out_font->base_line = line_height - (lv_coord_t)(fi->ascent * scale);
    out_font->underline_position = (uint8_t)line_height - fi->descent;
    out_font->underline_thickness = 0;
    out_font->subpx = 0;
    fi->scale = scale;
    out_font->user_data = fi;
