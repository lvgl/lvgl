    ...
    if(pPage->ucPaletteType == GIF_PALETTE_1BPP || pPage->ucPaletteType == GIF_PALETTE_1BPP_OLED) {  // 1-bit mono
        ...
        if(pPage->ucPaletteType == GIF_PALETTE_1BPP) {  // horizontal pixels
            ...
            if(pDraw->ucHasTransparency) {  // if transparency used
                ...
                uc = *d;
                ucMask = (0x80 >> (pDraw->iX & 7)); // left pixel is MSB
                while(s < pEnd) {
...
init_codetable:
    ...
    if(pImage->ucDrawType == GIF_DRAW_COOKED && pImage->pfnDraw &&
       pImage->pFrameBuffer) {  // convert each line through the palette
        GIFDRAW gd;
        gd.iX = pImage->iX;
        gd.iY = pImage->iY;
...
static int DecodeLZW(GIFIMAGE * pImage, int iOptions)
{
    ...
    if(pImage->ucDrawType == GIF_DRAW_COOKED && pImage->pFrameBuffer == NULL &&
       pImage->pfnDraw == NULL) {  // without a framebuffer and a GIFDRAW callback, we cannot continue
