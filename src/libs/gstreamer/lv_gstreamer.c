/**
 * @file lv_gstreamer.c
 *
 */

/*********************
 *      INCLUDES
 *********************/

#include "glib.h"
#include "lv_gstreamer_internal.h"

#if LV_USE_GSTREAMER

/*********************
 *      DEFINES
 *********************/

#define MY_CLASS (&lv_gstreamer_class)

#include <gst/gstelementfactory.h>
#include "../../core/lv_obj_class_private.h"

/**********************
 *      TYPEDEFS
 **********************/

/**********************
 *  STATIC PROTOTYPES
 **********************/

static void lv_gstreamer_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj);
static void lv_gstreamer_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj);
static void gstreamer_create_pipeline(lv_gstreamer_t *gstreamer);
static void on_pad_added(GstElement *element, GstPad *pad, gpointer user_data);
static GstFlowReturn on_new_sample(GstElement *sink, gpointer user_data);
static void gstreamer_timer_cb(lv_timer_t *timer);
static void gstreamer_poll_bus(lv_gstreamer_t *streamer);
static void gstreamer_update_frame(lv_gstreamer_t *streamer);

/**********************
 *  STATIC VARIABLES
 **********************/

const lv_obj_class_t lv_gstreamer_class = {
	.constructor_cb = lv_gstreamer_constructor,
	.destructor_cb = lv_gstreamer_destructor,
	.width_def = LV_SIZE_CONTENT,
	.height_def = LV_SIZE_CONTENT,
	.instance_size = sizeof(lv_gstreamer_t),
	.base_class = &lv_image_class,
	.name = "gstreamer",
};

/**********************
 *      MACROS
 **********************/

/**********************
 *   GLOBAL FUNCTIONS
 **********************/

lv_obj_t *lv_gstreamer_create(lv_obj_t *parent)
{
	if (!gst_is_initialized()) {
		gst_init(0, NULL);
	}

	LV_TRACE_OBJ_CREATE("begin");

	lv_obj_t *obj = lv_obj_class_create_obj(MY_CLASS, parent);
	lv_obj_class_init_obj(obj);

	LV_TRACE_OBJ_CREATE("end");
	return obj;
}

void lv_gstreamer_set_src(lv_obj_t *obj, const char *factory_name, const char *property, const char *source)
{
	LV_ASSERT_NULL(obj);
	LV_ASSERT_NULL(factory_name);
	LV_ASSERT_OBJ(obj, MY_CLASS);

	if (!obj || !factory_name) {
		LV_LOG_WARN("Refusing to set source with invalid params. Obj: %p Factory Name: %s", obj, factory_name);
		return;
	}

	lv_gstreamer_t *streamer = (lv_gstreamer_t *)obj;

	if (streamer->source) {
		LV_LOG_WARN("LVGL doesn't allow modifying the GStreamer source");
		return;
	}

	streamer->source = gst_element_factory_make(factory_name, "lv_gstreamer_source");

	if (!streamer->source) {
		LV_LOG_WARN("Failed to create source from factory '%s'", factory_name);
		return;
	}

	if (property != NULL && source != NULL) {
		g_object_set(G_OBJECT(streamer->source), property, source, NULL);
	}

	gstreamer_create_pipeline(streamer);
}

void lv_gstreamer_play(lv_obj_t *obj)
{
	LV_ASSERT_OBJ(obj, MY_CLASS);
	if (!obj) {
		return;
	}
	lv_gstreamer_t *streamer = (lv_gstreamer_t *)obj;
	GstStateChangeReturn ret = gst_element_set_state(streamer->pipeline, GST_STATE_PLAYING);
	if (ret == GST_STATE_CHANGE_FAILURE) {
		LV_LOG_ERROR("Unable to play pipeline");
	}
}

void lv_gstreamer_pause(lv_obj_t *obj)
{
	LV_ASSERT_OBJ(obj, MY_CLASS);
	if (!obj) {
		return;
	}
	lv_gstreamer_t *streamer = (lv_gstreamer_t *)obj;
	GstStateChangeReturn ret = gst_element_set_state(streamer->pipeline, GST_STATE_PAUSED);

	if (ret == GST_STATE_CHANGE_FAILURE) {
		LV_LOG_ERROR("Unable to pause pipeline");
	}
}

void lv_gstreamer_stop(lv_obj_t *obj)
{
	LV_ASSERT_OBJ(obj, MY_CLASS);
	if (!obj) {
		return;
	}
	lv_gstreamer_t *streamer = (lv_gstreamer_t *)obj;
	GstStateChangeReturn ret = gst_element_set_state(streamer->pipeline, GST_STATE_READY);
	if (ret == GST_STATE_CHANGE_FAILURE) {
		LV_LOG_ERROR("Unable to stop pipeline");
	}
}

void lv_gstreamer_seek(lv_obj_t *gstreamer, uint32_t position)
{
	LV_UNUSED(gstreamer);
	LV_UNUSED(position);
}

uint32_t lv_gstreamer_get_duration(lv_obj_t *gstreamer)
{
	LV_UNUSED(gstreamer);
	return 0;
}

uint32_t lv_gstreamer_get_position(lv_obj_t *gstreamer)
{
	LV_UNUSED(gstreamer);
	return 0;
}

lv_gstreamer_state_t lv_gstreamer_get_state(lv_obj_t *gstreamer)
{
	LV_UNUSED(gstreamer);
	return LV_GSTREAMER_STATE_NULL;
}

/**********************
 *   STATIC FUNCTIONS
 **********************/

static void lv_gstreamer_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
	LV_UNUSED(class_p);
	LV_TRACE_OBJ_CREATE("begin");
	lv_gstreamer_t *streamer = (lv_gstreamer_t *)obj;

	streamer->pipeline = gst_pipeline_new("lv_gstreamer_pipeline");
	LV_ASSERT_NULL(streamer->pipeline);

	streamer->video_convert = gst_element_factory_make("videoconvert", "lv_gstreamer_video_convert");
	LV_ASSERT_NULL(streamer->video_convert);

	streamer->video_rate = gst_element_factory_make("videorate", "lv_gstreamer_video_rate");
	LV_ASSERT_NULL(streamer->video_rate);

	streamer->video_queue = gst_element_factory_make("queue", "lv_gstreamer_video_queue");
	LV_ASSERT_NULL(streamer->video_queue);

	streamer->video_app_sink = gst_element_factory_make("appsink", "lv_gstreamer_video_sink");
	LV_ASSERT_NULL(streamer->video_app_sink);

	streamer->audio_convert = gst_element_factory_make("audioconvert", "lv_gstreamer_audio_convert");
	LV_ASSERT_NULL(streamer->audio_convert);

	streamer->audio_resample = gst_element_factory_make("audioresample", "lv_gstreamer_audio_resample");
	LV_ASSERT_NULL(streamer->audio_resample);

	streamer->audio_sink = gst_element_factory_make("autoaudiosink", "lv_gstreamer_audio_sink");
	LV_ASSERT_NULL(streamer->audio_sink);

	streamer->gstreamer_timer = lv_timer_create(gstreamer_timer_cb, LV_DEF_REFR_PERIOD, streamer);
	LV_ASSERT_NULL(streamer->gstreamer_timer);
	streamer->frame_queue = g_async_queue_new();

	lv_memzero(&streamer->frame, sizeof(streamer->frame));
	LV_TRACE_OBJ_CREATE("finished");
}

static void gstreamer_poll_bus(lv_gstreamer_t *streamer)
{
	GstBus *bus = gst_element_get_bus(streamer->pipeline);
	GstMessage *msg;

	while ((msg = gst_bus_pop(bus)) != NULL) {
		const GstMessageType message_type = GST_MESSAGE_TYPE(msg);
		switch (message_type) {
		case GST_MESSAGE_ERROR: {
			GError *err;
			gchar *debug;
			gst_message_parse_error(msg, &err, &debug);
			LV_LOG_ERROR("GStreamer error: %s", err->message);
			g_error_free(err);
			g_free(debug);
			break;
		}
		case GST_MESSAGE_EOS:
			LV_LOG_INFO("End of stream");
			break;
		case GST_MESSAGE_STATE_CHANGED: {
			GstState old_state, new_state;
			gst_message_parse_state_changed(msg, &old_state, &new_state, NULL);
			LV_LOG_INFO("State changed: %s -> %s", gst_element_state_get_name(old_state),
				    gst_element_state_get_name(new_state));
			break;
		}
		default:
			LV_LOG_TRACE("Received message %d", message_type);
			break;
		}
		gst_message_unref(msg);
	}
	gst_object_unref(bus);
}

static void gstreamer_update_frame(lv_gstreamer_t *streamer)
{
	GstSample *sample = g_async_queue_try_pop(streamer->frame_queue);

	if (!sample) {
		return;
	}

	if (!streamer->is_video_info_valid) {
		GstCaps *caps = gst_sample_get_caps(sample);
		if (!caps || !gst_video_info_from_caps(&streamer->video_info, caps)) {
			LV_LOG_ERROR("Failed to get video info from caps");
			gst_sample_unref(sample);
			return;
		}
		streamer->is_video_info_valid = true;
	}

	GstBuffer *buffer = gst_sample_get_buffer(sample);
	GstMapInfo map;
	if (buffer && gst_buffer_map(buffer, &map, GST_MAP_READ)) {
		uint8_t *new_frame_data = lv_realloc((uint8_t *)streamer->frame.data, map.size);
		LV_ASSERT_MALLOC(new_frame_data);
		streamer->frame.data = new_frame_data;
		/* TODO: Avoid copying every frame. */
		lv_memcpy((uint8_t *)streamer->frame.data, map.data, map.size);
		streamer->frame = (lv_image_dsc_t){ .data = new_frame_data,
						    .data_size = map.size,
						    .header = {
							    .magic = LV_IMAGE_HEADER_MAGIC,
							    .cf = LV_COLOR_FORMAT_RGB888,
							    .h = GST_VIDEO_INFO_HEIGHT(&streamer->video_info),
							    .w = GST_VIDEO_INFO_WIDTH(&streamer->video_info),
							    .stride = GST_VIDEO_INFO_PLANE_STRIDE(&streamer->video_info, 0),
						    } };
		lv_image_set_src((lv_obj_t *)streamer, &streamer->frame);
	}
	gst_sample_unref(sample);
}
static void gstreamer_timer_cb(lv_timer_t *timer)
{
	lv_gstreamer_t *streamer = lv_timer_get_user_data(timer);

	gstreamer_poll_bus(streamer);
	gstreamer_update_frame(streamer);
}

static void lv_gstreamer_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
	LV_UNUSED(class_p);
	lv_gstreamer_t *streamer = (lv_gstreamer_t *)obj;
	gst_element_set_state(streamer->pipeline, GST_STATE_NULL);
	gst_object_unref(streamer->pipeline);
	lv_free((void *)streamer->frame.data);
}

static void gstreamer_create_pipeline(lv_gstreamer_t *streamer)
{
	LV_LOG_USER("Create pipeline");
	gst_bin_add_many(GST_BIN(streamer->pipeline), streamer->source, streamer->video_convert, streamer->video_rate,
			 streamer->video_queue, streamer->video_app_sink, streamer->audio_convert, streamer->audio_resample,
			 streamer->audio_sink, NULL);

	int target_fps = 1000 / LV_DEF_REFR_PERIOD;
	char caps[128];
	lv_snprintf(caps, sizeof(caps), "video/x-raw,format=RGB,framerate=%d/1", target_fps);

	GstCaps *appsink_caps = gst_caps_from_string(caps);
	g_object_set(G_OBJECT(streamer->video_app_sink), "emit-signals", TRUE, "sync", TRUE, "max-buffers", 1, "drop", TRUE,
		     "caps", appsink_caps, NULL);
	gst_caps_unref(appsink_caps);

	g_signal_connect(streamer->video_app_sink, "new-sample", G_CALLBACK(on_new_sample), streamer);

	/* At this point we don't yet know the input format
	 * Once the source starts receiving the data, it will create the necessary pads,
	 * i.e one pad for audio and one for video
	 * We add a callback so that we automatically connect to the data once it's figured out*/
	g_signal_connect(streamer->source, "pad-added", G_CALLBACK(on_pad_added), streamer);
}

static void on_pad_added(GstElement *element, GstPad *pad, gpointer user_data)
{
	LV_UNUSED(element);
	lv_gstreamer_t *streamer = (lv_gstreamer_t *)user_data;
	GstCaps *caps = gst_pad_get_current_caps(pad);
	GstStructure *structure = gst_caps_get_structure(caps, 0);
	const gchar *name = gst_structure_get_name(structure);

	LV_LOG_USER("Pad discovered %s", name);

	if (g_str_has_prefix(name, "video/")) {
		GstPad *video_convert_sink_pad = gst_element_get_static_pad(streamer->video_convert, "sink");
		if (!gst_pad_is_linked(video_convert_sink_pad)) {
			/* decodebin -> videoconvert -> videorate -> appsink */
			if (!gst_element_link_many(streamer->video_convert, streamer->video_rate, streamer->video_queue,
						   streamer->video_app_sink, NULL)) {
				LV_LOG_ERROR("Failed to link video convert to sink");
			}
			if (gst_pad_link(pad, video_convert_sink_pad) != GST_PAD_LINK_OK) {
				LV_LOG_ERROR("Failed to link decodebin to video convert");
			}
		} else {
			LV_LOG_WARN("Received another video pad but our video pipeline is already linked");
		}
		gst_object_unref(video_convert_sink_pad);
	}

	if (g_str_has_prefix(name, "audio/")) {
		GstPad *audio_convert_sink_pad = gst_element_get_static_pad(streamer->audio_convert, "sink");

		if (!gst_pad_is_linked(audio_convert_sink_pad)) {
			/* decodebin -> audioconvert -> audiosink */
			if (!gst_element_link_many(streamer->audio_convert, streamer->audio_resample, streamer->audio_sink,
						   NULL)) {
				LV_LOG_ERROR("Failed to link audio convert to sink");
			}
			if (gst_pad_link(pad, audio_convert_sink_pad) != GST_PAD_LINK_OK) {
				LV_LOG_ERROR("Failed to link decodebin to audio convert");
			}
		} else {
			LV_LOG_WARN("Received another audio pad but our audio pipeline is already linked");
		}
		gst_object_unref(audio_convert_sink_pad);
	}

	gst_caps_unref(caps);
}

static GstFlowReturn on_new_sample(GstElement *sink, gpointer user_data)
{
	/* This function is called from a thread other than the main one so we can't call anything related to LVGL here
	 * Instead, we acquire the new sample (the new frame) and push it so that we can retrieve it from an LVGL timer */
	lv_gstreamer_t *streamer = (lv_gstreamer_t *)user_data;
	GstSample *sample;

	g_signal_emit_by_name(sink, "pull-sample", &sample);
	if (!sample) {
		return GST_FLOW_OK;
	}

	if (streamer->frame_waiting) {
		gst_sample_unref(sample);
		return GST_FLOW_OK;
	}

	g_async_queue_push(streamer->frame_queue, sample);
	return GST_FLOW_OK;
}
#endif
