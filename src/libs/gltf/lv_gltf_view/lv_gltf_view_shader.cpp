...
static lv_result_t add_texture_defines_impl(lv_array_t *array, const fastgltf::TextureInfo &material_prop, const char *define,
					    const char *uv_define);

static lv_result_t add_texture_defines(lv_array_t *array, const fastgltf::Optional<fastgltf::TextureInfo> &material_prop,
				       const char *define, const char *uv_define);

static lv_result_t add_texture_defines(lv_array_t *array, const fastgltf::Optional<fastgltf::NormalTextureInfo> &material_prop,
				       const char *define, const char *uv_define);

static lv_result_t add_texture_defines(lv_array_t *array, const fastgltf::Optional<fastgltf::OcclusionTextureInfo> &material_prop,
				       const char *define, const char *uv_define);
							...
							fastgltf::Primitive *prim)
{
	...
	if (add_define_if_primitive_attribute_exists(result, asset, prim, "JOINTS_0", "HAS_JOINTS_0_VEC4") == LV_RESULT_INVALID) {
		return LV_RESULT_INVALID;
	}
	if (add_define_if_primitive_attribute_exists(result, asset, prim, "JOINTS_1", "HAS_JOINTS_1_VEC4") == LV_RESULT_INVALID) {
		return LV_RESULT_INVALID;
	}
...
lv_gltf_renwin_shaderset_t lv_gltf_view_shader_compile_program(lv_gltf_view_t *view, const lv_array_t *defines)
{
	uint32_t frag_shader_hash = lv_gl_shader_manager_select_shader(view->shader_manager, "__MAIN__.frag",
								       (lv_gl_shader_t *)defines->data, lv_array_size(defines));
