...
static GLuint lv_gltf_view_render_model(lv_gltf_view_t *viewer, lv_gltf_data_t *model, bool prepare_bg, uint32_t crop_left,
...
static void setup_primitive(int32_t prim_num, lv_gltf_view_t *viewer, lv_gltf_data_t *gltf_data, fastgltf::Node &node,
			    std::size_t mesh_index, const fastgltf::math::fmat4x4 &matrix,
			    const lv_gltf_view_env_textures_t *env_tex, bool is_transmission_pass);

static void draw_material(lv_gltf_view_t *viewer, lv_gltf_uniform_locations_t *uniforms, lv_gltf_data_t *model,
				...
				if (node.meshIndex) {
					...
					if (node.skinIndex) {
						...
						if (!lv_gltf_data_validated_skins_contains(model, skin_index)) {
							...
							if (skin.inverseBindMatrices) {
								...
								if (_ibmAccessor
									    .bufferViewIndex) { // To-do: test if this gets confused when bufferViewIndex == 0
									fastgltf::iterateAccessorWithIndex<
										fastgltf::math::fmat4x4>(
										*asset, _ibmAccessor,
										[&](fastgltf::math::fmat4x4 _matrix,
										    std::size_t idx) {
											auto &_jointNode =
												asset->nodes[skin.joints[idx]];
											viewer->ibm_by_skin_the_node
												[skin_index][&_jointNode] =
												_matrix;
										});
								}
							}
					...
					for (size_t mp = 0; mp < asset->meshes[mesh_index].primitives.size(); mp++) {
						...
						int64_t materialIndex =
							(!mappings.empty() && mappings[vstate->material_variant]) ?
								mappings[vstate->material_variant].value() + 1 :
								((_prim_gltf_data.materialIndex) ?
									 (_prim_gltf_data.materialIndex.value() + 1) :
									 0);
						if (materialIndex < 0) {
							...
							if (material.transmission)
								vstate->render_opaque_buffer = true;
							if (material.alphaMode == fastgltf::AlphaMode::Blend ||
							    (material.transmission != NULL)) {
								lv_gltf_data_add_blended_node_primitive(
									model, materialIndex + 1, &node, mp);
							} else {
								lv_gltf_data_add_opaque_node_primitive(model, materialIndex + 1,
												       &node, mp);
							}
			...
			*asset, sceneIndex, fastgltf::math::fmat4x4(),
			[&](fastgltf::Node &node, fastgltf::math::fmat4x4 matrix) {
				...
				for (uint64_t mp = 0; mp < asset->meshes[mesh_index].primitives.size(); mp++) {
					...
					int64_t material_index = (!mappings.empty() && mappings[vstate->material_variant]) ?
									 mappings[vstate->material_variant].value() + 1 :
									 ((prim_gltf_data.materialIndex) ?
										  (prim_gltf_data.materialIndex.value() + 1) :
										  0);
					...
					if (material_index > -1 && !shaderset->ready) {
						...
						lv_result_t result = lv_gltf_view_shader_injest_discover_defines(
							&defines, model, &node, &prim_gltf_data);
						LV_ASSERT_MSG(result == LV_RESULT_OK, "Couldn't injest shader defines");
						lv_gltf_renwin_shaderset_t shaderset = lv_gltf_view_shader_compile_program(
							viewer, (lv_gl_shader_t *)defines.data, lv_array_size(&defines));
						lv_gltf_uniform_locations_t uniform_locations =
							lv_gltf_uniform_locations_create(shaderset.program);
			    ...
			    const lv_gltf_view_env_textures_t *env_tex, bool is_transmission_pass)
{
	...
	/* Fast path, primitive setup in the last render */
	if ((model->last_material_index == materialIndex) && (model->last_pass_was_transmission == is_transmission_pass)) {
		GL_CALL(glUniformMatrix4fv(get_uniform_ids(model, materialIndex)->model_matrix, 1, GL_FALSE, &matrix[0][0]));
		return;
	}

	model->last_material_index = materialIndex;
	model->last_pass_was_transmission = is_transmission_pass;
	auto program = lv_gltf_data_get_shader_program(model, materialIndex);
	const auto &uniforms = get_uniform_ids(model, materialIndex);
