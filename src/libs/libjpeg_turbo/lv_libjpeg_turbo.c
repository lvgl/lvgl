...
static void convert_size_with_orientation(image_orientation_t image_op, uint32_t * width, uint32_t * height)
{
    if(image_op == IMAGE_CLOCKWISE_NONE || image_op == IMAGE_CLOCKWISE_0
       || image_op == IMAGE_CLOCKWISE_180 || image_op == IMAGE_FLIP_VER || image_op == IMAGE_FLIP_HOR) {
        return;
    }
...
static lv_draw_buf_t * decode_jpeg_file(const char * filename)
{
    ...
    image_orientation_t image_op = get_jpeg_direction(data, data_size);
    ...
    convert_size_with_orientation(image_op, &width, &height);
