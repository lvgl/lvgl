...
typedef struct ArduinoEspFFatFile {
    File file;
} ArduinoEspFFatFile;
...
extern "C" void lv_fs_arduino_esp_ffat_init(void)
{
    ...
    lv_fs_drv_t * fs_drv = &(LV_GLOBAL_DEFAULT()->arduino_esp_ffat_fs_drv);
...
static void * fs_open(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    File file = FFat.open(buf, flags);
    if(!file) {
        return NULL;
    }

    ArduinoEspFFatFile * lf = new ArduinoEspFFatFile{file};
