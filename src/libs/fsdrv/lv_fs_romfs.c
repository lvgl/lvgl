 ...
 *   GLOBAL FUNCTIONS
 **********************/
void lv_romfs_set_handler(rom_fs_t * rfs)
{
    lv_fs_drv_t * drv = lv_fs_get_drv(LV_FS_ROMFS_LETTER);
    drv->user_data = rfs;
}
...
int lv_fs_romfs_backend_init(const char * path, uint32_t offset)
{
    rom_fs_t * rfs = romfs_filebackend_init(path, offset);
    if(rfs == NULL) {
        return -1;
    }

    lv_romfs_set_handler(rfs);

    return 0;
}

void lv_fs_romfs_backend_exit(const char * path)
{
    lv_fs_drv_t * drv = lv_fs_get_drv(LV_FS_ROMFS_LETTER);
    rom_fs_t * rfs = (rom_fs_t *)drv->user_data;
    romfs_filebackend_exit(rfs);
}
