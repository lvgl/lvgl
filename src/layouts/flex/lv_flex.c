...
static void flex_update(lv_obj_t * cont, void * user_data)
{
    ...
    lv_obj_send_event(cont, LV_OBJ_EVENT_LAYOUT_CHANGED, NULL);
    ...
    while(item && item_first_id != item_last_id) {
        ...
        if(grow_size) {
            ...
            if(s != area_get_main_size(&item->coords)) {
                ...
                lv_obj_send_event(item, LV_OBJ_EVENT_SIZE_CHANGED, &old_coords);
                lv_obj_send_event(lv_obj_get_parent(item), LV_OBJ_EVENT_CHILD_CHANGED, item);
                lv_obj_invalidate(item);
            }
        }
        else {
