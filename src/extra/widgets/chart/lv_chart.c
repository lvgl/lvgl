...
static void lv_chart_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_PRESSED) {
        ...
        if(_lv_ll_is_empty(&chart->series_ll) == false) {
            if(chart->type == LV_CHART_TYPE_LINE) draw_series_line(obj, draw);
            else if(chart->type == LV_CHART_TYPE_BAR) draw_series_bar(obj, draw);
            else if(chart->type == LV_CHART_TYPE_SCATTER) draw_series_scatter(obj, draw);
        }

        draw_cursors(obj, draw);
    }
}
...
static void draw_div_lines(lv_obj_t * obj, lv_draw_ctx_t * draw)
{
    ...
    bool mask_ret = _lv_area_intersect(&series_clip_area, &obj->coords, draw->clip_area);
    if(mask_ret == false) return;

    const lv_area_t * clip_area_ori = draw->clip_area;
    draw->clip_area = &series_clip_area;
                ...
                if(crowded_mode) {
                    if(ser->y_points[p_prev] != LV_CHART_POINT_NONE && ser->y_points[p_act] != LV_CHART_POINT_NONE) {
                        ...
                        if(p1.x != p2.x) {
                            ...
                            lv_draw_line(draw, &line_dsc_default, &p1, &p2);
                    ...
                    if(point_w && point_h && ser->y_points[p_prev] != LV_CHART_POINT_NONE) {
                        lv_draw_rect(draw, &point_dsc_default, &point_area);
                    }
    ...
    for(i = 0; i < chart->point_cnt; i++) {
        ...
        _LV_LL_READ_BACK(&chart->series_ll, ser) {
            ...
            if(ser->y_points[p_act] != LV_CHART_POINT_NONE) {
                ...
                part_draw_dsc.value = ser->y_points[p_act];
                lv_event_send(obj, LV_EVENT_DRAW_PART_BEGIN, &part_draw_dsc);
                lv_draw_rect(draw, &col_dsc, &col_a);
                lv_event_send(obj, LV_EVENT_DRAW_PART_END, &part_draw_dsc);
            }
        }
    }
...
static void draw_cursors(lv_obj_t * obj, lv_draw_ctx_t * draw)
{
    ...
    lv_area_t clip_area;
    if(!_lv_area_intersect(&clip_area, draw->clip_area, &obj->coords)) return;
    ...
    _LV_LL_READ_BACK(&chart->cursor_ll, cursor) {
        ...
        if(cursor->dir & LV_DIR_HOR) {
            ...
            lv_event_send(obj, LV_EVENT_DRAW_PART_BEGIN, &part_draw_dsc);
            lv_draw_line(draw, &line_dsc_tmp, &p1, &p2);
            lv_draw_rect(draw, &point_dsc_tmp, &point_area);
            lv_event_send(obj, LV_EVENT_DRAW_PART_END, &part_draw_dsc);
        }
    ...
    for(i = 0; i <= total_tick_num; i++) {
        ...
        if(major && t->label_en)  {
            ...
            if(a.y2 >= obj->coords.y1 &&
               a.y1  <= obj->coords.y2) {
                lv_draw_label(draw, &label_dsc, &a, part_draw_dsc.text, NULL);
            }
        }
        else {
...
static void draw_x_ticks(lv_obj_t * obj, lv_draw_ctx_t * draw, lv_chart_axis_t axis)
{
    ...
    if(axis == LV_CHART_AXIS_PRIMARY_X) {
        if(y_ofs > draw->clip_area->y2) return;
        if(y_ofs + label_gap  + label_dsc.font->line_height + t->major_len < draw->clip_area->y1) return;
    }
...
static void draw_axes(lv_obj_t * obj, lv_draw_ctx_t * draw)
{
    ...
    draw_x_ticks(obj, draw, LV_CHART_AXIS_PRIMARY_X);
    draw_x_ticks(obj, draw, LV_CHART_AXIS_SECONDARY_X);
}
