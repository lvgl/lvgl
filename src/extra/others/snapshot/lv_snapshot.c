 ...
 */
lv_img_src_t * lv_snapshot_take(lv_obj_t * obj, lv_img_cf_t cf)
{
    uint32_t buff_size = lv_snapshot_buf_size_needed(obj, cf);

    lv_img_src_t * src = NULL;
    void * buf = NULL;
    ...
    buf = lv_mem_alloc(buff_size);
    if(buf == NULL) goto fail;

    dsc = lv_mem_alloc(sizeof(lv_img_dsc_t));
    if(dsc == NULL) goto fail;

    if(lv_snapshot_take_to_buf(obj, cf, dsc, buf, buff_size) == LV_RES_INV) goto fail;

    lv_img_src_set_raw(src, dsc);
    return src;
 ...
 *
 */
void lv_snapshot_free(lv_img_src_t * img_src)
{
    if(!img_src) return;

    if(img_src->type == LV_IMG_SRC_VARIABLE) {
        lv_img_dsc_t * data = (lv_img_dsc_t *)img_src->data;
        if(data->data) lv_mem_free((void *)data->data);
        lv_mem_free(data);
    }
