...
const void * lv_font_glyph_acquire_draw_data(lv_font_glyph_dsc_t * g_dsc, lv_draw_buf_t * draw_buf)
{
    ...
    return font_p->glyph_acquire_draw_data(g_dsc, draw_buf);
}
...
bool lv_font_glyph_get_glyph_info(const lv_font_t * font, lv_font_glyph_dsc_t * dsc_out, uint32_t letter,
                                  uint32_t letter_next)
{
    ...
    while(f) {
        bool found = f->glyph_get_info(f, dsc_out, letter, f->kerning == LV_FONT_KERNING_NONE ? 0 : letter_next);
        if(found) {
    ...
    if(placeholder_font != NULL) {
        placeholder_font->glyph_get_info(placeholder_font, dsc_out, letter,
...
uint16_t lv_font_get_glyph_width(const lv_font_t * font, uint32_t letter, uint32_t letter_next)
{
    ...
    lv_font_glyph_get_glyph_info(font, &g, letter, letter_next);
    return g.adv_w;
}
