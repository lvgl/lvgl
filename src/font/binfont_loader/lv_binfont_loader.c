...
lv_font_t * lv_binfont_create(const char * path)
{
    ...
    if(!lvgl_load_font(file, font)) {
        ...
        */
        ...
        keep_open = (dsc != NULL && dsc->loader != NULL);
        /*
         * When keep_open=true, the file handle ownership transfers to loader->fp
         * and will be closed in lv_binfont_destroy() via loader cleanup.
         */
#endif
        if(!keep_open) {
            lv_fs_close(file);
...
static void release_glyph_cb(const lv_font_t * font, lv_font_glyph_dsc_t * glyph_dsc)
{
    ...
    if(loader->raw_lock_held) {
        if(loader->raw_lock_gid == glyph_dsc->gid.index) {
            /* Expected case */
            ...
            /* Defensive: gid mismatch */
            LV_LOG_WARN("Glyph lock mismatch: expected gid %d, got %d",
                        (int)loader->raw_lock_gid, (int)glyph_dsc->gid.index);
            loader->raw_lock_held = false;
            loader->raw_lock_gid = 0;
            lv_mutex_unlock(&loader->lock);
        }
