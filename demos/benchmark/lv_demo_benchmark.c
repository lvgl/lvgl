...
static void moving_wallpaper_cb(void)
{
    ...
    shake_anim(img, - lv_disp_get_ver_res(NULL) / 3);
}
...
{
    lv_obj_set_flex_flow(lv_scr_act(), LV_FLEX_FLOW_ROW_WRAP);
    lv_obj_set_flex_align(lv_scr_act(), LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    lv_obj_set_style_pad_row(lv_scr_act(), 20, 0);
    ...
    for(y = 0; y < ver_cnt; y++) {
        ...
        for(x = 0; x < hor_cnt; x++) {
            ...
            lv_img_set_src(obj, &img_benchmark_cogwheel_rgb);
...
static void rotated_argb_image_cb(void)
{
    ...
    for(y = 0; y < ver_cnt; y++) {
        ...
        for(x = 0; x < hor_cnt; x++) {
            ...
            lv_img_set_angle(obj, lv_rand(100, 3500));
            shake_anim(obj, 80);
        }
    }
}
...
static void multiple_labels_cb(void)
{
    lv_obj_set_flex_flow(lv_scr_act(), LV_FLEX_FLOW_ROW_WRAP);
    lv_obj_set_flex_align(lv_scr_act(), LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    lv_obj_set_style_pad_row(lv_scr_act(), 80, 0);
...
static void multiple_arcs_cb(void)
{
    ...
    lv_obj_set_style_pad_column(lv_scr_act(), lv_dpx(40), 0);
    lv_obj_set_style_pad_row(lv_scr_act(), lv_dpx(40), 0);
...
void lv_demo_benchmark(void)
{
    ...
    lv_obj_set_style_pad_all(lv_scr_act(), 8, 0);
    lv_obj_set_style_pad_top(lv_scr_act(), 48, 0);
...
static void load_scene(uint32_t scene)
{
    ...
    lv_obj_set_flex_align(lv_scr_act(), LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);

    lv_anim_del(scr, scroll_anim_y_cb);
    lv_anim_del(scr, shake_anim_y_cb);
    lv_anim_del(scr, color_anim_cb);

    lv_anim_del(lv_layer_top(), color_anim_cb);
...
static void summary_create(void)
{
    ...
    lv_obj_set_style_pad_hor(lv_scr_act(), 0, 0);
...
static void scroll_anim(lv_obj_t * obj, int32_t y_max)
{
    uint32_t t = lv_anim_speed_to_time(lv_disp_get_dpi(NULL), 0, y_max);
...
static lv_obj_t * card_create(void)
{
    ...
    lv_obj_t * child = lv_img_create(panel);
    lv_obj_align(child, LV_ALIGN_LEFT_MID, 0, 0);
    lv_img_set_src(child, &img_transform_avatar_15);
