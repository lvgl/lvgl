...
static void profile_create(lv_obj_t * parent)
{
    ...
    lv_obj_add_event(user_name, ta_event_cb, LV_OBJ_EVENT_ALL, kb);
    ...
    lv_obj_add_event(password, ta_event_cb, LV_OBJ_EVENT_ALL, kb);
    ...
    lv_obj_add_event(birthdate, birthday_event_cb, LV_OBJ_EVENT_ALL, NULL);
    ...
    lv_obj_add_event(slider1, slider_event_cb, LV_OBJ_EVENT_ALL, NULL);
...
static void analytics_create(lv_obj_t * parent)
{
    ...
    lv_obj_add_event(chart1, chart_event_cb, LV_OBJ_EVENT_ALL, NULL);
    ...
    lv_obj_add_event(chart2, chart_event_cb, LV_OBJ_EVENT_ALL, NULL);
...
void shop_create(lv_obj_t * parent)
{
    ...
    lv_obj_add_event(chart3, shop_chart_event_cb, LV_OBJ_EVENT_ALL, NULL);
...
static void color_changer_create(lv_obj_t * parent)
{
    ...
    for(i = 0; palette[i] != _LV_PALETTE_LAST; i++) {
        ...
        lv_obj_add_event(c, color_event_cb, LV_OBJ_EVENT_ALL, &palette[i]);
    ...
    lv_obj_add_event(btn, color_changer_event_cb, LV_OBJ_EVENT_ALL, color_cont);
    ...
    if(code == LV_OBJ_EVENT_FOCUSED) {
        if(lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_POINTER) {
            if(calendar == NULL) {
                ...
                lv_obj_add_event(calendar, calendar_event_cb, LV_OBJ_EVENT_ALL, ta);
