/**
 * @file lv_demo_layout_previewer_flex_loader.c
 *
 */
    ...
    do {                                                                            \
        ...
        for (i = 0; i < ARRAY_SIZE(flex_align_map); i++) {                      \
            if (flex_align == flex_align_map[i]) {                                  \
                lv_roller_set_selected(ui->ctrl_pad.tab.align.roller_align_##item,  \
                                       i, LV_ANIM_ON);                              \
                break;                                                              \
            }                                                                       \
        }                                                                           \
...
#define FLEX_ALIGN_EVENT_ATTACH(item)                                               \
    do {                                                                            \
        lv_obj_add_event_cb(ui->ctrl_pad.tab.align.roller_align_##item,             \
