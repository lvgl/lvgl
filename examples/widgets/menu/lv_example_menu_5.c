...
void lv_example_menu_5(void)
{
    ...
    lv_obj_add_event_cb(menu, back_event_handler, LV_OBJ_EVENT_CLICKED, menu);
    lv_obj_set_size(menu, lv_disp_get_horizonal_resolution(NULL), lv_disp_get_vertical_resolution(NULL));
    lv_obj_center(menu);
    ...
    lv_obj_add_event_cb(lv_obj_get_child(cont, 2), switch_handler, LV_OBJ_EVENT_VALUE_CHANGED, menu);
    ...
    lv_obj_send_event(lv_obj_get_child(lv_obj_get_child(lv_menu_get_cur_sidebar_page(menu), 0), 0), LV_OBJ_EVENT_CLICKED,
                      NULL);
}
