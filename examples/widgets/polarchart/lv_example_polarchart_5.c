...
/**
 * Add a faded area effect to the line polarchart and make some division lines ticker
 */
void lv_example_polarchart_5(void)
{
    ...
    lv_obj_set_style_pad_all(polarchart, 0, 0);
    lv_obj_set_style_radius(polarchart, 0, 0);
    lv_obj_center(polarchart);

    lv_polarchart_set_div_line_count(polarchart, 5, 7);
    ...
    uint32_t i;
    for(i = 0; i < 10; i++) {
        lv_polarchart_set_next_value(polarchart, ser, (int32_t)lv_rand(10, 80));
    }
}
