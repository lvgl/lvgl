...
void lv_example_fragment_2(void)
{
    ...
    lv_obj_add_event_cb(root, sample_container_del, LV_OBJ_EVENT_DELETE, manager);

    int depth = 0;
    lv_fragment_t * fragment = lv_fragment_create(&sample_cls, &depth);
    lv_fragment_manager_push(manager, fragment, &container);
    lv_obj_add_event_cb(push_btn, sample_push_click, LV_OBJ_EVENT_CLICKED, manager);
    lv_obj_add_event_cb(pop_btn, sample_pop_click, LV_OBJ_EVENT_CLICKED, manager);
}
...
static lv_obj_t * sample_fragment_create_obj(lv_fragment_t * self, lv_obj_t * parent)
{
    ...
    lv_obj_add_event_cb(inc_btn, sample_fragment_inc_click, LV_OBJ_EVENT_CLICKED, fragment);
