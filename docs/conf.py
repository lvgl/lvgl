...
def get_version():
    ...
    path = os.path.join(base_path, '..', 'lvgl.h')
    with open(path, 'rb') as f:
        data = f.read().decode('utf-8')

    data = data.split('#define LVGL_VERSION_MAJOR', 1)[-1]
    major, data = data.split('\n', 1)
    data = data.split('#define LVGL_VERSION_MINOR', 1)[-1]
    minor, data = data.split('\n', 1)
    data = data.split('#define LVGL_VERSION_PATCH', 1)[-1]
    patch, data = data.split('\n', 1)

    ver = '{0}.{1}.{2}'.format(major.strip(), minor.strip(), patch.strip())

    if '#define LVGL_VERSION_INFO' in data:
        data = data.split('#define LVGL_VERSION_INFO', 1)[-1]
...
exclude_patterns = ['_build', 'doxygen_html', 'Thumbs.db', '.DS_Store',
                    'README.md', 'lv_examples', 'out_html', 'env', '_ext' ]
