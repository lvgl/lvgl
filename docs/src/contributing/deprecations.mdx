...
```c
/**
 * @deprecated Use `LV_CHECK_ARG(cond, action)` instead.
 *             `LV_ASSERT_ARG` aborts on failure; `LV_CHECK_ARG` logs a warning
 *             and executes the supplied action, which is safer in production.
 */
#define LV_ASSERT_ARG(cond)                                                                 \
  do {                                                                                      \
    LV_DEPRECATED_MACRO_WARN("LV_ASSERT_ARG is deprecated. Use LV_CHECK_ARG instead.");      \
    LV_ASSERT_MSG(cond, "Invalid argument");                                                 \
  } while(0)
```
