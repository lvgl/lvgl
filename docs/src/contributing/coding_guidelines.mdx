...
- The header appears in `ALLOWED_SYSTEM_HEADERS` in `scripts/static_checks/check_headers.py`.
...
To add a new allowed angle-bracket include, add it to `ALLOWED_SYSTEM_HEADERS` in
