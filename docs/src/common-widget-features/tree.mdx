---
title: Widget Tree
description: "LVGL stores Widgets in a parent-child structure (tree). The root element is the Screen, which has no parent."
---
...
- <ApiLink name="lv_obj_get_screen" display="lv_obj_get_screen(widget)" />: Get a Widget's screen ("root" parent).
- <ApiLink name="lv_obj_get_parent" display="lv_obj_get_parent(widget)" />: Get the Widget's current parent.
...
- <ApiLink name="lv_obj_get_child" display="lv_obj_get_child(parent, idx)" />: Return a specific child of a parent.
  Some examples for `idx`:
  
  - `0`: get the first created child
  - `1`: get the second created child
  - `-1`: get the last created child
- <ApiLink name="lv_obj_get_child_by_type" display="lv_obj_get_child_by_type(parent, idx, &lv_slider_class)" />: Similar to <ApiLink name="lv_obj_get_child" /> but filters to children
  with a given type.
- <ApiLink name="lv_obj_find_by_name" display="lv_obj_find_by_name(parent, &quot;text&quot;)" />: Find a Widget with a given name under a parent. It does not have to be a direct child.
- <ApiLink name="lv_obj_get_child_by_name" display="lv_obj_get_child_by_name(parent, &quot;container/button/text&quot;)" />: Get a Widget by navigating a path using parent names (separated by `/`).
- <ApiLink name="lv_obj_get_index" display="lv_obj_get_index(widget)" />: Return the index of the Widget in its parent.
  It is equivalent to the number of older siblings in the parent.
- <ApiLink name="lv_obj_move_to_index" display="lv_obj_move_to_index(widget, idx)" />: Move the Widget to a specified index.
  `0` is the oldest child's position, `-1` is the youngest.
- <ApiLink name="lv_obj_swap" display="lv_obj_swap(widget1, widget2)" />: Swap the positions of two Widgets.
...
When a Widget is created, its reference can be stored in an <ApiLink name="lv_obj_t" display="lv_obj_t *" /> pointer
...
by setting <ApiLink name="LV_USE_OBJ_NAME" /> in `lv_conf.h`.

A custom name can be assigned using <ApiLink name="lv_obj_set_name" display="lv_obj_set_name(obj, &quot;name&quot;)" /> or
<ApiLink name="lv_obj_set_name_static" display="lv_obj_set_name_static(obj, &quot;name&quot;)" />. The "static" variant requires that the passed
...
1. **Get a direct child by name** using <ApiLink name="lv_obj_get_child_by_name" display="lv_obj_get_child_by_name(parent, &quot;child_name&quot;)" />.
   Example:
   <ApiLink name="lv_obj_get_child_by_name" display="lv_obj_get_child_by_name(header, &quot;title&quot;)" />.
   You can also use a path to find nested children:
   <ApiLink name="lv_obj_get_child_by_name" display="lv_obj_get_child_by_name(cont, &quot;buttons/mybtn_2&quot;)" />.
2. **Find a descendant at any level** using <ApiLink name="lv_obj_find_by_name" display="lv_obj_find_by_name(parent, &quot;child_name&quot;)" />.
   Example:
   <ApiLink name="lv_obj_find_by_name" display="lv_obj_find_by_name(cont, &quot;mybtn_1&quot;)" />
