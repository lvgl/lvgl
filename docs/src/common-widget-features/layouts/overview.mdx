---
title: Overview
description: "Layouts assigned to a Widget manage the positions and in some cases the sizes of that Widget's children. They can be used to automatically arrange the children into a line, a column, or into more c..."
---
...
<ApiLink name="lv_obj_set_layout" /> `(widget, <LAYOUT_NAME>)` sets the layout on a Widget.
...
- <ApiLink name="LV_OBJ_FLAG_HIDDEN" /> Hidden Widgets are ignored in layout
  calculations.
- <ApiLink name="LV_OBJ_FLAG_IGNORE_LAYOUT" /> The Widget is simply ignored by the
  layouts. Its coordinates can be set as usual.
- <ApiLink name="LV_OBJ_FLAG_FLOATING" /> Same as
  <ApiLink name="LV_OBJ_FLAG_IGNORE_LAYOUT" /> but the Widget with
  <ApiLink name="LV_OBJ_FLAG_FLOATING" /> will be ignored in
  <ApiLink name="LV_SIZE_CONTENT" /> calculations.

These flags can be added/removed with <ApiLink name="lv_obj_add_flag" display="lv_obj_add_flag(widget, FLAG)" /> and
<ApiLink name="lv_obj_remove_flag" display="lv_obj_remove_flag(widget, FLAG)" />
...
lv_layout_callbacks_t callbacks {
    .layout_update_cb = my_layout_update,
    .get_min_size_cb = my_get_min_size, // or NULL
};
