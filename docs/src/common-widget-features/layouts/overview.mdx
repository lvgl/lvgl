...
- <ApiLink name="lv_obj_set_hidden" /> Hidden Widgets are ignored in layout
  calculations.
- <ApiLink name="lv_obj_set_ignore_layout" /> The Widget is simply ignored by the
  layouts. Its coordinates can be set as usual.
- <ApiLink name="lv_obj_set_floating" /> Same as
  <ApiLink name="lv_obj_set_ignore_layout" /> but a floating Widget is also
  ignored in <ApiLink name="LV_SIZE_CONTENT" /> calculations.

Each is read back with the matching `lv_obj_is_<attribute>()` getter. See
[Flags](/common-widget-features/flags) for the full list.
