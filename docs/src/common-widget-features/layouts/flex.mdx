...
Reordering a row is just <ApiLink name="lv_obj_move_to_index" /> on a flex child:

<LvglExample name="lv_example_flex_list_reorder" path="layouts/flex/lv_example_flex_list_reorder" />
...
<ApiLink name="lv_obj_set_flex_in_new_track" display="lv_obj_set_flex_in_new_track(child, true)" />.

## Ignoring layout

<LvglExample name="lv_example_flex_ignore_layout" path="layouts/flex/lv_example_flex_ignore_layout" />

A child can opt out of flex positioning entirely:

```c
lv_obj_set_ignore_layout(child, true);
```
