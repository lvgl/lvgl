...
To opt a child out of Grid positioning, call
<ApiLink name="lv_obj_set_ignore_layout" /> on that child:

```c
lv_obj_set_ignore_layout(child, true);
```

When this is set, Grid skips the child completely. It does not occupy a
