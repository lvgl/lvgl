...
description: "Similar to many other parts of LVGL, the concept of setting coordinates is inspired by CSS. LVGL does not implement CSS completely, but a comparable subset is provided (sometimes with minor adjustm..."
---
...
- Widget position or size can change by state. E.g., 100 px wide in
  <ApiLink name="LV_STATE_DEFAULT" /> but 120 px in
  <ApiLink name="LV_STATE_PRESSED" />.
...
<ApiLink name="lv_obj_update_layout" /> first to ensure the values are up to date.

Note: Since layout and size may depend on the parent,
<ApiLink name="lv_obj_update_layout" /> recalculates coordinates for **all** widgets on
...
![Box model in LVGL](/_static/images/boxmodel.png)
...
- <ApiLink name="LV_ALIGN_TOP_LEFT" />
- <ApiLink name="LV_ALIGN_TOP_MID" />
- <ApiLink name="LV_ALIGN_TOP_RIGHT" />
- <ApiLink name="LV_ALIGN_BOTTOM_LEFT" />
- <ApiLink name="LV_ALIGN_BOTTOM_MID" />
- <ApiLink name="LV_ALIGN_BOTTOM_RIGHT" />
- <ApiLink name="LV_ALIGN_LEFT_MID" />
- <ApiLink name="LV_ALIGN_RIGHT_MID" />
- <ApiLink name="LV_ALIGN_CENTER" />

Use <ApiLink name="lv_obj_center" display="lv_obj_center(widget)" /> as a shortcut to center a widget.
...
![Align](/_static/images/align.png)

### Layouts

[Flex](/common-widget-features/layouts/flex) and [Grid](/common-widget-features/layouts/grid) layouts can automatically position widgets
...
Use <ApiLink name="LV_SIZE_CONTENT" /> to size the widget based on its children:
...
Ignored for hidden (<ApiLink name="LV_OBJ_FLAG_HIDDEN" />) or floating
(<ApiLink name="LV_OBJ_FLAG_FLOATING" />) widgets.
...
Use them to set limits when using <ApiLink name="LV_SIZE_CONTENT" /> or percentage sizes:
...
By using <ApiLink name="lv_dpx" display="lv_dpx(n)" />, the values are scaled based on the DPI of the display
...
The default DPI of the displays is set by <ApiLink name="LV_DPI_DEF" /> in `lv_conf.h`,
but it can be modified with:

<ApiLink name="lv_display_set_dpi" display="lv_display_set_dpi(disp, new_dpi)" />
