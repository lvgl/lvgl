...
callback using <ApiLink name="lv_obj_add_delete_cb" display="lv_obj_add_delete_cb(widget, cb, item)" />.
The callback will be automatically invoked with `item` as its argument when the widget
is deleted.
...
```c
void my_delete_cb(void * item)
{
    free(item);
}

void * my_data = malloc(64);
lv_obj_add_delete_cb(obj, my_delete_cb, my_data);
```
