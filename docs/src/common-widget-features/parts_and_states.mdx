...
Every state also has a dedicated setter and getter for a single state, which is usually
more convenient:
...
`lv_obj_set_<state>()` and `lv_obj_is_<state>()` exist for all states, e.g.
<ApiLink name="lv_obj_set_alt" />, <ApiLink name="lv_obj_set_focused" />,
<ApiLink name="lv_obj_set_pressed" />, <ApiLink name="lv_obj_is_edited" />.
The custom states are named
<ApiLink name="lv_obj_set_state_user_1" />...`lv_obj_set_state_user_4()` and
<ApiLink name="lv_obj_is_state_user_1" />...`lv_obj_is_state_user_4()`.
