...
To use a ``defconfig`` file, one can use the ``-DDEFCONFIG=<path_to_defconfig>`` flag:

.. code-block:: shell

    cd <lvgl_repo>
    cmake -B build -DLV_USE_KCONFIG=ON -DDEFCONFIG=<path_to_defconfig>
    cmake --build build
...
.. code-block:: shell
    ...
    cmake -B build -DLV_USE_KCONFIG=ON -DDEFCONFIG=configs/defconfigs/my_custom_defconfig
    cmake --build build
