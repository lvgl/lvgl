...
Point count (number of data points in all data series added to the chart)
    ...
    - For LINE-, BAR, and STACKED-charts, this is the number of points on the X axis.
    - LINE-, BAR, and STACKED-charts require only one data-value array to supply Y-values for each data point.
