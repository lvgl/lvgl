...
Configuration for Sphinx doc build of LVGL documentation.

What is built here is hosted at https://docs.lvgl.io/master/ .

Notes from `conf.py` generated by `sphinx-quickstart`:
------------------------------------------------------
...
if not os.path.isfile(version_src_path):
    # This can happen when `conf.py` is being run directly.
    version_src_path = os.path.join(base_path, '../..', cfg_lv_version_file)
