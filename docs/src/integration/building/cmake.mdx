...
| LV_BUILD_SET_CONFIG_OPTS | BOOLEAN | When enabled, this option runs a script that processes the `lv_conf.h`/Kconfig configuration to generate corresponding `CONFIG_LV_*` and `CONFIG_LV_BUILD_*` CMake variables.h`. This requires python3 and a C compiler able to expand the configuration with `-E -dD`, which GCC and Clang are. [pcpp](https://pypi.org/project/pcpp/) can be used as an alternative. If KConfig is used, this is enabled automatically. |
