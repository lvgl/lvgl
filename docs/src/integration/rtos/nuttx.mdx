...
LVGL ships a NuttX driver that covers both display interfaces and the input devices:

| What you need | Use |
|---|---|
| Display from `/dev/fb*` or `/dev/lcd*` | **[NuttX driver](/main-modules/display_drivers/nuttx)** |
| Touchscreen and mouse input | **[NuttX driver](/main-modules/display_drivers/nuttx)** — same driver, `LV_USE_NUTTX_TOUCHSCREEN` / `LV_USE_NUTTX_MOUSE` |
| Keyboard input | **Not supported yet** — see the note below |
| libuv as the event loop manager | `LV_USE_NUTTX_LIBUV`; libuv apps integrate seamlessly |
| Rendering | **[Software renderer](/main-modules/renderers/sw)** — NuttX targets rarely have a GPU LVGL can use |

The [NuttX driver page](/main-modules/display_drivers/nuttx) has the full config option list, the
`lv_nuttx_init()` API and the per-device create functions.

<Callout type="warn">
There is no support for input/uinput keyboards yet. The NuttX X11 sim keyboard driver emits
keycodes that are not standard, so the LVGL integration with it would not be generic if it were
added.
</Callout>
...
The main reason for enabling OS features is for [multi-core rendering](/main-modules/renderers/overview).
