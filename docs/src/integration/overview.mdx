...
If setting up `lvgl/include` as an include path isn't an option for your
...
default (<ApiLink name="LV_USE_CHECK_ARG" />) and its behaviour can be configured.
See [Argument Checking](/debugging/argument_checking).
...
3. **Compiler defines**: values passed on the command line (e.g. `-DLV_COLOR_FORMAT_DEFAULT=LV_COLOR_FORMAT_XRGB8888`).
