...
Under the hood, Kconfig is the single source of truth: `lv_conf_template.h` (the
file you copy) and the internal headers that apply defaults are *generated* from
the [Kconfig](https://github.com/lvgl/lvgl/blob/master/Kconfig) tree, so the
methods stay in sync.

For the full details — setting up `lv_conf.h`, building with Kconfig, and
ready-made presets such as the minimal configuration — see
