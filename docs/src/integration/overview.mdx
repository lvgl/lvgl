...
1. Set the <ApiLink name="LV_CONF_INCLUDE_SIMPLE" /> define to your compiler options (e.g. `-DLV_CONF_INCLUDE_SIMPLE` for GCC compiler) and set the
...
2. Set a custom path via the <ApiLink name="LV_CONF_PATH" /> define. For example: `-DLV_CONF_PATH="/home/joe/my_project/my_custom_conf.h"`.

3. Delete some settings from `lv_conf.h` and set them via compile options. For example: `-DLV_COLOR_DEPTH=32 -DLV_USE_BUTTON=1`.

4. To fully skip `lv_conf.h` and use only compiler options (see point 3), define <ApiLink name="LV_CONF_SKIP" /> as a compiler option. Unset options will get a
