...
The driver implementation is in `src/drivers/display/lovyan_gfx/`.
...
With Kconfig, enable `LV_USE_LOVYAN_GFX` and set `LV_LGFX_USER_INCLUDE` to the
same header name.
...
into your project or use it as the value of `LV_LGFX_USER_INCLUDE`, then enable
exactly one of its options.
...
Internally, LVGL calls these `LGFX` methods:
...
- `pushImageDMA()`
- `waitDMA()`
- `width()` and `height()` for rotated touch coordinates
- `getTouch()` when touch input is enabled
