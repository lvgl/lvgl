...
<ApiLink name="lv_draw_eve_display_init" display="lv_draw_eve_display_init(disp, op_cb)" />, which
powers up and configures EVE. The display is unusable until the second call, so it almost always
follows on the next line. Only what `op_cb` needs goes in between:

```c
lv_display_t * disp = lv_draw_eve_display_create(&params);
lv_display_set_user_data(disp, your_user_data);   /* optional */
if(lv_draw_eve_display_init(disp, op_cb) != LV_RESULT_OK) {
    /* EVE did not come up */
}
```
