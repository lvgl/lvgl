---
title: EVE External GPU Renderer
description: "EVE is a type of external GPU IC which accepts high-level drawing commands over SPI and outputs the rendered graphics to a display over parallel RGB."
---
...
Find your display parameters and populate a <ApiLink name="lv_draw_eve_parameters_t" />
struct with them.
...
{
    /* optional: get the pointer you set with `lv_display_set_user_data()` */
    void * your_user_data = lv_display_get_user_data(disp);
...
<ApiLink name="lv_draw_eve_display_create" display="lv_draw_eve_display_create(params)" />, which
returns the created display but does not touch the chip yet, and then
<ApiLink name="lv_draw_eve_display_init" display="lv_draw_eve_display_init(disp)" />, which
powers up and configures EVE. `op_cb` is set in between with
<ApiLink name="lv_draw_eve_display_set_operation_cb" />, and init fails without it:

```c
lv_display_t * disp = lv_draw_eve_display_create(&params);
lv_display_set_user_data(disp, your_user_data);   /* optional, can be used in for operation_cb */
lv_draw_eve_display_set_operation_cb(disp, op_cb);
if(lv_draw_eve_display_init(disp) != LV_RESULT_OK) {
    /* EVE did not come up */
}
```

It should be called after GPIO and SPI is initialized. You may choose to initialize your IO
...
<ApiLink name="lv_draw_eve_touch_create" display="lv_draw_eve_touch_create(disp)" /> creates a touch [input device (lv_indev)](/main-modules/indev) for the display.
...
Efficient display rotation is fully supported through <ApiLink name="lv_display_set_rotation" />.
...
<ApiLink name="lv_draw_eve_pre_upload_image" /> is used to upload images.
<ApiLink name="lv_draw_eve_pre_upload_font_range" /> is used to upload a range of characters.
...
<ApiLink name="lv_draw_eve_pre_upload_font_text" /> is used to upload all the glyphs that
...
[offline font converter](https://github.com/lvgl/lv_font_conv) and specify a stride
...
The functions <ApiLink name="lv_draw_eve_memread8" />, <ApiLink name="lv_draw_eve_memread16" />, <ApiLink name="lv_draw_eve_memread32" />,
<ApiLink name="lv_draw_eve_memwrite8" />, <ApiLink name="lv_draw_eve_memwrite16" />, and <ApiLink name="lv_draw_eve_memwrite32" />
...
as <ApiLink name="LV_EVE_REG_ID" /> and `EVE_ROM_CHIPID` is available as
<ApiLink name="LV_EVE_EVE_ROM_CHIPID" />, etc.
