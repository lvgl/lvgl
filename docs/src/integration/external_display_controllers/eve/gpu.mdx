...
<ApiLink name="lv_draw_eve_touch_create" display="lv_draw_eve_touch_create(disp)" /> creates a touch [Input devices (lv_indev)](/main-modules/indev) for the display.
...
as <ApiLink name="LV_EVE_REG_ID" /> and <ApiLink name="EVE_ROM_CHIPID" /> is available as
