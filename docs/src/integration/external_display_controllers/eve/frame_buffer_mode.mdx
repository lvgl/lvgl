---
title: "EVE (FT81x)"
description: A minimal framebuffer driver for EVE FT81x smart display controllers. Works for BT81x too.
---
...
{
    spi_device_handle_t spi = lv_display_get_user_data(disp);
    switch(operation) {
...
int main() {
    ...
    /* the display is unusable until `lv_ft81x_init` runs; only what `spi_cb` needs goes between */
    lv_display_t * disp = lv_ft81x_create(&params, fb, FB_SIZE);
    lv_display_set_user_data(disp, spi);
    lv_ft81x_set_spi_cb(disp, spi_cb);
    if(lv_ft81x_init(disp) != LV_RESULT_OK) {
