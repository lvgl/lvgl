---
title: "Generic MIPI DCS-Compatible LCD Controller Driver"
description: "From the Wikipedia:"
---

## Overview

From the [Wikipedia](https://en.wikipedia.org/wiki/MIPI_Alliance):

[MIPI Alliance](https://www.mipi.org/) is a global business alliance that
...
MIPI driver, so you do not need to set <ApiLink name="LV_USE_GENERIC_MIPI" /> explicitly
...
responsibility to call <ApiLink name="lv_display_flush_ready" /> when the color transfer
...
them to the display object as usual by calling <ApiLink name="lv_display_set_buffers" />
...
int main(int argc, char ** argv)
{
    ...
    /* Create the LVGL display object, then initialize the LCD controller */
    my_disp = lv_lcd_generic_mipi_create(LCD_H_RES, LCD_V_RES);
    lv_lcd_generic_mipi_set_send_cmd_cb(my_disp, my_lcd_send_cmd);
    lv_lcd_generic_mipi_set_send_color_cb(my_disp, my_lcd_send_color);
    if (lv_lcd_generic_mipi_init(my_disp, LV_LCD_FLAG_NONE) != LV_RESULT_OK) {
        lv_display_delete(my_disp);
        return;
    }
...
### Reaching your own data from the callbacks

The display is unusable until <ApiLink name="lv_lcd_generic_mipi_init" /> has run, so it almost
always follows <ApiLink name="lv_lcd_generic_mipi_create" /> on the next line.  Only what
`my_lcd_send_cmd` and `my_lcd_send_color` need goes in between:

```c
lv_display_t * disp = lv_lcd_generic_mipi_create(LCD_H_RES, LCD_V_RES);
lv_display_set_user_data(disp, &my_spi_context);
lv_lcd_generic_mipi_set_send_cmd_cb(disp, my_lcd_send_cmd);
lv_lcd_generic_mipi_set_send_color_cb(disp, my_lcd_send_color);
lv_lcd_generic_mipi_init(disp, LV_LCD_FLAG_NONE);
```

Inside the callbacks, use `lv_display_get_user_data(disp)` to get the pointer back.

The chip-specific drivers follow the same pattern: `lv_st7789_create()` /
`lv_st7789_init()`.

### Init flags

The second and last argument of the <ApiLink name="lv_lcd_generic_mipi_init" /> function is a flag
...
disp = lv_lcd_generic_mipi_create(MY_DISPLAY_PIXEL_WIDTH, MY_DISPLAY_PIXEL_HEIGHT);
lv_lcd_generic_mipi_set_send_cmd_cb(disp, my_lcd_send_cmd);
lv_lcd_generic_mipi_set_send_color_cb(disp, my_lcd_send_color);
lv_lcd_generic_mipi_init(disp, LV_LCD_FLAG_MIRROR_X | LV_LCD_FLAG_BGR);
```
...
<ApiLink name="LV_LCD_CMD_DELAY_MS" />, which must be followed by the delay given in 10ms units.  To
terminate the command list you must use a delay with a value of <ApiLink name="LV_LCD_CMD_EOF" />,
as shown above.

See an actual example of sending a command list
[here](https://github.com/lvgl/lvgl/blob/master/src/drivers/display/st7789/lv_st7789.c).
