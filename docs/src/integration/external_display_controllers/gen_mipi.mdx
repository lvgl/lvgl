...
The only difference between the <ApiLink name="my_lcd_send_cmd" />
and <ApiLink name="my_lcd_send_color" /> functions is that <ApiLink name="my_lcd_send_cmd" /> is
used to send short commands and it is expected to complete the transaction when it
returns (in other words, it should be blocking), while <ApiLink name="my_lcd_send_color" />
...
The third argument of the <ApiLink name="lv_lcd_generic_mipi_create" /> function is a flag
...
You can use the <ApiLink name="my_lcd_send_cmd" /> function to send an arbitrary command to
