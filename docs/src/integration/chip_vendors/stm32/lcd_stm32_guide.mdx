---
title: SPI Display Driver Creation for STM32
description: "Driving an RGB565 240x320 SPI display and an I2C touch panel from an STM32 HAL project, with example flush and read callbacks."
faqs:
  - question: "Which render mode does the example use?"
    answer: "Single-buffered direct mode, but partial mode works too. Performance can be improved by using DMA with double buffering."
  - question: "When should the display initialization commands be sent?"
    answer: "Before entering the timer loop. The generated STM32 HAL code initializes the SPI peripheral itself, based on the STM32CubeIDE or STM32CubeMX configuration."
  - question: "Why does the flush callback check lv_display_flush_is_last()?"
    answer: "In direct mode LVGL renders into the full-size buffer and flushes it once per refresh, so the whole buffer only needs to be transmitted on the last flush call."
  - question: "How does the touch driver know when data is available?"
    answer: "Through an interrupt pin. The STM32 HAL calls HAL_GPIO_EXTI_Falling_Callback(), which sets a flag that the read callback checks with the interrupt temporarily disabled."
---

## Display Driver

Here is how you can drive an RGB565 240x320 SPI display using STM32 HAL SPI.
...
lv_display_set_color_format(disp, LV_COLOR_FORMAT_RGB565); /* not needed if LV_COLOR_FORMAT_DEFAULT is LV_COLOR_FORMAT_RGB565 */
```
