...
- Select NXP PXP engine in "lv_conf.h": Set <ApiLink name="LV_USE_DRAW_PXP" /> to `1`.
...
- Enable VGLite asserts in "lv_conf.h": Set <ApiLink name="LV_USE_VGLITE_ASSERT" /> to
  ...
  can stop the program execution in case the <ApiLink name="LV_ASSERT_HANDLER" /> is set
  to `while(1);` (Halt by default). Else, there will be logged just an error
  message via `LV_LOG_ERROR`.

#### Basic initialization:

Initialize VGLite GPU before calling <ApiLink name="lv_init" /> by specifying the
...
VGLite draw initialization is done automatically in <ApiLink name="lv_init" /> once
...
- Enable VGLite draw task synchronously in "lv_conf.h":
  Set <ApiLink name="LV_USE_VGLITE_DRAW_ASYNC" /> to `1`.
...
- Select NXP G2D engine in "lv_conf.h": Set <ApiLink name="LV_USE_DRAW_G2D" /> to `1`.
- In order to use G2D as a draw unit, select in "lv_conf.h": Set <ApiLink name="LV_USE_DRAW_G2D" /> to `1`.
