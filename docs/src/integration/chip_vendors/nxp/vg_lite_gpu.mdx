---
title: "VG-Lite General GPU"
description: "A generic VG-Lite rendering backend that uses VeriSilicon's generic API, so any chip adapted to the VG-Lite API can accelerate LVGL without adaptation work."
faqs:
  - question: "Which chips can use the generic VG-Lite backend?"
    answer: "Any chip whose hardware has been adapted to the VG-Lite API, regardless of the manufacturer. As long as the same version of the VG-Lite API is used as the rendering backend, no LVGL adaptation work is needed."
  - question: "Who initializes the GPU, LVGL or the SDK?"
    answer: "Either. If the SDK initializes the GPU during system startup, set LV_VG_LITE_USE_GPU_INIT to 0. If LVGL should initialize it, set the option to 1 and implement the public gpu_init() function in the SDK."
  - question: "What should I do when VG-Lite returns VG_LITE_OUT_OF_RESOURCES?"
    answer: "Increase the size of the GPU memory pool, or reduce the number of gradient cache entries (LV_VG_LITE_GRAD_CACHE_CNT) or stroke cache entries (LV_VG_LITE_STROKE_CACHE_CNT). Each gradient consumes around 4 KB of the GPU memory pool."
  - question: "How can I debug a VG-Lite adaptation without hardware?"
    answer: "LVGL integrates a VG-Lite simulator based on ThorVG. Enabling LV_VG_LITE_USE_ASSERT also adds strict parameter checking, which catches issues like a missing path end symbol or a wrong buffer stride."
---
...
<Callout type="info">
This backend replaced the NXP-specific VGLite draw unit, which was removed in LVGL v9.4.
</Callout>
...
Set <ApiLink name="LV_USE_DRAW_VG_LITE" /> to 1 in `lv_conf.h` to enable the VG-Lite rendering backend.
...
<Step>
**Confirm the GPU initialization method**
...
Set the <ApiLink name="LV_VG_LITE_USE_ASSERT" /> configuration to enable GPU call parameter checking.
...
<Step>
**Specify the flush method**

Set the <ApiLink name="LV_VG_LITE_FLUSH_MAX_COUNT" /> configuration to specify the flush method.
...
<Step>
**Choose how shadows are rendered**

Set the <ApiLink name="LV_VG_LITE_USE_BOX_SHADOW" /> configuration to use GPU rendering for shadows.
...
<Step>
**Size the gradient cache**

Set the <ApiLink name="LV_VG_LITE_GRAD_CACHE_CNT" /> configuration to specify the number of gradient cache entries.
...
<Step>
**Size the stroke cache**

Set the <ApiLink name="LV_VG_LITE_STROKE_CACHE_CNT" /> configuration to specify the number of stroke path caches.
...
For detailed instructions, see [VG-Lite GPU Simulator](/debugging/vg_lite_tvg).
...
| --- | --- | --- |
| `LV_COLOR_FORMAT_I1` | `LV_COLOR_FORMAT_I8` | VG-Lite index formats require endian + bit flipping, converted to I8 for simplicity |
...
| `LV_COLOR_FORMAT_RGB888` | `LV_COLOR_FORMAT_XRGB8888` | Converted when GPU doesn't support 24-bit format |
| `LV_COLOR_FORMAT_ARGB8565` | `LV_COLOR_FORMAT_ARGB8888` | Converted when GPU doesn't support 24-bit format |
