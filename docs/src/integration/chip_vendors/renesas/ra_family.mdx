...
- Double-click on `configuration.xml`. This will activate the `Configuration Window`.
  ...
  ![Code generation with FSP](/_static/images/renesas/generate.png)
- Build the project by pressing `Ctrl` + `Alt` + `B`
- Click the Debug button (![img_debug_btn](/_static/images/renesas/debug_btn.png)). If prompted with `Debug Configurations`,
  on the `Debugger` tab select the `J-Link ARM` as `Debug hardware` and the proper
  IC as `Target Device`:

  - `R7FA8D1BH` for EK-RA8D1

    ![Debugger parameters for RA8](/_static/images/renesas/debug_ra8.png)

  - `R7FA6M3AH` for EK-RA6M3G

    ![Debugger parameters for RA6](/_static/images/renesas/debug_ra6.png)
