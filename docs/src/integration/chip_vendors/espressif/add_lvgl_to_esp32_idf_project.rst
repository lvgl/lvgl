...
The simplest way to integrate LVGL into your ESP-IDF project is via the `esp_lvgl_port <https://components.espressif.com/components/espressif/esp_lvgl_port>`__ component. This component provides helper functions for easy installation of LVGL and display drivers. Moreover, it can add support for touch, rotary encoders, button or USB HID inputs. It simplifies power savings, screen rotation and other platform specific nuances.
...
In case you do not want to use esp_lvgl_port, you can add `LVGL component <https://components.espressif.com/component/lvgl/lvgl>`__ into your project with the following command:
...
If you prefer using third-party libraries like `LovyanGFX <https://github.com/lovyan03/LovyanGFX>`_ as your display driver:
...
3. Enable the driver in ``menuconfig`` under ``Component config -> LVGL configuration -> Devices -> Use LovyanGFX driver``.
...
It is possible to create per-chip default configuration files. These files are named
starting with the chip variant; for example, `sdkconfig.esp32p4` will only apply the
default configuration for an ESP32-P4 IDF project.
...
or `lvgl_port_init()` if LVGL was manually configured (e.g. without using the `esp_bsp` component).
...
the ESP-LVGL port directly. Note that because the background task is running,
locking is required when calling LVGL functions:
...
Building an IDF project that features LVGL usage is similar to any other
...
After flashing, the monitor console will be launched automatically.
