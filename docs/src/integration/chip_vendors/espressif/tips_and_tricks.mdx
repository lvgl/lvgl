...
## DMA2D image copy/conversion warnings

When enabling `CONFIG_LV_USE_ESP_DMA2D`, a common symptom of misconfiguration is seeing
repeated `esp_cache_msync` alignment errors and DMA2D draw warnings in monitor logs.

For stable operation, make sure:
...
- `CONFIG_LV_DRAW_BUF_ALIGN` should match cache line size on ESP32-P4 (64 bytes).
- DMA2D draw unit only accelerates eligible tasks; unsupported ones automatically fall back to SW.
- If your application is memory-bandwidth bound, reduce burst size or limit simultaneous DMA-heavy paths.
