---
title: Overview
description: "Alif Semiconductor is a next-generation chip vendor making chips that come in a variety of configurations. Their chips are designed to excel at multiple aspects at once."
faqs:
  - question: "Which Alif project should I start from?"
    answer: "The alif_m55-lvgl example project, which is based on the Alif VS Code Template. Any project based on that template has a nearly identical setup process."
  - question: "Do I need the Alif Security Toolkit?"
    answer: "Yes. Flashing goes through the Security Toolkit, and its extracted path has to be set as alif.setools.root in the VS Code settings.json."
  - question: "Flashing fails or asks for a COM port again. What should I do?"
    answer: "The serial port chosen on the first flash is saved as the default. Run the Program with Security Toolkit (select COM port) configuration to override it."
  - question: "Should I build for the HP or the HE core?"
    answer: "Build for the HP (High Performance) core and pick the release build type for the best LVGL performance. The HE (High Efficiency) core is the choice when power usage matters more."
  - question: "Is the GPU used for rendering?"
    answer: "Yes, the example project renders with LVGL's D/AVE 2D draw unit. See the Dave2D GPU page."
---
...
such as with Zephyr RTOS. See [Alif's GitHub repos](https://github.com/orgs/alifsemi/repositories).

This guide is for Linux and Windows.

This project uses D/AVE 2D rendering acceleration with LVGL's
[D/AVE 2D draw unit](/main-modules/renderers/dave2d).
...
with the output of `pwd`, is the path you need to use later, e.g.
...
Press <Kbd>Ctrl</Kbd> + <Kbd>Shift</Kbd> + <Kbd>P</Kbd>. Type "preferences" and select the option
"Preferences: Open User Settings (JSON)" from the choices.
A `settings.json` will open.

<Callout type="info">
If using a Docker container, it's better to edit the JSON file in the project directory at
...
Click the hammer icon next to the play icon to compile the project.
...
| What you need | Use |
|---|---|
| 2D GPU acceleration | **[Dave2D](/main-modules/renderers/dave2d)** — Ensemble chips contain a TES D/AVE 2D GPU |
| Helium acceleration on the CPU | **[Arm-2D](/main-modules/renderers/arm2d)**, or `LV_USE_DRAW_SW_ASM`. See [Arm](/integration/chip_vendors/arm) |
| An SPI panel | **[LCD controllers](/main-modules/display_drivers/lcd)** |
| Touch input | Write a short read callback. See [Touchpad and Mouse](/main-modules/indev/pointer) |

The [alif_m55-lvgl](https://github.com/alifsemi/alif_m55-lvgl) project used above already has the
Dave2D draw unit enabled, so it accelerates rendering out of the box. No Alif-specific draw unit
is needed. Alif and Renesas expose the same D/AVE 2D driver API.
