...
The two kinds share one buffer and are told apart by their content: a payload counts as
text when it ends with a `NUL` and holds no other `NUL`, which is exactly what
`lv_qrcode_set_text()` stores. That keeps the Widget one `uint16_t` of flags, but it has a
caveat - a binary payload whose *only* `NUL` is its last byte is indistinguishable from
text, so that byte is dropped:

```c
const uint8_t bin[] = {'h', 'i', 0x00};
lv_qrcode_set_data(qr, bin, sizeof(bin));   /* encodes only "hi" - the trailing NUL is dropped */

const uint8_t ok[] = {'h', 0x00, 'i'};
lv_qrcode_set_data(qr, ok, sizeof(ok));     /* 3 bytes: the NUL is not at the end */
```

If you encode binary data that can end in a single `NUL`, append your own sentinel or
otherwise avoid a lone trailing `NUL`.
