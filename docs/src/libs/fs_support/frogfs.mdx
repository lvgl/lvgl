---
title: FrogFS
description: "frogfs is a read-only filesystem for packing a directory tree of files into a single \"blob\" which can be distributed easily or built into a firmware. It is suitable for packaging XML files into you..."
---

[frogfs](https://github.com/jkent/frogfs)
is a read-only [filesystem](/main-modules/fs) for packing a directory
...
app. See the intro to [XML in LVGL](/xml/xml) if you are
...
frogfs project. Get it from [GitHub](https://github.com/jkent/frogfs)
with `git clone`.
...
Set <ApiLink name="LV_USE_FS_FROGFS" /> to `1` in your `lv_conf.h`. Set
<ApiLink name="LV_FS_FROGFS_LETTER" /> to a letter like `'F'`.
...
If you have a frogfs blob stored in a different [filesystem](/main-modules/fs)
...
See the [other filesystems available in LVGL](/libs/fs_support/fs).
...
See the intro to [XML in LVGL](/xml/xml).
