...
2. **Call `lv_init()`** - Initialize LVGL itself.
3. **Create display and input devices** - Create display(s) (<ApiLink name="lv_display_t" />) and input device(s) (<ApiLink name="lv_indev_t" />) and set up their callbacks.
4. **Create the UI** - Call LVGL functions to create screens, widgets, styles, animations, events, etc.
5. **Call `lv_timer_handler()` in a loop** - This handles all the LVGL-related tasks: - refresh display(s), - read input devices, - fire events based on user input (and other things), - run any animations, and - run user-created timers.
