...
used as constant and [Subject](/main-modules/observer/observer) types.
...
| `int` | Integer number in the range of roughly -2B to 2B by default. (Same as `int32_t` in C.) |
| `px` | Simple pixel units. The unit `px` can be omitted. |
| `%` | Percentage. `%` must be appended to the value as the unit. (Means the same as <ApiLink name="lv_pct" />.) |
| `content` | Means <ApiLink name="LV_SIZE_CONTENT" />. |
...
| `color` | A color stored as 24-bit RGB (<ApiLink name="lv_color_t" />). |
| `opa` | Opacity value in the range of 0 to 255 or 0 to 100%. Like CSS, percentage values must be be followed by '%'. |
| `lv_obj` | Pointer to a Widget (<ApiLink name="lv_obj_t" display="lv_obj_t *" />). |
| `screen` | Pointer to a screen (also <ApiLink name="lv_obj_t" display="lv_obj_t *" />). |
...
| `style_prop` | A style property, for example `"bg_opa"` meaning <ApiLink name="LV_STYLE_BG_OPA" />. |
...
To learn more about how to register the name-data pairs visit [Loading XML Files at Runtime](/xml/integration/xml).
...
| `string[]` | No `NULL` termination and no count parameter. Used when the number of elements is not known or delivered via another mechanism, such as via another setting. |
