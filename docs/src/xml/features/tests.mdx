---
title: UI Testing
description: The XML test module is a powerful and flexible way to define functional UI tests.
---
...
- `<screenshot_compare path="path/to/image.png"/>: Compares the current screen with a reference image.
  ...
  - If the comparison fails, an image with err` suffix is saved for inspection.
...
- From file: <ApiLink name="lv_xml_test_register_from_file" display="lv_xml_test_register_from_file(&quot;path/to/test.xml&quot;, &quot;A:ref_images&quot;)" />
- From string: <ApiLink name="lv_xml_test_register_from_data" display="lv_xml_test_register_from_data(xml_data, &quot;A:ref_images&quot;)" />

The second parameter specifies a prefix for screenshot comparison paths.

To unregister tests, use:

- <ApiLink name="lv_xml_test_unregister" />
...
To execute the registered test, use <ApiLink name="lv_xml_test_run_all" display="lv_xml_test_run_all(slowdown)" />.
...
First, call <ApiLink name="lv_xml_test_run_init" /> to prepare for executing the steps.
It will clean the screen and create a fresh instance of the `view` to be tested.

Get the number of steps using <ApiLink name="lv_xml_test_get_step_count" />
and then call <ApiLink name="lv_xml_test_run_next" display="lv_xml_test_run_next(slowdown)" /> as many times.

<ApiLink name="lv_xml_test_run_next" /> returns `true` if the given step passed, or `false` if it failed.

Finally, call <ApiLink name="lv_xml_test_run_stop" /> to clean up and exit testing mode.
