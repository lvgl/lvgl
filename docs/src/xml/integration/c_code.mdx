---
title: Using the Exported C Code
description: "When C code is exported from LVGL's UI Editor, no XML is needed at all (LV_USE_XML can be 0), and it's very similar to the case when the C code is written by hand."
---

## Overview

When C code is exported from LVGL's UI Editor, no XML is needed at all
(<ApiLink name="LV_USE_XML" /> can be `0`), and it's very similar to the case when the C code is
written by hand.
...
Note that "init" function creates only the [permanent screens](/xml/ui_elements/screens) but
...
The generated files end with `_gen.c or gen.h`. These shouldn't be modified
...
[lv_timer](/main-modules/timer), a subject with a special observer, complex animations, or
