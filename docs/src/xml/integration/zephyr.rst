...
through the ``prj.conf`` file to configure parameters, thus avoiding direct modification of ``lv_conf.h``.

Configuration
-------------

Zephyr has its own configuration wrapper for LVGL that translates Kconfig options into LVGL settings.
Therefore, some symbols need to be included in ``prj.conf``:
...
Additionally, it is necessary to enable display and input drivers in ``prj.conf``:
