...
Use <ApiLink name="lv_obj_set_style_text_align" display="lv_obj_set_style_text_align(spangroup, LV_TEXT_ALIGN_..., LV_PART_MAIN)" />
to set text alignment.

## Modes

A Spangroup's mode is not set directly, it follows from its width and height:

- <ApiLink name="LV_SPAN_MODE_EXPAND" /> the width is <ApiLink name="LV_SIZE_CONTENT" />, so the
  Spangroup expands to the text size and stays on one line.
- <ApiLink name="LV_SPAN_MODE_BREAK" /> the width is fixed and the height is
  <ApiLink name="LV_SIZE_CONTENT" />, so lines that are too long are broken and the height
  auto-expands.
- <ApiLink name="LV_SPAN_MODE_FIXED" /> both the width and the height are fixed.

Read the resulting mode with
<ApiLink name="lv_spangroup_get_mode" display="lv_spangroup_get_mode(spangroup)" />.
