...
<LvglXMLExample name="lv_example_calendar_basic" path="widgets/calendar/calendar_basic/lv_example_calendar_basic" />
...
The mode isn't yet exposed through an XML attribute and requires a CJK
font, so the example is C-only:
