...
<ApiLink name="lv_obj_set_adv_hittest" display="lv_obj_set_adv_hittest(slider, true)" />.
...
A Slider's value can also drive another widget's state through a custom
Observer added with <ApiLink name="lv_subject_add_observer_obj" /> — handy for
guards like "disable submit when the value exceeds a threshold".
