...
<LvglXMLExample name="lv_example_scale_modes" path="widgets/scale/scale_modes/lv_example_scale_modes" />
...
<LvglXMLExample name="lv_example_scale_round_modes" path="widgets/scale/scale_round_modes/lv_example_scale_round_modes" />

### Setting range

<LvglXMLExample name="lv_example_scale_range" path="widgets/scale/scale_range/lv_example_scale_range" />
...
<LvglXMLExample name="lv_example_scale_ticks" path="widgets/scale/scale_ticks/lv_example_scale_ticks" />
...
<LvglXMLExample name="lv_example_scale_styling" path="widgets/scale/scale_styling/lv_example_scale_styling" />

Scale has three stylable parts: <ApiLink name="LV_PART_MAIN" /> (the rail
line or arc), <ApiLink name="LV_PART_ITEMS" /> (every minor tick) and
<ApiLink name="LV_PART_INDICATOR" /> (every major tick *plus* its label).
Use line-family properties (`line_color`/`line_width`/`line_opa`) on each
part for the ticks and rail; on INDICATOR you can also set text properties
to style the numeric labels.

The label rotation comes from the indicator's `transform_rotation` style —
units are 0.1°, so `style_transform_rotation-indicator="300"` tilts every
label by 30°. Use this on linear scales when dense labels would overlap,
or to lay out angled labels on round modes.
...
<LvglXMLExample name="lv_example_scale_bind_section" path="widgets/scale/scale_bind_section/lv_example_scale_bind_section" />
