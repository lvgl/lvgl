...
Attach a style to a section with one setter per part:

- <ApiLink name="lv_scale_set_section_style_main" display="lv_scale_set_section_style_main(scale, section, &style)" />
- <ApiLink name="lv_scale_set_section_style_indicator" display="lv_scale_set_section_style_indicator(scale, section, &style)" />
- <ApiLink name="lv_scale_set_section_style_items" display="lv_scale_set_section_style_items(scale, section, &style)" />

To restyle multiple parts, attach the same style object to each part
separately. Each (section, part) pair holds exactly one style; setting again
replaces, not adds. Pass `NULL` to detach the part's style.
