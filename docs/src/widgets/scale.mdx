...
- For the main line *when it is a straight line* (<ApiLink name="LV_PART_MAIN" />):
...
- For the main line or a section *when it is an arc* (<ApiLink name="LV_PART_MAIN" />):
  ...
  :LV_STYLE_PAD_RADIAL:         <ApiLink name="lv_style_set_pad_radial" />

  Note that PAD_RADIAL applies only to sections. Positive values will move the arc toward the center.
