...
Here's the updated section with a clearer callout:
...
#### GPU (VG-Lite)
Here's a clearer version with the possible values spelled out:
...
Each backend-selecting driver (SDL, Wayland and DRM) now expose an explicit `LV_<DRIVER>_BACKEND` choice instead of inferring the backend automatically.
...
way. New projects should use `lLV_ASSERT_CUSTOM_INCLUDE` and define their
