...
- A few options were renamed or restructured (see below). Old names still work
  for now — they emit a `#warning` at compile time and are automatically mapped
  to the new option — but they are **removed in v10.0**, so update them.
...
re-run `scripts/generate_lv_conf.py`. Either way, watch the compiler output for
the deprecation `#warning`s listed below.

### Minimal configuration

The `LV_CONF_MINIMAL` switch — which disabled most widgets, themes, layouts and
...
preset now lives at `configs/defconfigs/minimal.defconfig`:

```bash
# Build LVGL from the minimal preset
...
warning. Start from the minimal defconfig and re-enable only what you need, or —
on the `lv_conf.h` path — simply disable the widgets/themes/layouts you don't
use directly.
...
place), and is **removed in v10.0**. Watch your build log after upgrading and
update each one the warning points at.
...
Enable the library and pick the source with the `_INTERNAL` flag:
...
The VG-Lite GPU is now chosen with a single `LV_VG_LITE_GPU` option instead of a
free-form series/revision pair:
...
Each backend-selecting driver now exposes an explicit `LV_<DRIVER>_BACKEND`
choice instead of inferring the backend automatically.

- **SDL** — buffer count is now a single integer; the backend is explicit:

  | Removed | Replacement |
  |---|---|
  | `LV_SDL_SINGLE_BUFFER` | `LV_SDL_BUF_COUNT` = 1 |
  | `LV_SDL_DOUBLE_BUFFER` | `LV_SDL_BUF_COUNT` = 2 |
  | `LV_SDL_AUTO_BACKEND` | set to 0 and select `LV_SDL_BACKEND` |

- **Linux DRM** — `LV_LINUX_DRM_AUTO_BACKEND`: set to 0 and select a backend
  with `LV_LINUX_DRM_BACKEND`.

- **Wayland** — setting `LV_WAYLAND_USE_EGL` / `LV_WAYLAND_USE_G2D` /
  `LV_WAYLAND_USE_SHM` directly is deprecated. Set `LV_WAYLAND_AUTO_BACKEND` to
  0 and select a backend with `LV_WAYLAND_BACKEND`.

- **X11** — render mode is now a single value mapped onto the standard
  `LV_DISPLAY_RENDER_MODE_*` constants:
...
| Removed | Replacement |
|---|---|
| `LV_ENABLE_GLOBAL_CUSTOM` | `LV_GLOBAL_USE_CUSTOM_INCLUDE` |
