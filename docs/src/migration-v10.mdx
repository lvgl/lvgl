...
cmake -B build -DLV_USE_KCONFIG=ON -DLV_DEFCONFIG_PATH=configs/defconfigs/empty.defconfig
```
...
Some settings may produce errors, in this case we mention what to define so that you
freely move around from v9.6 to previous v9 versions without warnings/errors.

Watch your build log after upgrading and update each one the warning points at.
