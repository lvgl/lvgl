...
People migrating might encounter a compilation error if one of the old `LV_USE_ASSERT_*` macros is enabled in `lv_conf.h` as 
they are now gated by the new symbol. Simply enable it if you wish to continue using assertions.
