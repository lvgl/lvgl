...
- Caveat for binary payloads: text and binary share one buffer and are told apart by a
  heuristic (text ends with a `NUL` and contains no other `NUL`). A binary payload whose
  only `NUL` is its final byte is treated as text, so that byte is not encoded. If you
  encode binary data that can end in a single `NUL`, append your own sentinel or otherwise
  avoid a lone trailing `NUL`.
