---
title: Migrating from v9 to v10-dev
...
<Callout type="info" title="v10.0.0">
  LVGL v10.0.0 is currently in development and not yet released.
</Callout>
...
`LV_COLOR_16_SWAP` was kept for compatibility with LVGL v8, with the addition of <ApiLink name="LV_COLOR_FORMAT_DEFAULT"/> which you can use to make LVGL
render natively in big-endian RGB565 format, this config now produces a warning and will be completely removed in v10.

If you're still using this configuration, simply define <ApiLink name="LV_COLOR_FORMAT_DEFAULT"/> as <ApiLink name="LV_COLOR_FORMAT_RGB565_SWAPPED"/> 
or set the display's color format at runtime with <ApiLink name="lv_display_set_color_format" display="lv_display_set_color_format(display, LV_COLOR_FORMAT_RGB565_SWAPPED)"/>.
...
Starting in v9.6, LVGL's public API headers were moved into `include/lvgl`.
The canonical way to bring in LVGL is to add `lvgl/include` to your
include paths and use:

```c
#include <lvgl/lvgl.h>
```

Including the top-level `lvgl/lvgl.h` file is also fully supported if adding a new include path
isn't an option for your project.

The header files remaining under `src` are now LVGL's **private API**
and are not meant to be included directly.

**If your project includes headers that were moved out of `src` directly:**
Those headers still exist and still work in v9.6, but including any of
them now triggers a deprecation warning. Direct access to headers under
`src` will be removed entirely in v10, so switch to including via
`lvgl/include/lvgl/` or `lvgl/lvgl.h` before upgrading.

### Wayland

- `lv_wayland_display_close_cb_t` and the `close_cb` parameter of
  <ApiLink name="lv_wayland_window_create"/> have been removed. Add an
  `LV_EVENT_DELETE` event to the display the function returns instead.

  ```c
  /* Before */
  lv_display_t * disp = lv_wayland_window_create(H_RES, V_RES, "Title", my_close_cb);

  /* After */
  lv_display_t * disp = lv_wayland_window_create(H_RES, V_RES, "Title");
  lv_display_add_event_cb(disp, my_close_cb, LV_EVENT_DELETE, NULL);
  ```

  The v9.4 alias `lv_wayland_display_close_f_t` is gone with it.
...
LVGL v9.6 standardizes public-API argument validation. `LV_ASSERT_OBJ` and
`LV_ASSERT_STYLE` have been removed. Use <ApiLink name="LV_CHECK_OBJ" /> and
<ApiLink name="LV_CHECK_ARG" />, which log a warning and let the caller recover
gracefully instead of halting the program.
...
The `LV_USE_ASSERT_OBJ` config option is gone with `LV_ASSERT_OBJ`. The
equivalent checks are configured per layer with `LV_USE_CHECK_OBJ_CLASSTYPE`
and `LV_USE_CHECK_OBJ_VALIDITY`, see below. `LV_USE_ASSERT_STYLE` remains, it
still controls the style sentinel field that `LV_STYLE_SENTINEL_OK()` reads.
...
- `lv_obj_find_by_id()` has been removed, use <ApiLink name="lv_obj_find_by_name"/> instead:
  ```c
  /* Before */
...
- The generic flag API — `lv_obj_add_flag()`, `lv_obj_remove_flag()`,
  `lv_obj_set_flag()`, `lv_obj_has_flag()` and `lv_obj_has_flag_any()` — has been
  removed, along with the v8/v9 aliases `lv_obj_clear_flag()` and
  `lv_obj_update_flag()`. Each flag now has a dedicated setter
  (`lv_obj_set_<flag>(obj, en)`) and getter (`lv_obj_is_<flag>(obj)`):
  ```c
  /* Before */
  ...
  The `lv_obj_flag_t` enum itself is kept: the `LV_PROPERTY_OBJ_FLAG_*` property
  range still uses it, and the GDB pretty-printer decodes a widget's flags from it.
...
- `lv_obj_style_set_disabled()` and `lv_obj_style_get_disabled()` have been removed
...
The `lv_subject_init_<type>()` family and `lv_subject_deinit()` have been removed.
A Subject is now allocated by LVGL with
...
<ApiLink name="lv_subject_delete"/> accepts `NULL`, and unlike the removed
...
- `lv_subject_copy_string()` is deprecated in favor of
  <ApiLink name="lv_subject_set_string"/>, so that every Subject type is written
  with `lv_subject_set_<type>()`. The behavior is unchanged: the string is still
...
- The `lv_obj_bind_flag_if_*` and `lv_obj_bind_state_if_*` families
  (`_eq`/`_not_eq`/`_gt`/`_ge`/`_lt`/`_le`) have been removed. For a boolean subject,
...
- `lv_spangroup_set_align()` has been removed, use the `text_align` style property instead:
  ```c
  /* Before */
...
- `lv_spangroup_set_mode()` has been removed, control expanding/wrapping by setting the widget width instead:
  ```c
  /* Before */
  ...
  The `align` and `mode` entries of the property API (`LV_PROPERTY_SPAN_ALIGN`
  and `LV_PROPERTY_SPAN_MODE`) are gone with them. Use the `text_align` style
  property and the widget's width/height instead.

### lv_textarea

- `lv_textarea_set_align()` has been removed, use the `text_align` style property instead:
  ```c
  /* Before */
...
- `lv_scale_section_set_style()` has been removed, use the per-part setters instead:
  ```c
  /* Before */
...
- The `lv_menu` widget is removed. A menu is page navigation over base widgets
...
- The `lv_list` widget has been removed, along with `LV_USE_LIST`. A list is just a
  flex container with a column flow, so build one directly from `lv_obj` with a
  `LV_FLEX_FLOW_COLUMN` layout instead. See the
  [`lv_example_flex_list`](common-widget-features/layouts/flex#building-a-list)
  example for a starting point, and `lv_example_flex_list_reorder` for reordering
  rows with <ApiLink name="lv_obj_move_to_index"/>.

  | Removed | Build it as |
  |---|---|
  | `lv_list_create` | an `lv_obj` with `LV_FLEX_FLOW_COLUMN` |
  | `lv_list_add_button` | an `lv_button` with `lv_pct(100)` width holding an icon and a label |
  | `lv_list_add_text` | a full-width `lv_label` |
  | `lv_list_get_button_text` / `lv_list_set_button_text` | read/write the button's `lv_label` child |
  | `lv_list_add_translation_tag`, `lv_list_add_button_translation_tag`, `lv_list_set_button_translation_tag` | <ApiLink name="lv_label_set_text"/> with a tag resolved through the translation API |

  The v8 aliases `lv_list_add_btn`, `lv_list_set_btn_text` and `lv_list_get_btn_text`
  are gone with them.

### lv_win

- The `lv_win` widget has been removed, along with `LV_USE_WIN`. A window is just a
  flex column with a header bar and a content area, so build one directly from
  `lv_obj` instead. See the
  [`lv_example_flex_win`](common-widget-features/layouts/flex#building-a-window)
  example for a starting point.

  | Removed | Build it as |
  |---|---|
  | `lv_win_create` | an `lv_obj` with `LV_FLEX_FLOW_COLUMN` holding a header and a content child |
  | `lv_win_get_header` | `lv_obj_get_child(win, 0)` |
  | `lv_win_get_content` | `lv_obj_get_child(win, 1)` |
  | `lv_win_add_title` | an `lv_label` in the header with `lv_obj_set_flex_grow(title, 1)` |
  | `lv_win_add_button` | an `lv_button` in the header with a fixed width |
...
- `lv_image_buf_set_palette()` has been removed, use <ApiLink name="lv_draw_buf_set_palette"/> instead.
- `lv_image_buf_free()` has been removed, use <ApiLink name="lv_draw_buf_destroy"/> instead.

### lv_snapshot

- `lv_snapshot_free()` has been removed, use <ApiLink name="lv_draw_buf_destroy"/> directly instead.
- `lv_snapshot_take_to_buf()` has been removed, use <ApiLink name="lv_snapshot_take_to_draw_buf"/> instead:
...
`lv_layout_register` has been removed. Use <ApiLink name="lv_layout_create"/>, which
takes a callback struct so a layout can also report its minimum size.
...
- The `lv_file_explorer` widget has been removed. A file explorer is a path header
...
- The `lv_fragment` module has been removed.
...
- `lv_gltf_set_env_brightness()` and `lv_gltf_get_env_brightness()` have been
  removed. Use <ApiLink name="lv_gltf_set_environment_brightness"/> and
  <ApiLink name="lv_gltf_get_environment_brightness"/>, which take and return a
  multiplier (`1.0` = neutral) instead of a percentage.
  ...
  /* After */
  lv_gltf_set_environment_brightness(gltf, 1.5f);
  ```

### rlottie

- The rlottie player has been removed, along with `LV_USE_RLOTTIE` and the
  `env_support/esp/rlottie` build glue. Use the
  [`lv_lottie`](widgets/lottie) widget, which renders through ThorVG and needs
  no external library.

  | Removed | Replacement |
  |---|---|
  | `lv_rlottie_create_from_file` | <ApiLink name="lv_lottie_create"/> + <ApiLink name="lv_lottie_set_src_file"/> |
  | `lv_rlottie_create_from_raw` | <ApiLink name="lv_lottie_create"/> + <ApiLink name="lv_lottie_set_src_data"/> |
  | `lv_rlottie_set_play_mode` | <ApiLink name="lv_lottie_get_anim"/> and the `lv_anim` API |
  | `lv_rlottie_set_current_frame` | <ApiLink name="lv_lottie_get_anim"/> and the `lv_anim` API |
