...
`lv_display_t`, `_init()` takes the callbacks and talks to the hardware, and you set the
...
lv_st7789_init(disp, LV_LCD_FLAG_NONE, my_send_cmd, my_send_color);
```
...
Every `_init()` returns `lv_result_t`, and the display has no flush callback until it
succeeds, so a failed or forgotten init leaves the screen blank.

- `lv_ft81x_create()` lost its `spi_cb` and `user_data` parameters and no longer deletes the
  display on failure; pass the callback to `lv_ft81x_init()`.
- `lv_draw_eve_display_create()` lost its `op_cb` and `user_data` parameters; pass the
  callback to `lv_draw_eve_display_init()`.
