...
### Display driver creation is split from initialization

`_create()` used to send the first commands to the hardware, so the `send_cmd` /
`send_color` callbacks ran before you had any chance to give them a user data pointer to
work with. Creation and initialization are now separate. `_create()` only builds the
`lv_display_t`, the callbacks are installed with their own setters, and `_init()` talks to
the hardware.

```c
/* Before */
lv_display_t * disp = lv_st7789_create(240, 320, LV_LCD_FLAG_NONE, my_send_cmd, my_send_color);

/* After */
lv_display_t * disp = lv_st7789_create(240, 320);
lv_display_set_user_data(disp, &my_context);   /* the callbacks can read this now */
lv_lcd_generic_mipi_set_send_cmd_cb(disp, my_send_cmd);
lv_lcd_generic_mipi_set_send_color_cb(disp, my_send_color);
lv_st7789_init(disp, LV_LCD_FLAG_NONE);
```

This applies to the generic MIPI driver and the controllers built on it (`lv_st7735`,
`lv_st7789`, `lv_st7796`, `lv_ili9341`, `lv_nv3007`), plus `lv_ft81x` and the EVE display.
Every `_init()` returns `lv_result_t` and fails, with an error log, if a required callback
was not set. The display has no flush callback until init succeeds, so a failed or
forgotten init leaves the screen blank.

The MIPI callback setters are on the generic driver, so they are shared by all five
controllers rather than duplicated per driver.

- `lv_ft81x_create()` lost its `spi_cb` and `user_data` parameters and no longer deletes the
  display on failure; set the callback with `lv_ft81x_set_spi_cb()`.
- `lv_draw_eve_display_create()` lost its `op_cb` and `user_data` parameters; set the
  callback with `lv_draw_eve_display_set_operation_cb()`.
- `lv_ft81x_get_user_data()` and `lv_draw_eve_display_get_user_data()` are removed, use
  <ApiLink name="lv_display_get_user_data"/>.
- The NXP eLCDIF driver used the display user data for its `LCDIF_Type *`; it now uses the
  driver data, so <ApiLink name="lv_display_set_user_data"/> is free there too.
