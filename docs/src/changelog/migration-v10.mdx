...
- Data setting and bitmap generation are now separate steps, and the data setters
  changed signature. `lv_qrcode_set_data()` keeps taking a string (source
  compatible), and a new `lv_qrcode_set_data_binary()` takes a buffer and length.
  `lv_qrcode_update()` no longer takes the data; it regenerates the bitmap from the
  stored data (and is a no-op when nothing changed). Use `lv_qrcode_force_update()`
  ...
  /* After */
  lv_qrcode_set_data_binary(qr, data, data_len);
  /* set_data_binary auto-regenerates by default. If you disabled auto updating
   * with lv_qrcode_set_auto_update(qr, false), also call: */
  lv_qrcode_update(qr);
  ```

- Setting the data, size, or quiet zone now regenerates the bitmap regardless of
  order (previously changing size/quiet zone after the data had no effect). Use
  `lv_qrcode_set_auto_update(qr, false)` + `lv_qrcode_update()` to batch changes
  into a single regeneration.

- Caveat for binary payloads: strings and binary data share one buffer and are
  told apart by a heuristic (a string ends with a NUL and contains no other NUL).
  A binary payload whose only NUL is its final byte is treated as a string, so
  that trailing NUL is not encoded. If you encode binary data that can end in a
  single NUL, append your own sentinel or otherwise avoid a lone trailing NUL.
