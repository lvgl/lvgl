...
- If you use an RTOS:
  ...
  * Increase the stack size of the task that calls <ApiLink name="lv_timer_handler" />.
...
Be sure you are calling <ApiLink name="lv_tick_inc" display="lv_tick_inc(x)" /> as prescribed in
[Tick Interface](/integration/overview) and are calling <ApiLink name="lv_timer_handler" /> as prescribed in
...
Be sure you are calling <ApiLink name="lv_display_flush_ready" display="lv_display_flush_ready(drv)" /> at the end of your
