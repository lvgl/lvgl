...
- `dump obj [-L <depth>] [<lv_obj_t *>]`: Dump the widget tree, from the given
...
- `dump widget props [<class>|<lv_obj_t *>]`: List the fields a widget has, by
  class name (`lv_keyboard`) or from a live widget. With no argument, lists the
  widget classes.
- `info widget [<lv_obj_t *>] [<field>...]`: Inspect one widget, or list every
  widget. Field names limit the output to those fields.
...
`dump obj <lv_obj_t *>`: Dump the tree starting from that widget, e.g.
`dump obj my_screen`, `dump obj lv_global->disp_default->act_scr`, or
`dump obj 0x507000025da0`.
...
Wherever these commands take a widget, any C expression GDB can evaluate to an
`lv_obj_t *` will do: a variable (`my_label`), a member chain
(`lv_global->disp_default->act_scr`), or a plain address (`0x50e000000820`).
