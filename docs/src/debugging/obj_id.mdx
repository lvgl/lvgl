...
- <ApiLink name="lv_obj_get_id" display="lv_obj_get_id(widget)" /> becomes available;
...
IDs are only used to print the Widget Tree. To look a Widget up, give it a name
with <ApiLink name="lv_obj_set_name" display="lv_obj_set_name(widget, &quot;my_widget&quot;)" />
and find it with
<ApiLink name="lv_obj_find_by_name" display="lv_obj_find_by_name(parent, &quot;my_widget&quot;)" />,
which walks `parent`'s children recursively.
