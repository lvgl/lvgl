...
description: "A Hardware Button is an external button typically located next to the screen and assigned to specific coordinates on the screen. If a button is pressed, it simulates a press at the assigned coordin..."
---
...
<ApiLink name="lv_indev_set_button_points" display="lv_indev_set_button_points(my_indev, points_array)" />.
...
`data->state` should also be set to either <ApiLink name="LV_INDEV_STATE_PRESSED" /> or
<ApiLink name="LV_INDEV_STATE_RELEASED" />.
