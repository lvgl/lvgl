---
title: Encoder
description: "An encoder is a device with a pressable turning knob that tells the hosting device:"
---
...
By using [Groups](/main-modules/indev/groups), even complex UIs can be navigated by a single
encoder.
...
[group](/main-modules/indev/groups).
...
it with an encoder or keypad, it enters the <ApiLink name="LV_STATE_FOCUSED" /> state.

If a widget switches to edit mode, it enters the
<ApiLink name="LV_STATE_FOCUSED" display="LV_STATE_FOCUSED | LV_STATE_EDITED" /> states, so any style properties
...
- <ApiLink name="LV_KEY_ENTER" />: simulates pressing the encoder button.
- <ApiLink name="LV_KEY_LEFT" />: simulates turning the encoder left.
- <ApiLink name="LV_KEY_RIGHT" />: simulates turning the encoder right.
