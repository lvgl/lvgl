---
title: Gestures
description: "Pointer input devices can detect simple (up, down, left, right) and multi-touch (swipe, pinch, scroll) gestures."
---
...
Screen widget in the form of an <ApiLink name="LV_EVENT_GESTURE" /> event. For example:

To prevent passing the gesture event to the parent from a widget, use
<ApiLink name="lv_obj_remove_flag" display="lv_obj_remove_flag(widget, LV_OBJ_FLAG_GESTURE_BUBBLE)" />.

Note that gestures are not triggered if a widget is being scrolled.

If you performed some action on a gesture, you can call
<ApiLink name="lv_indev_wait_release" display="lv_indev_wait_release(lv_indev_active())" /> in the event handler to prevent
...
These limits can be set via <ApiLink name="lv_indev_set_gesture_min_velocity" /> and
<ApiLink name="lv_indev_set_gesture_min_distance" /> accordingly.
...
To enable multi-touch gesture recognition, set the <ApiLink name="LV_USE_GESTURE_RECOGNITION" />
option in the `lv_conf.h` file.

### Usage

The recognizers can be updated to recognize gestures by calling
<ApiLink name="lv_indev_gesture_recognizers_update" display="lv_indev_gesture_recognizers_update(indev, touches, touch_cnt)" />. This must
...
<ApiLink name="lv_indev_gesture_recognizers_set_data" display="lv_indev_gesture_recognizers_set_data(indev, data)" /> will update the
<ApiLink name="lv_indev_data_t" /> structure. This should also be done in the indev
`read_cb`.
...
- <ApiLink name="LV_INDEV_GESTURE_STATE_RECOGNIZED" />: The gesture has been recognized and is now
  active.
- <ApiLink name="LV_INDEV_GESTURE_STATE_ENDED" />: The gesture has ended.

### Events

Once a gesture is recognized or ended, a <ApiLink name="LV_EVENT_GESTURE" /> is sent. The user can
use these functions to gather more information about the gesture:

- <ApiLink name="lv_event_get_gesture_type" display="lv_event_get_gesture_type(lv_event_t * e)" />: Get the type of the gesture.
  Use this to check which multi-touch gesture is currently reported.
- <ApiLink name="lv_event_get_gesture_state" display="lv_event_get_gesture_state(lv_event_t * e, lv_indev_gesture_type_t type)" />:
  Get the state of the gesture as <ApiLink name="lv_indev_gesture_state_t" />. It can be one
  of:
  
  - <ApiLink name="LV_INDEV_GESTURE_STATE_NONE" />: The gesture is not active.
  - <ApiLink name="LV_INDEV_GESTURE_STATE_RECOGNIZED" />: The gesture is recognized and
    can be used.
  - <ApiLink name="LV_INDEV_GESTURE_STATE_ENDED" />: The gesture ended.

The user can then request the gesture values with the following functions:

- <ApiLink name="lv_event_get_pinch_scale" display="lv_event_get_pinch_scale(lv_event_t * e)" />: Get the pinch scale. Only
  relevant for pinch gestures.
- <ApiLink name="lv_event_get_rotation" display="lv_event_get_rotation(lv_event_t * e)" />: Get the rotation in radians. Only
  relevant for rotation gestures.
- <ApiLink name="lv_event_get_two_fingers_swipe_distance" display="lv_event_get_two_fingers_swipe_distance(lv_event_t * e)" />: Get the
  distance in pixels from the gesture starting center. Only relevant for two-finger
  swipe gestures.
- <ApiLink name="lv_event_get_two_fingers_swipe_dir" display="lv_event_get_two_fingers_swipe_dir(lv_event_t * e)" />: Get the direction
...
- <ApiLink name="lv_indev_set_pinch_up_threshold" display="lv_indev_set_pinch_up_threshold(lv_indev_t * indev, float threshold)" />:
  Set the pinch-up (zoom in) threshold in pixels.
- <ApiLink name="lv_indev_set_pinch_down_threshold" display="lv_indev_set_pinch_down_threshold(lv_indev_t * indev, float threshold)" />:
  Set the pinch-down (zoom out) threshold in pixels.
- <ApiLink name="lv_indev_set_rotation_rad_threshold" display="lv_indev_set_rotation_rad_threshold(lv_indev_t * indev, float threshold)" />:
