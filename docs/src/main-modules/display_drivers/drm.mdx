...
Compared with the [framebuffer driver](/main-modules/display_drivers/fbdev), DRM gives you proper
...
| Color formats | RGB565 or XRGB8888, from <ApiLink name="LV_COLOR_DEPTH" /> | XRGB8888, from <ApiLink name="LV_COLOR_DEPTH" /> | RGB565 or ARGB8888, from <ApiLink name="LV_COLOR_DEPTH" /> |
...
| Input | None built in. Pair with [evdev](/main-modules/input_drivers/evdev) or [libinput](/main-modules/input_drivers/libinput) | Same | Same |
...
- [EGL](/main-modules/display_drivers/egl): the EGL layer underneath the EGL backend
- [OpenGL Overview](/integration/embedded_linux/opengl): GPU draw units usable with the EGL backend
- [Linux Framebuffer](/main-modules/display_drivers/fbdev): simpler alternative when only `/dev/fb` exists
- [evdev](/main-modules/input_drivers/evdev): input to pair with this driver
