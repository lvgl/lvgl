...
After a successful upload to VRAM, an LVGL-owned CPU-side allocation is freed to
minimize RAM usage. Caller-owned storage is retained; for mutable buffers,
ensure CPU residency before accessing it so that any VRAM changes are downloaded
and the VRAM backing is released. Read-only images can retain both copies.
Conversely, after a download to CPU memory, the VRAM
