...
- <ApiLink name="LV_EVENT_PARTIAL_SYNC_WAIT_START" />: Sent at the beginning of internal call to
  `wait_for_partial_syncing()` -- happens whether or not any waiting will actually occur.
  Call returns immediately if `disp->partial_syncing == 0`.
- <ApiLink name="LV_EVENT_PARTIAL_SYNC_WAIT_FINISH" />: Sent when the call to `wait_for_partial_syncing()`
  is about to return, regardless whether any actual waiting occurred.
