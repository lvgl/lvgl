---
title: BDF Font
description: "Small displays with low resolution don't look pretty with automatically rendered fonts. A bitmap font provides the solution, but it's necessary to convert the bitmap font (BDF) to a TTF."
---
...
the bitmap information. The bitmap itself will be embedded into the TTF as well. `lv_font_conv](https://github.com/lvgl/lv_font_conv/) uses
...
But newer versions don't. So you might want to change 2 lines in `potrace-wrapper.sh`. Open `potrace-wrapper.sh`
