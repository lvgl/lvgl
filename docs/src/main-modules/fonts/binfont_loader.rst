...
Dynamic loading is thread-safe, so no extra actions are required in the application.  
However, if you are writing a rendering engine for LVGL, it is important to know:

- A mutex protects access to the shared file handle and glyph buffer.  
- Only one "raw bitmap" can be held at a time per font.  
