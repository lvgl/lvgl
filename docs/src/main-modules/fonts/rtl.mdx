...
description: "Most languages use a Left-to-Right (LTR for short) writing direction, however some languages (such as Hebrew, Persian or Arabic) use Right-to-Left (RTL for short) direction."
---
...
![Bidi](/_static/images/bidi.png)

BiDi support is enabled by setting <ApiLink name="LV_USE_BIDI" /> to a non-zero value in `lv_conf.h`.
...
<ApiLink name="LV_BIDI_BASE_DIR_DEF" /> in `lv_conf.h` and other Widgets inherit the
base direction from their parent.

To set a Widget's base direction use <ApiLink name="lv_obj_set_style_base_dir" display="lv_obj_set_style_base_dir(widget, base_dir, selector)" />.
The possible base directions are:

- <ApiLink name="LV_BASE_DIR_LTR" />: Left to Right base direction
- <ApiLink name="LV_BASE_DIR_RTL" />: Right to Left base direction
- <ApiLink name="LV_BASE_DIR_AUTO" />: Auto detect base direction
...
LVGL supports these rules if <ApiLink name="LV_USE_ARABIC_PERSIAN_CHARS" /> is enabled
in `lv_conf.h`.
...
- Static text (i.e. const) is not processed. E.g. text set by <ApiLink name="lv_label_set_text" />
  will be "Arabic processed" but <ApiLink name="lv_label_set_text_static" /> will not.
- Text get functions (e.g. <ApiLink name="lv_label_get_text" />) will return the processed text.
