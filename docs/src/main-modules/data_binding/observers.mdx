...
(or any other pointer with
<ApiLink name="lv_subject_add_observer_with_target" display="lv_subject_add_observer_with_target()" />),
then read it back in the callback with
<ApiLink name="lv_observer_get_target_obj" display="lv_observer_get_target_obj()" />:
...
An Observer bound to a Widget is removed automatically when that Widget is deleted,
so most of the time you don't need to do anything. Otherwise:
...
non-zero), and clicking the Widget writes `1` or `0` back to the Subject.
