...
an array of recognizers, one per gesture type. These recognizers can be initialized by calling
``lv_indev_gesture_init_recognizers`` after the indev device is created. The the recognizers can be configured to
