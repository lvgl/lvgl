...
def _process_end_of_eligible_doc(b: str) -> (str, str, int):
    """
    ...
    7.  If `_re_api_startswith` present:
        ...
        - compute list of hyperlinks from symbols
        - add hyperlinks to `genned_link_set` if not in `editor_link_set`.
    8.  If len(editor_link_set) > 0 or len(genned_link_set) > 0:
        - `C` = _auto_gen_sep + '\n\n' + `genned_link_set`
          (with a blank line between each).
    9.  Return tuple: (new_B, C, links_added_count).
        ...
        for symbol in symbols:
            for symbol_dict in _symbol_dict_list:
                if symbol in symbol_dict:
                    _conditionally_add_hyperlink(symbol_dict[symbol], genned_link_set, editor_link_set)
                ...
                for key in symbol_dict:
                    if key is None:
                        ...
                        if key.startswith(symbol):
                            _conditionally_add_hyperlink(symbol_dict[key], genned_link_set, editor_link_set)

    # 8.  If len(genned_link_set) > 0:
    #     - `C` = _auto_gen_sep + '\n\n' + `genned_link_set`
    #       (with a blank line between each).
    if len(genned_link_set) > 0:
        links_added_count = len(genned_link_set)
        c = '\n' + _auto_gen_sep + '\n\n'
        for link_name in genned_link_set:
            c += ':ref:`' + link_name + '`\n\n'
...
def _process_one_file(rst_file: str):
    ...
    1.  Doc editors may have already added a set of hyperlinks of their
        own.  This routine takes note of and does not duplicate what is
        already there.

    2.  Doc editors may also have added specifications for this routine
        that look like this:
    ...
    3.  Lacking any of the above custom directives, use the lower-case
    ...
    3-9.  new_B, C, links_added_count = _process_end_of_eligible_doc()
    10. Write `A` + `new_B` + `C` back to `rst_file`.
    11. Return links_added_count.

    :param rst_file:  Full path to `.rst` file in question.
                        It may or may not be eligible.
    :return:          Number of links added.

    TODO:  sort out exact string match attempts not finding
           intended target data because of a preceding '_'.
           Example:  'lv_obj_t' is linking to 'lv_types_h',
           but not to 'lv_obj_h' or 'lv_obj_private_h'.

    TODO:  Sort out what to do with 'lv_types_h' links -- seems
           inappropriate.
    """
    links_added_count = 0
    ...
    if match is not None:
        ...
        a = rst_contents[:i]
        b = rst_contents[i:]
        new_b, c, links_added_count = _process_end_of_eligible_doc(b)
    ...
    if test:
        ...
        for rel_path in doc_rel_paths:
            ...
            for dir_bep, sub_dirs, files in os.walk(top_dir, topdown=False):
                for lsFile in files:
                    if lsFile.endswith('.rst'):
                        file_list.append(os.path.join(dir_bep, lsFile))
        ...
        announce(__file__, f'Eligible docs found: {total_eligible_doc_count:>4}')
        announce(__file__, f'API links added    : {total_links_added_count:>4}')
    else:
