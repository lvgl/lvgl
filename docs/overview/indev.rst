...
:ref:`Text area <widgets/textarea/text area (lv_textarea)>` is focused and you press some letter
on a keyboard, the keys will be sent and inserted into the text area.
Similarly, if a :ref:`Slider <widgets/slider/slider (lv_slider)>` is focused and you press
...
- :cpp:enumerator:`LV_KEY_ESC`: Close or exit (E.g. close a :ref:`Drop down list <widgets/dropdown/drop-down list (lv_dropdown)>`)
- :cpp:enumerator:`LV_KEY_DEL`: Delete (E.g. a character on the right in a :ref:`Text area <widgets/textarea/text area (lv_textarea)>`)
- :cpp:enumerator:`LV_KEY_BACKSPACE`: Delete a character on the left (E.g. in a :ref:`Text area <widgets/textarea/text area (lv_textarea)>`)
- :cpp:enumerator:`LV_KEY_HOME`: Go to the beginning/top (E.g. in a :ref:`Text area <widgets/textarea/text area (lv_textarea)>`)
- :cpp:enumerator:`LV_KEY_END`: Go to the end (E.g. in a :ref:`Text area <widgets/textarea/text area (lv_textarea)>`)
...
which cannot be pressed (like a :ref:`Slider <widgets/slider/slider (lv_slider)>`) leaves
*Edit* mode upon a short click. But with objects where a short click has
meaning (e.g. :ref:`Button <widgets/button/button (lv_button)>`), a long press is required.
