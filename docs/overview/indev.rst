...
To create a group use ``lv_group_t * g =`` :cpp:func:`lv_group_create()` and to add
...
``lv_group_t * g =`` :cpp:func:` lv_group_create()` and set the default group with
...
:cpp:enumerator:`LV_STATE_FOCUSED` ``|`` :cpp:enumerator:`LV_STATE_EDITED` states so these style properties
will be shown.
