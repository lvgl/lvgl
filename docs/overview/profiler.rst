...
.. code:: c
    ...
    {
        LV_PROFILER_BEGINEX("do_something_1");
        do_something_1();
        LV_PROFILER_ENDEX("do_something_1");

        LV_PROFILER_BEGINEX("do_something_2");
        do_something_2();
        LV_PROFILER_ENDEX("do_something_2");
    }
...
- :c:macro:`LV_PROFILER_BEGINEX`: Profiler start point function with custom string.
- :c:macro:`LV_PROFILER_ENDEX`: Profiler end point function with custom string.
...
.. code:: c
    ...
    #define LV_PROFILER_BEGINEX(str) sched_note_beginex(NOTE_TAG_ALWAYS, str)
    #define LV_PROFILER_ENDEX(str)   sched_note_endex(NOTE_TAG_ALWAYS, str)
...
4. Make sure that the string passed in by c:macro:`LV_PROFILER_BEGINEX/ENDEX` is not a local variable on the stack or a string in shared memory, because currently only the string address is recorded and the content is not copied.
