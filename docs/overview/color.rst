...
The type :c:struct:`lv_color_t` is used to store a color. Its fields are set
...
:c:expr:`lv_color_white()` and :c:expr:`lv_color_black()` return ``0xFFFFFF`` and
``0x000000`` respectively.

Opacity
*******

To describe opacity the :c:type:`lv_opa_t` type is created from ``uint8_t``.
Some special purpose defines are also introduced:

-  :c:enumerator:`LV_OPA_TRANSP` Value: 0, means no opacity making the color
   completely transparent
-  :c:enumerator:`LV_OPA_10` Value: 25, means the color covers only a little
-  ``LV_OPA_20 ... OPA_80`` follow logically
-  :c:enumerator:`LV_OPA_90` Value: 229, means the color near completely covers
-  :c:enumerator:`LV_OPA_COVER` Value: 255, means the color completely covers (full
   opacity)

You can also use the ``LV_OPA_*`` defines in :c:expr:`lv_color_mix()` as a
mixing *ratio*.
...
-  :c:struct:`lv_color1_t` Monochrome color. Also has R, G, B fields for
   compatibility but they are always the same value (1 byte)
-  :c:struct:`lv_color8_t` A structure to store R (3 bit),G (3 bit),B (2 bit)
   components for 8-bit colors (1 byte)
-  :c:struct:`lv_color16_t` A structure to store R (5 bit),G (6 bit),B (5 bit)
   components for 16-bit colors (2 byte)
-  :c:struct:`lv_color32_t` A structure to store R (8 bit),G (8 bit), B (8 bit)
   components for 24-bit colors (4 byte)
-  :c:struct:`lv_color_t` Equal to ``lv_color1/8/16/24_t`` depending on the
   configured color depth setting
-  :c:type:`lv_color_int_t` ``uint8_t``, ``uint16_t`` or ``uint32_t``
   depending on the color depth setting. Used to build color arrays from
   plain numbers.
-  :c:type:`lv_opa_t` A simple ``uint8_t`` type to describe opacity.

The :c:struct:`lv_color_t`, :c:struct:`lv_color1_t`, :c:struct:`lv_color8_t`, :c:struct:`lv_color16_t`
and :c:struct:`lv_color32_t` types have four fields:

-  :c:var:`ch.red` red channel
-  :c:var:`ch.green` green channel
-  :c:var:`ch.blue` blue channel
-  :c:var:`full` red + green + blue as one number
...
.. raw:: html
    ...
    <div include-html="misc\lv_color.html"></div>
