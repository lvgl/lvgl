...
``LV_PART_MAIN`` The main part. Uses all the typical background
properties - ``LV_PART_ITEMS`` The buttons. Also uses all typical
background properties as well as the *text* properties.
...
``LV_KEYBOARD_MODE_USER_1`` through ``LV_KEYBOARD_MODE_USER_4``
User-defined modes.
...
use ``lv_keyboard_set_textarea(kb, ta)``.
...
the ``LV_BTNMATRIX_CTRL_POPOVER`` flag for all keys that you want to
show a popover.
...
with the original map: - ``LV_SYMBOL_OK`` Send ``LV_EVENT_RADY`` to the
assigend Text area. - ``LV_SYMBOL_CLOSE`` or ``LV_SYMBOL_KEYBOARD`` Send
``LV_EVENT_CANCEL`` to the assigend Text area. - ``LV_SYMBOL_BACKSPACE``
Delete on the left. - ``LV_SYMBOL_LEFT`` Move the cursor left. -
``LV_SYMBOL_RIGHT`` Move the cursor right. - ``LV_SYMBOL_NEW_LINE`` New
line. - *“ABC”* Load the uppercase map. - *“abc”* Load the lower case
map. - *“1#”* Load the lower case map.

Events
******

-  ``LV_EVENT_VALUE_CHANGED`` Sent when the button is pressed/released
