...
-  ``LV_PART_MAIN`` The is the background of the Checkbox and it uses
   the text and all the typical background style properties.
   ``pad_column`` adjusts the spacing between the tickbox and the label
-  ``LV_PART_INDICATOR`` The “tick box” is a square that uses all the
...
``lv_checkbox_set_text(cb, "New text")`` function and will be
...
.. raw:: html
    ...
    <div include-html="widgets\checkbox\lv_checkbox.html"></div>
