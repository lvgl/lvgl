...
-  ``LV_PART_MAIN`` The background of the button. Uses the typical
   background properties and text properties for the text on it.
-  ``LV_PART_INDICATOR`` Typically an arrow symbol that can be an image
   or a text (``LV_SYMBOL``).
...
-  ``LV_PART_MAIN`` The list itself. Uses the typical background
   properties. ``max_height`` can be used to limit the height of the
   list.
-  ``LV_PART_SCROLLBAR`` The scrollbar background, border, shadow
   properties and width (for its own width) and right padding for the
   spacing on the right.
-  ``LV_PART_SELECTED`` Refers to the currently pressed, checked or
...
``lv_dropdown_get_list(dropdown)`` to get the list object. For example:
...
events <../overview/event.html#generic-events>`__, the following
`Special events <../overview/event.html#special-events>`__ are sent by
the drop-down list: - ``LV_EVENT_VALUE_CHANGED`` Sent when the new
option is selected or the list is opened/closed. - ``LV_EVENT_CANCEL``
Sent when the list is closed - ``LV_EVENT_READY`` Sent when the list is
opened
...
-  ``LY_KEY_ENTER`` Apply the selected option (Sends
   ``LV_EVENT_VALUE_CHANGED`` event and closes the drop-down list).
