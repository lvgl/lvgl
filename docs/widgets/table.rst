...
-  ``LV_PART_MAIN`` The background of the table uses all the typical
   background style properties.
-  ``LV_PART_ITEMS`` The cells of the table also use all the typical
...
``lv_table_set_cell_value(table, row, col, "Content")``. The text is
...
``lv_table_set_row_cnt(table, row_cnt)`` and
...
``lv_table_set_col_width(table, col_id, width)``. The overall width of
...
If the label’s width or height is set to ``LV_SIZE_CONTENT`` that size
...
-  ``LV_EVENT_VALUE_CHANGED`` Sent when a new cell is selected with
   keys.
...
``LV_TABLE_CELL_NONE`` no cell is selected.
...
.. raw:: html
    ...
    <div include-html="widgets\table\lv_table.html"></div>
