...
-  ``LV_PART_MAIN`` The background of the switch uses all the typical
...
-  ``LV_PART_KNOB`` A rectangle (or circle) drawn at left or right side
...
``lv_obj_has_state(switch, LV_STATE_CHECKED)``.

Call ``lv_obj_add_state(switch, LV_STATE_CHECKED)`` to turn it on, or
``lv_obj_clear_state(switch, LV_STATE_CHECKED)`` to turn it off.
...
.. raw:: html
    ...
    <div include-html="widgets\switch\lv_switch.html"></div>
