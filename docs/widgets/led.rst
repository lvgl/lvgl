...
The LEDs have only one main part, called ``LV_LED_PART_MAIN`` and it
...
``lv_led_set_color(led, lv_color_hex(0xff0080))``. This will be used as
...
-  ``LV_EVENT_DRAW_PART_BEGIN`` and ``LV_EVENT_DRAW_PART_END`` is sent
   ...
   -  ``LV_LED_DRAW_PART_RECTANGLE`` The main rectangle.
      ``LV_OBJ_DRAW_PART_RECTANGLE`` is not sent by the base object.
...
.. raw:: html
    ...
    <div include-html="widgets\led\lv_led.html"></div>
