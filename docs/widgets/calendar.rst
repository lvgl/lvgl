...
``LV_PART_MAIN`` The background of the calendar. Uses all the background
related style properties. - ``LV_PART_ITEMS`` Refers to the dates and
day names. Button matrix control flags are set to differentiate the
buttons and a custom drawer event is added modify the properties of the
buttons as follows: - day names have no border, no background and drawn
with a gray color - days of the previous and next month have
``LV_BTNMATRIX_CTRL_DISABLED`` flag - today has a thicker border with
the theme’s primary color - highlighted days have some opacity with the
theme’s primary color.

Usage
*****

Some functions use the ``lv_calendar_date_t`` type which is a structure
...
``lv_calendar_date_t`` array loaded by
...
-  ``LV_EVENT_VALUE_CHANGED`` Sent if a date is clicked.
   ``lv_calendar_get_pressed_date(calendar, &date)`` set ``date`` to the
   date currently being pressed. Returns ``LV_RES_OK`` if there is a
   valid pressed date, else ``LV_RES_INV``.
...
``lv_calendar_header_arrow_create(calendar)`` creates a header that
...
``lv_calendar_header_dropdown_create(calendar)`` creates a header that
