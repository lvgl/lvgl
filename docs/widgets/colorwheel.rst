...
-  ``LV_PART_MAIN`` Only ``arc_width`` is used to set the width of the
   color wheel
-  ``LV_PART_KNOB`` A rectangle (or circle) drawn on the current value.
...
``lv_colorwheel_set_mode(colorwheel, LV_COLORWHEEL_MODE_HUE/SATURATION/VALUE)``.
...
-  ``LV_KEY_UP``, ``LV_KEY_RIGHT`` Increment the current parameter’s
   value by 1
-  ``LV_KEY_DOWN``, ``LV_KEY_LEFT`` Decrement the current parameter’s
   value by 1
-  ``LV_KEY_ENTER`` A long press will show the next mode. Double click
...
.. raw:: html
    ...
    <div include-html="widgets\colorwheel\lv_colorwheel.html"></div>
