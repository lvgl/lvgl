...
void test_math_cubic_bezier_boundary_values(void)
{
    ...
    TEST_ASSERT(result1 >= 0 && result1 <= 1024);
...
void test_math_sqrt_fractional(void)
{
    ...
    TEST_ASSERT(result.f > 0); /* Should have fractional part */

    /* Test 100 - should give 10 */
    lv_sqrt(100, &result, 0x8000);
    TEST_ASSERT_EQUAL(10, result.i);
}
...
void test_math_sqrt_with_different_masks(void)
{
    ...
    TEST_ASSERT(result.i == 15 || result.i == 16);  /* Allow small variation */

    lv_sqrt(65536, &result, 0x800); /* Root < 256 */
    TEST_ASSERT(result.i >= 255 && result.i <= 256);

    lv_sqrt(1048576, &result, 0x8000); /* Root >= 256 */
    TEST_ASSERT(result.i >= 1023 && result.i <= 1024);
}
...
{
    /* Test basic values */
    TEST_ASSERT_EQUAL(0, lv_sqrt32(0));
    TEST_ASSERT_EQUAL(1, lv_sqrt32(1));
    TEST_ASSERT_EQUAL(2, lv_sqrt32(4));
    TEST_ASSERT_EQUAL(3, lv_sqrt32(9));
    TEST_ASSERT_EQUAL(4, lv_sqrt32(16));
    TEST_ASSERT_EQUAL(10, lv_sqrt32(100));
    TEST_ASSERT_EQUAL(256, lv_sqrt32(65536));
    TEST_ASSERT_EQUAL(1024, lv_sqrt32(1048576));
}

void test_math_sqrt32_range(void)
{
    /* Test various ranges */
    TEST_ASSERT_EQUAL(255, lv_sqrt32(65025));   /* 255^2 = 65025 */
    TEST_ASSERT_EQUAL(256, lv_sqrt32(65536));   /* 256^2 = 65536 */
    /* Avoid testing near max to prevent overflow */
    TEST_ASSERT_EQUAL(1000, lv_sqrt32(1000000)); /* 1000^2 = 1,000,000 */
}
...
void test_math_sqrt32_approximation(void)
{
    ...
    TEST_ASSERT(result == 1 || result == 2); /* Should be close */
...
void test_math_atan2_quadrants(void)
{
    ...
    TEST_ASSERT_EQUAL(90, lv_atan2(1, 0));      /* Positive X axis (x=1, y=0) */
    TEST_ASSERT_EQUAL(0, lv_atan2(0, 1));       /* Positive Y axis (x=0, y=1) */
    TEST_ASSERT_EQUAL(270, lv_atan2(-1, 0));    /* Negative X axis (x=-1, y=0) */
    TEST_ASSERT_EQUAL(180, lv_atan2(0, -1));    /* Negative Y axis (x=0, y=-1) */
}

void test_math_atan2_diagonal(void)
{
    /* Test diagonal angles - lv_atan2(x, y) */
    TEST_ASSERT_EQUAL(45, lv_atan2(100, 100));    /* Q1: (100, 100) */
    TEST_ASSERT_EQUAL(135, lv_atan2(100, -100));  /* Q2: (100, -100) */
    TEST_ASSERT_EQUAL(225, lv_atan2(-100, -100)); /* Q3: (-100, -100) */
    TEST_ASSERT_EQUAL(315, lv_atan2(-100, 100));  /* Q4: (-100, 100) */
}

void test_math_atan2_edge_cases(void)
{
    /* Test edge cases - lv_atan2(x, y) */
    TEST_ASSERT_EQUAL(90, lv_atan2(1000, 0));   /* (1000, 0) = positive X axis */
    TEST_ASSERT_EQUAL(0, lv_atan2(0, 1000));    /* (0, 1000) = positive Y axis */
    TEST_ASSERT_EQUAL(270, lv_atan2(-1000, 0)); /* (-1000, 0) = negative X axis */
    TEST_ASSERT_EQUAL(180, lv_atan2(0, -1000)); /* (0, -1000) = negative Y axis */
}

void test_math_atan2_large_values(void)
{
    /* Test with large values - lv_atan2(x, y) */
    TEST_ASSERT_EQUAL(90, lv_atan2(1456, 1));    /* (1456, 1) ≈ positive X axis */
    TEST_ASSERT_EQUAL(270, lv_atan2(-1456, 1));  /* (-1456, 1) ≈ negative X axis */
    TEST_ASSERT_EQUAL(0, lv_atan2(1, 1456));     /* (1, 1456) ≈ positive Y axis */
    TEST_ASSERT_EQUAL(180, lv_atan2(1, -1456));  /* (1, -1456) ≈ negative Y axis */
}
...
{
    /* Test basic powers */
    TEST_ASSERT_EQUAL(1, lv_pow(5, 0));
    TEST_ASSERT_EQUAL(5, lv_pow(5, 1));
    TEST_ASSERT_EQUAL(25, lv_pow(5, 2));
    TEST_ASSERT_EQUAL(125, lv_pow(5, 3));
    TEST_ASSERT_EQUAL(625, lv_pow(5, 4));
}

void test_math_pow_negative_exponent(void)
{
    /* Test negative exponents - in integer math, result < 1 so returns 0 */
    TEST_ASSERT_EQUAL(0, lv_pow(5, -1));   /* 5^(-1) = 1/5 = 0.2 -> 0 */
    TEST_ASSERT_EQUAL(0, lv_pow(10, -5));  /* 10^(-5) = 1/100000 = 0.00001 -> 0 */
    TEST_ASSERT_EQUAL(0, lv_pow(2, -10));  /* 2^(-10) = 1/1024 = 0.00098 -> 0 */

    /* Special cases */
    TEST_ASSERT_EQUAL(1, lv_pow(1, -1));   /* 1^(-1) = 1 */
    TEST_ASSERT_EQUAL(1, lv_pow(1, -100)); /* 1^(-100) = 1 */
    TEST_ASSERT_EQUAL(0, lv_pow(0, -1));   /* 0^(-1) is undefined, return 0 */
}

void test_math_pow_zero_base(void)
{
    /* Test zero base */
    TEST_ASSERT_EQUAL(0, lv_pow(0, 1));
    TEST_ASSERT_EQUAL(1, lv_pow(0, 0));
}

void test_math_pow_large_values(void)
{
    /* Test moderately large values */
    TEST_ASSERT_EQUAL(1024, lv_pow(2, 10));
    TEST_ASSERT_EQUAL(1000, lv_pow(10, 3));
    TEST_ASSERT_EQUAL(256, lv_pow(4, 4));
}
...
{
    /* Test basic mapping */
    TEST_ASSERT_EQUAL(50, lv_map(50, 0, 100, 0, 100));   /* Identity */
    TEST_ASSERT_EQUAL(0, lv_map(0, 0, 100, 0, 100));
    TEST_ASSERT_EQUAL(100, lv_map(100, 0, 100, 0, 100));
}

void test_math_map_scaling(void)
{
    /* Test scaling */
    TEST_ASSERT_EQUAL(25, lv_map(50, 0, 100, 0, 50));    /* Scale down */
    TEST_ASSERT_EQUAL(100, lv_map(50, 0, 100, 0, 200));  /* Scale up */
    TEST_ASSERT_EQUAL(75, lv_map(50, 0, 100, 50, 100));  /* Shift range */
}
...
void test_math_map_negative_ranges(void)
{
    ...
    TEST_ASSERT(result1 >= 0 && result1 <= 100);
    TEST_ASSERT(result2 >= 0 && result2 <= 100);
    TEST_ASSERT(result3 >= -100 && result3 <= 0);
}

void test_math_map_reverse_range(void)
{
    /* Test reverse mapping */
    TEST_ASSERT_EQUAL(100, lv_map(0, 0, 100, 100, 0));    /* (0-0)/(100-0) * (-100) + 100 = 100 */
    TEST_ASSERT_EQUAL(0, lv_map(100, 0, 100, 100, 0));    /* (100-0)/(100-0) * (-100) + 100 = 0 */
    TEST_ASSERT_EQUAL(50, lv_map(50, 0, 100, 100, 0));    /* (50-0)/(100-0) * (-100) + 100 = 50 */
}
...
void test_math_map_edge_case_division_by_zero(void)
{
    ...
    TEST_ASSERT_EQUAL(50, result1);
    TEST_ASSERT(result2 == 0 || result2 == 50);  /* Either is valid depending on interpretation */
}

void test_math_map_out_of_bounds(void)
{
    /* Test out of bounds input */
    TEST_ASSERT_EQUAL(100, lv_map(150, 0, 100, 0, 100)); /* Above max_in */
    TEST_ASSERT_EQUAL(0, lv_map(-50, 0, 100, 0, 100));   /* Below min_in */
}
...
void test_math_rand_set_seed(void)
{
    ...
    TEST_ASSERT_EQUAL(r1, r2); /* Same seed should give same sequence */
}
...
{
    /* Test squaring */
    TEST_ASSERT_EQUAL(0, lv_sqr(0));
    TEST_ASSERT_EQUAL(1, lv_sqr(1));
    TEST_ASSERT_EQUAL(4, lv_sqr(2));
    TEST_ASSERT_EQUAL(25, lv_sqr(5));
    TEST_ASSERT_EQUAL(100, lv_sqr(10));
    TEST_ASSERT_EQUAL(10609, lv_sqr(103)); /* 103^2 = 10609 */
}
...
void test_math_sqr_large(void)
{
    ...
    TEST_ASSERT_EQUAL(1000000, lv_sqr(1000)); /* 1000^2 = 1,000,000 */
    TEST_ASSERT_EQUAL(250000, lv_sqr(500));   /* 500^2 = 250,000 */
    TEST_ASSERT_EQUAL(10609, lv_sqr(103));    /* 103^2 = 10609 */
}
