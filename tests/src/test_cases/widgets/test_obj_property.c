...
void test_obj_property_set_get_should_match(void)
{
    ...
    /* Object flags (flag property range kept for backward compatibility) */
    ...
    prop.num = 0;
    TEST_ASSERT_TRUE(lv_obj_set_property(obj, &prop) == LV_RESULT_OK);
    TEST_ASSERT_FALSE(lv_obj_is_clickable(obj));
...
void test_obj_property_flag(void)
{
    ...
    lv_obj_t * obj = lv_obj_create(lv_screen_active());
    /*The flag property range is kept for backward compatibility; each entry round-trips
     *through lv_obj_set_property()/lv_obj_get_property().*/
