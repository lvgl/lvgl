...
void test_line_properties(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(lv_obj_get_property(obj, LV_PROPERTY_LINE_Y_INVERT).num, 1);
    TEST_ASSERT_TRUE(lv_line_get_y_invert(obj));

    prop.num = 0;
    TEST_ASSERT_TRUE(lv_obj_set_property(obj, &prop) == LV_RESULT_OK);
    TEST_ASSERT_EQUAL_INT(lv_obj_get_property(obj, LV_PROPERTY_LINE_Y_INVERT).num, 0);
