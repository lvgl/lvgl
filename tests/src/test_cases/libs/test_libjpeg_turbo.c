...
/* Test cases for decoding failures */

void test_libjpeg_turbo_invalid_file_path(void)
{
    /* Temporarily remove tjpgd decoder */
    lv_tjpgd_deinit();
    ...
    /* Verify the decoder handles invalid file path gracefully */
    TEST_ASSERT_NOT_NULL(img);
    /* The image should be in error state or not displayed */

    /* Re-add tjpgd decoder */
    lv_tjpgd_init();
}

void test_libjpeg_turbo_invalid_signature(void)
{
    /* Temporarily remove tjpgd decoder */
    lv_tjpgd_deinit();

    lv_obj_t * img = lv_image_create(lv_screen_active());
    /* Try to load a PNG file as JPEG */
    lv_image_set_src(img, "A:src/test_assets/test_img_lvgl_logo_png.c");

    lv_obj_center(img);
    lv_refr_now(NULL);

    /* Should fail due to invalid JPEG signature */
    TEST_ASSERT_NOT_NULL(img);
