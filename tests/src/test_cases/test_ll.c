...
void setUp(void)
{
    ...
    _lv_ll_init(&test_ll, NODE_SIZE);
}

void tearDown(void)
{
    /* Clean up linked list after each test */
    _lv_ll_clear(&test_ll);
}
...
{
    int32_t * node = _lv_ll_ins_tail(&test_ll);
    TEST_ASSERT_NOT_NULL(node);
    TEST_ASSERT_EQUAL_PTR(node, test_ll.head);
    TEST_ASSERT_EQUAL_PTR(node, test_ll.tail);

    /* Insert second node */
    int32_t * node2 = _lv_ll_ins_tail(&test_ll);
    TEST_ASSERT_EQUAL_PTR(node, test_ll.head);
...
{
    int32_t * head = _lv_ll_ins_head(&test_ll);
    int32_t * tail = _lv_ll_ins_tail(&test_ll);

    /* Insert before tail */
    int32_t * middle = _lv_ll_ins_prev(&test_ll, tail);
    TEST_ASSERT_EQUAL_PTR(head, test_ll.head);
    TEST_ASSERT_EQUAL_PTR(tail, test_ll.tail);
    TEST_ASSERT_EQUAL_PTR(middle, _lv_ll_get_next(&test_ll, head));
    TEST_ASSERT_EQUAL_PTR(middle, _lv_ll_get_prev(&test_ll, tail));

    int32_t * new_head = _lv_ll_ins_prev(&test_ll, head);
...
void test_ll_remove_head(void)
{
    ...
    _lv_ll_remove(&test_ll, head);
    lv_free(head);
    TEST_ASSERT_EQUAL_PTR(tail, test_ll.head);
    TEST_ASSERT_EQUAL_PTR(tail, test_ll.tail);
    TEST_ASSERT_NULL(_lv_ll_get_prev(&test_ll, tail));
}
...
void test_ll_remove_tail(void)
{
    ...
    _lv_ll_remove(&test_ll, tail);
    lv_free(tail);
    TEST_ASSERT_EQUAL_PTR(head, test_ll.head);
    TEST_ASSERT_EQUAL_PTR(head, test_ll.tail);
    TEST_ASSERT_NULL(_lv_ll_get_next(&test_ll, head));
}

void test_ll_remove_middle(void)
{
    int32_t * head = _lv_ll_ins_head(&test_ll);
    int32_t * middle = _lv_ll_ins_tail(&test_ll);
    int32_t * tail = _lv_ll_ins_tail(&test_ll);

    _lv_ll_remove(&test_ll, middle);
    lv_free(middle);
    TEST_ASSERT_EQUAL_PTR(head, test_ll.head);
    TEST_ASSERT_EQUAL_PTR(tail, test_ll.tail);
    TEST_ASSERT_EQUAL_PTR(tail, _lv_ll_get_next(&test_ll, head));
    TEST_ASSERT_EQUAL_PTR(head, _lv_ll_get_prev(&test_ll, tail));
}

void test_ll_move_before(void)
{
    int32_t * node1 = _lv_ll_ins_head(&test_ll);
    int32_t * node2 = _lv_ll_ins_tail(&test_ll);
    int32_t * node3 = _lv_ll_ins_tail(&test_ll);

    /* Move node3 before node2 */
    _lv_ll_move_before(&test_ll, node3, node2);
    TEST_ASSERT_EQUAL_PTR(node1, test_ll.head);
    TEST_ASSERT_EQUAL_PTR(node2, test_ll.tail);
    TEST_ASSERT_EQUAL_PTR(node3, _lv_ll_get_next(&test_ll, node1));
    TEST_ASSERT_EQUAL_PTR(node2, _lv_ll_get_next(&test_ll, node3));
}

void test_ll_chg_list(void)
{
    lv_ll_t dest_ll;
    _lv_ll_init(&dest_ll, NODE_SIZE);

    int32_t * node = _lv_ll_ins_head(&test_ll);
    _lv_ll_chg_list(&test_ll, &dest_ll, node, true);
    ...
    _lv_ll_clear(&dest_ll);
}

void test_ll_get_len(void)
{
    TEST_ASSERT_EQUAL(0, _lv_ll_get_len(&test_ll));

    _lv_ll_ins_head(&test_ll);
    TEST_ASSERT_EQUAL(1, _lv_ll_get_len(&test_ll));

    _lv_ll_ins_tail(&test_ll);
    TEST_ASSERT_EQUAL(2, _lv_ll_get_len(&test_ll));
}

void test_ll_is_empty(void)
{
    TEST_ASSERT_TRUE(_lv_ll_is_empty(NULL));
    TEST_ASSERT_TRUE(_lv_ll_is_empty(&test_ll));

    _lv_ll_ins_head(&test_ll);
    TEST_ASSERT_FALSE(_lv_ll_is_empty(&test_ll));
...
void test_ll_node_alignment(void)
{
    ...
    lv_ll_t small_list;
    _lv_ll_init(&small_list, 1); /* 1 byte node */
    ...
    lv_ll_t large_list;
    _lv_ll_init(&large_list, 100); /* 100 byte node */
...
void test_ll_node_pointer_operations(void)
{
    ...
    TEST_ASSERT_EQUAL_PTR(node2, _lv_ll_get_next(&test_ll, node1));
    TEST_ASSERT_EQUAL_PTR(node3, _lv_ll_get_next(&test_ll, node2));
    TEST_ASSERT_EQUAL_PTR(node1, _lv_ll_get_prev(&test_ll, node2));
    TEST_ASSERT_EQUAL_PTR(node2, _lv_ll_get_prev(&test_ll, node3));
}
...
void test_ll_move_before_edge_cases(void)
{
    ...
    _lv_ll_move_before(&test_ll, node2, node1);
    TEST_ASSERT_EQUAL_PTR(node2, test_ll.head);
    TEST_ASSERT_EQUAL_PTR(node1, _lv_ll_get_next(&test_ll, node2));

    /* Move tail before NULL (should become new tail) */
    _lv_ll_move_before(&test_ll, node3, NULL);
...
void test_ll_chg_list_middle_node(void)
{
    ...
    _lv_ll_chg_list(&test_ll, &dest_ll, node2, false);
...
void test_ll_clear_empty_list(void)
{
    ...
    _lv_ll_clear_custom(&test_ll, NULL);
    TEST_ASSERT_TRUE(_lv_ll_is_empty(&test_ll));
}
...
void test_ll_chg_list_tail_node(void)
{
    ...
    _lv_ll_clear(&test_ll);
    node1 = _lv_ll_ins_head(&test_ll);

    int32_t * head = _lv_ll_ins_head(&dest_ll);
    TEST_ASSERT_EQUAL_PTR(head, dest_ll.head);
    _lv_ll_chg_list(&test_ll, &dest_ll, node1, true);
...
{
    TEST_ASSERT_NULL(_lv_ll_ins_prev(NULL, NULL));
    TEST_ASSERT_NULL(_lv_ll_ins_prev(&test_ll, NULL));

    _lv_ll_remove(NULL, NULL);

    TEST_ASSERT_NULL(_lv_ll_get_head(NULL));
    TEST_ASSERT_NULL(_lv_ll_get_tail(NULL));
}
