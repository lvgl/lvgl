...
static lv_key_t remap_cb(lv_key_t key, void * user_data)
{
    switch(key) {
        case 'a':
            return LV_KEY_LEFT;
...
void test_indev_key_remap(void)
{
    ...
    lv_indev_set_key_remap_cb(lv_indev_active(), remap_cb, NULL);
    ...
    lv_indev_set_key_remap_cb(lv_indev_active(), NULL, NULL);

    lv_test_key_hit('a');
    lv_test_key_hit(LV_KEY_ESC);

    TEST_ASSERT_EQUAL_UINT32(4, key_buffer.count);
