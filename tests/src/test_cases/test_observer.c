...
void test_observer_int(void)
{
    ...
    TEST_ASSERT_EQUAL(0, lv_subject_get_previous_int(subject));
...
void test_observer_float(void)
{
    ...
    TEST_ASSERT_EQUAL_FLOAT(0, lv_subject_get_previous_float(subject));
    TEST_ASSERT_EQUAL(1, observer_called);
...
void test_observer_pointer(void)
{
    ...
    TEST_ASSERT_EQUAL_PTR(NULL, lv_subject_get_previous_pointer(subject));
...
void test_observer_color(void)
{
    ...
    TEST_ASSERT_EQUAL_COLOR(lv_color_black(),
...
{
    /*The group keeps the list by pointer, so it has to outlive the group Subject*/
    static lv_subject_t * list[5];
...
void test_observer_group_deinit_unsubscribes_from_elements(void)
{
    ...
    list[0] = subject_create(LV_SUBJECT_TYPE_INT);
    lv_subject_set_int(list[0], 1);
    list[1] = subject_create(LV_SUBJECT_TYPE_INT);
    lv_subject_set_int(list[1], 2);

    lv_subject_t * group_subject = subject_create(LV_SUBJECT_TYPE_GROUP);
    lv_subject_set_group_list_static(group_subject, list, 2);

    /* The group subscribed to both of its elements. */
    TEST_ASSERT_EQUAL(1, lv_ll_get_len(&list[0]->subs_ll));
    TEST_ASSERT_EQUAL(1, lv_ll_get_len(&list[1]->subs_ll));

    group_observer_called = 0;
    lv_subject_add_observer(group_subject, group_observer_cb, NULL);
    TEST_ASSERT_EQUAL(1, group_observer_called);

    /* Deleting the group must unsubscribe it from all its elements,
     * otherwise they keep a dangling observer pointing to the group. */
    subject_delete(group_subject);

    TEST_ASSERT_EQUAL(0, lv_ll_get_len(&list[0]->subs_ll));
    TEST_ASSERT_EQUAL(0, lv_ll_get_len(&list[1]->subs_ll));
...
/* Bind a flag from a boolean subject by passing a dedicated per-flag setter
 * directly to lv_obj_bind_bool. */
...
/* Toggle a state from a custom observer added with lv_subject_add_observer_obj. */
