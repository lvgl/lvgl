...
SKIPPED_FUNCTIONS = (
    ...
    "lv_font_get_glyph_dsc_fmt_txt",  # hot-path function that is also part of the public API for now
)
...
def _validation_patterns(param: str) -> list[re.Pattern]:
    p = re.escape(param)
    macros = "|".join(CHECK_MACROS)
    cond_macros = "|".join(CONDITION_CHECK_MACROS)
    pats = [
        # LV_CHECK_ARG(p != NULL, ...) / LV_CHECK_OBJ(p, cls, ...)
        re.compile(r"\b(?:" + macros + r")\s*\(\s*&?\s*" + p + r"\b"),
        # a compound check that mentions the parameter, e.g.
        # LV_CHECK_ARG(len == 0 || dst != NULL, ...)
        re.compile(
            r"\b(?:" + cond_macros + r")\s*\([^;]*?\b" + p + r"\s*(?:==|!=)\s*NULL"
        ),
        re.compile(
            r"\b(?:" + cond_macros + r")\s*\([^;]*?NULL\s*(?:==|!=)\s*" + p + r"\b"
        ),
    ]
    return pats
...
def is_validated(body: str, param: str) -> bool:
    """Whether `param` is checked in `body`. Pass `body` through `strip_noncode`."""
...
def collect(repo_root: Path, xml_dir: Path | None = None) -> dict:
    ...
    for func in sorted(api.public_functions(), key=lambda f: f.name):
        if func.header and any(func.header.startswith(d) for d in SKIPPED_HEADER_DIRS):
            continue
        if any(func.name == x for x in SKIPPED_FUNCTIONS):
            continue
...
def check_count(repo_root: Path, xml_dir: Path | None = None) -> bool:
    count = collect(repo_root, xml_dir)["pointer_params_missing"]
    print(count)
    return count == 0


def check_json(repo_root: Path, xml_dir: Path | None = None) -> bool:
    dic = collect(repo_root, xml_dir)
    count = dic["pointer_params_missing"]
    print(jsonlib.dumps(dic, indent=2))
    return count == 0
