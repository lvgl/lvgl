...
def is_validated(body: str, param: str) -> bool:
    return any(pat.search(body) for pat in _validation_patterns(param))
...
def collect(repo_root: Path, xml_dir: Path | None = None) -> dict:
    ...
    for func in sorted(api.public_functions(), key=lambda f: f.name):
        if any(func.header.startswith(d) for d in SKIPPED_HEADER_DIRS):
            continue
                ...
                missing.append(
                    {
                        ...
                        "null_mentioned_in_docs": bool(
                            param.doc and re.search(r"\bNULL\b", param.doc)
                        ),
                    }
                )
...
def check_audit(repo_root: Path) -> bool:
    ...
    for path in sorted(by_file):
        ...
        for item in sorted(by_file[path], key=lambda i: (i["function"], i["param"])):
            print(
                ...
                f"  {C.DIM}declared at {item['header']}:{item['header_line']}{C.RESET}"
            )
...
def check_count(repo_root: Path) -> bool:
    print(collect(repo_root)["pointer_params_missing"])
    return True


def check_json(repo_root: Path) -> bool:
    print(jsonlib.dumps(collect(repo_root), indent=2))
    return True
