#!/usr/bin/env python3
"""
Check that the file-name dependent parts of the lv_templ.c/lv_templ.h
templates match the actual file name.

Usage:
    check_file_template.py [--fix] [-q] [-v]
"""
...
c_pattern = re.compile(r"\s*/\*\*.+?" r"@file +(\S*)", flags=re.DOTALL)
...
def ignore_dir(path):
    rel = os.path.relpath(path, LVGL_ROOT_DIR)
    return rel.split(os.sep)[0] in ignored_top_dirs
...
arg_parser.add_argument(
    "-q", "--quiet", action="store_true", help="don't print mismatch info"
)
...
for root, dirs, basenames in os.walk(LVGL_ROOT_DIR):
    if ignore_dir(root):
        continue
