...
class LVDrawBuf(Value):
    ...
    def _get_bpp(self, color_format: int) -> int:
        """get bpp from color_format"""
        if color_format == int(gdb.parse_and_eval("LV_COLOR_FORMAT_RGB565")):
            return 16
        if color_format == int(gdb.parse_and_eval("LV_COLOR_FORMAT_RGB888")):
            return 24
        if color_format in (
            int(gdb.parse_and_eval("LV_COLOR_FORMAT_ARGB8888")),
            int(gdb.parse_and_eval("LV_COLOR_FORMAT_XRGB8888")),
        ):
            return 32
        return 0
    ...
    def color_format_info(self) -> dict:
        ...
        return {"value": cf, "name": fmt_name, "bpp": self._get_bpp(cf)}
    ...
    def _convert_to_image(
        ...
        try:
            if color_format == self._color_formats["RGB565"]:
                ...
                arr = np.frombuffer(bgr_data, dtype=np.uint8).reshape(height, width, 3)
...
class DumpDisplayBuf(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        ...
        for buf_name, buf_ptr in buffers.items():
            ...
            filename = f"{args.prefix if args.prefix else ''}{buf_name}.{args.format.lower() if args.format else 'bmp'}"
