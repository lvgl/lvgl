#!/usr/bin/env python3
"""
Shared report format for the PR report comment.
...
A report:
  ...
  {
    "section": "Static Checks",  # the row label, unique across all workflows
    "order":   10,               # optional, row order (default 50)
    "icon":    "ok",             # a name from ICONS below, or a raw emoji
...
# Rows that are expected to appear.
# New report not listed here is detected automatically and appended
# Anything listed but missing renders as "Pending"
EXPECTED_SECTIONS = [
    ("Static Checks", 10),
    ("Coverage", 20),
    ("Performance", 30),
...
def render(reports: list[dict], *, show_pending: bool = True) -> str:
    ...
    lines += [
        "",
        "<sub>Generated automatically</sub>",
    ]
    return "\n".join(lines) + "\n"
