...
class Function:
    ...
    brief: str = ""
    detail: str = ""
    # True for the memberdef that Doxygen emitted at the definition site; its
    # argsstring carries the parameter names the body actually uses.
    from_definition: bool = False
    # The signature exactly as the public header declares it. `params` above is
    # the definition's view, which is what the body refers to; these two differ
    # for the ~78 functions whose implementation renames an argument. Argument
    # checks want the definition's names, an API contract wants the header's.
    ...
    def parse(
        ...
        if xml_dir is None:
            with tempfile.TemporaryDirectory(prefix="lvgl-api-") as tmp:
                out_dir = Path(tmp)
                cls.generate_xml(out_dir, Path(repo_root), inputs)
                return cls._from_xml(out_dir / "xml", repo_root)

        out_dir = Path(xml_dir)
        xml_out_dir = out_dir / "xml"
        if not xml_out_dir.exists():
            cls.generate_xml(out_dir, Path(repo_root), inputs)

        return cls._from_xml(xml_out_dir, repo_root)

    @classmethod
    def generate_xml(cls, out_dir, repo_root, inputs):
        run_doxygen(repo_root, out_dir, inputs)
        return cls._from_xml(out_dir / "xml", repo_root)
...
def _function_from_memberdef(member: ET.Element, root: Path) -> Function | None:
    ...
    unknown = set(docs) - {p.name for p in params}
    if unknown:
        print(f"{name}: doc for unknown param(s) {sorted(unknown)}")
...
def _merge(prev: Function | None, new: Function) -> Function:
    ...
    # Carry over per-parameter docs, which only the documented entry has.
    # Match by position: the comment names the declaration's parameter, which
    # is exactly the name that differs for the ~78 renamed implementations.
    if documented is not definition and definition is not None:
        for i, p in enumerate(params):
            if not p.doc and i < len(documented.params):
                p.doc = documented.params[i].doc
