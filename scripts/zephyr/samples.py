...
The samples are GUI applications with no end condition, so a pass means the
sample kept running: each is given *seconds*, then stopped with SIGINT.

    exit 124 (the timeout fired) -> ran the whole window: pass
    exit 0                       -> exited on its own, which a GUI app should not
    anything else                -> crash (139 is SIGSEGV), assert, or abort

Neither subcommand fails on a broken sample; the workflow gates on the verdict.
"""
...
# 'dummy' alone leaves SDL without a renderer, so the software one is asked for
# explicitly. With both set no X server is needed.
...
# Icon names, not emoji: write_report() translates them and the renderer prints
# the stored field verbatim.
...
def pr_report():
    """``scripts/pr_report/pr_report.py``, loaded by path since ``scripts`` is
    not a package. None when absent: the check still runs, it just adds no row
    to the PR comment.
    """
...
# run
# --------------------------------------------------------------------------
def find_executables(outdir: Path) -> list[tuple[str, str, Path]]:
    """(platform, test name, executable) for every sample twister built.

    A sample built for more than one platform appears once per platform, so the
    platform is part of the key.
    """
    return [(exe.relative_to(outdir).parts[0], exe.parent.parent.name, exe)
...
def cmd_run(args) -> int:
    ...
    results = []
    for platform, name, exe in executables:
        res = run_one(name, exe, args.seconds)
        res["platform"] = platform
        results.append(res)
        print(f"{res['status']:5s} {platform} {name} ({res['note']})", flush=True)
...
def build_failures(suites: list[dict]) -> list[dict]:
    """Suites twister could not build. 'filtered' means the sample does not
    apply to this board, which is not a failure."""
    bad = []
    for s in suites:
...
def suite_key(suite: dict) -> tuple[str, str]:
    """(platform as twister names its output directory, test name)."""
    platform = re.sub(r"[/@]", "_", suite.get("platform", ""))
    return platform, suite["name"]


def run_key(result: dict) -> tuple[str, str]:
    return result.get("platform", ""), result["name"]


def expected_to_run(suites: list[dict]) -> list[tuple[str, str]]:
    """Suites that produced a binary and so must have a run result."""
    return [suite_key(s) for s in suites
...
def render_details(suites: list[dict], runs: list[dict], board: str, pin: str,
    ...
    lines += [
        "| | Sample | Platform | Build | Run |",
        "|:-:|---|---|---|---|",
    ]
    run_by_key = {run_key(r): r for r in runs}
    for s in sorted(suites, key=lambda x: (x["name"], x.get("platform", ""))):
        name = s["name"]
        status = (s.get("status") or "").lower()
        if status in ("error", "failed"):
        ...
        lines.append(f"| {icon} | `{name}` | `{s.get('platform', '')}` | "
                     f"{build_cell} | {run_cell} |")
    lines.append("")
    ...
    ran_keys = {run_key(r) for r in runs}
    never_ran = [n for n in expected_to_run(suites) if n not in ran_keys]
    if never_ran:
        lines += [
        ...
        lines += [f"- `{name}` on `{platform}`" for platform, name in never_ran]
        lines += [""]
...
def cmd_report(args) -> int:
    ...
    if args.setup_error:
        # No per-sample results exist, so say that rather than letting an empty
        # twister run look like "nothing was discovered".
    ...
    filtered = [s for s in suites if (s.get("status") or "").lower() == "filtered"]
    # Built but no run result means the run step died. Not counting it would let
    # "built and ran clean" through for samples that never ran.
    ran = {run_key(r) for r in runs}
    not_run = [key for key in expected_to_run(suites) if key not in ran]
    ...
    if failures or run_failures or not_run:
        ...
        # Twister never ran or found nothing; either way this verified nothing.
