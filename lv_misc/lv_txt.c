...
#define LV_TXT_LINE_BREAK_LONG_MIN_LEN  3
                              ...
                              lv_coord_t letter_space, lv_coord_t max_width, lv_txt_flag_t flag)
{
    ...
    uint32_t letter = 0;
    uint32_t char_cnt_from_last_break = 0;
    ...
    while(txt[i] != '\0') {
        ...
        if(letter == '\n' || letter == '\r') {
            ...
            char_cnt_from_last_break++;
            /*If the txt is too long then finish, this is the line end*/
            if(cur_w > max_width) {
                /*If already a break character is found, then break there*/
                if(last_break != NO_BREAK_FOUND && char_cnt_from_last_break < LV_TXT_LINE_BREAK_LONG_MIN_LEN) {
                    i = last_break;
                } else {
